package com.datablau.reverse.engineering.worker.greenplum;
 ...
 * @description: GreenplumWorker
 * @author: wang tong
 * @create: 2023-08-10 17:58
 **/
public class GreenplumWorker extends ReverseForwardStrategyJdbc implements JdbcPlugin {

    private static final Logger logger = LoggerFactory.getLogger(GreenplumWorker.class);
    ...
    protected void readEntireViewSQL(String catalog, String schema, ReversedSchema oSchema, ResultSet viewSqlResultSet) throws Exception {
        ...
        try (Statement stmt = jdbcDatasource.getConnection().createStatement()) {
            ...
            logger.error("Failed to get GREENPLUM database variables", e);
        }
    }
...
//                    "AND TABLE_NAME = '" + table.getName() + "'";
    ...
    protected void buildProcedures(ModelX model, String catalog) {
        // Procedures is defined as Functions in GREENPLUM
        return;
    }
    ...
    private int getMajorVersion(Connection connection) {
        try (Statement stmt = connection.createStatement();
            ...
            while(rs.next()) {
                ...
                logger.info("GREENPLUM version:" + version);
