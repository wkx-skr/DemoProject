...
import com.datablau.data.common.util.BaseControllerUtil;
import com.datablau.metric.management.type.DatablauServerType;
import com.datablau.metric.management.utility.RemoteServiceGetter;
...
public class BaseController {
    ...
    public void checkPermission(HttpServletRequest request) {
        BaseControllerUtil.checkPermission(RemoteServiceGetter.getRoleService(), DatablauServerType.Metric.name(), request);
    }
    ...
    public ErrorDto accessError(HttpServletResponse response, AccessDeniedException le) {
        ...
        ErrorDto error = new ErrorDto();
        error.setErrorMessage(GeneralUtility.getMessageService().getMessage("您没有权限操作或访问该功能"));
