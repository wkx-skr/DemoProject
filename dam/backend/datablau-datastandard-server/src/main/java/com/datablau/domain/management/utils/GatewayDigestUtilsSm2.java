 ...
 * @date: 2025/6/4 10:08
 */
public class GatewayDigestUtilsSm2 {
...
//    }
    ...
    // 工具类中新增：验签逻辑封装
    public static boolean verifySign(String plaintext, String signHex) throws Exception {
        byte[] signBytes = Hex.decode(signHex);
        byte[] data = plaintext.getBytes(StandardCharsets.UTF_8);

        var ecParams = GMNamedCurves.getByName(Sm2GmName);
        var domainParams = new ECDomainParameters(ecParams.getCurve(), ecParams.getG(), ecParams.getN());
        ECPoint pubPoint = ecParams.getCurve().decodePoint(Hex.decode(sm2PublicKeyHex));
