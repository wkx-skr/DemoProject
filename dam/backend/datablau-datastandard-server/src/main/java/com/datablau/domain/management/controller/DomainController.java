...
import com.datablau.project.util.CheckFileNameUtil;
import com.datablau.project.util.CheckNameUtil;
import com.datablau.project.util.SqlInjectionValidator;
...
public class DomainController extends BaseController {
    ...
    @Autowired
    private StandardServiceExt standardService;
    @Autowired
    ...
    public DomainTreeNodeDto getDomainTree(@RequestBody DomainQueryDto queryDto, HttpServletRequest request) {
        ...
        return domainFolderExtService.loadDomainTree(queryDto.getDomainState(), queryDto.getCategoryId(),
...
//    )
    ...
    public PageResult<DomainDto> getDomainPage(@RequestBody DomainQueryExtDto domainQueryDto,
                                               HttpServletRequest request) {
        //校验sql注入
        for (String orderColumn : domainQueryDto.getOrderColumn()) {
            SqlInjectionValidator.checkForSqlInjection(orderColumn, "orderColumn");
        }

        domainQueryDto.setCurrentUser(getCurrentUser(request));
        return domainExtServiceImpl.getPageDomains(domainQueryDto);
    }
    ...
    @Operation(summary = "分页获取一组已发布的数据标准")
    public PageResult<DomainDto> getUncheckPublicDomainPage(@RequestParam(value = "state", defaultValue = "A", required = false) DomainState state, @RequestBody DomainQueryDto domainQueryDto,
                                                     HttpServletRequest request) {
        domainQueryDto.setCurrentUser(getCurrentUser(request));
        domainQueryDto.setDomainState(state);
        return domainService.getPageDomains(domainQueryDto);
    }
    ...
    public DomainExtDto getDomainDetails(@RequestBody DomainQueryDto queryDto,
                                      HttpServletRequest request) {

        DomainExtDto domainExtDto = domainExtServiceImpl.getDomainById(queryDto.getDomainId());
        if (domainExtDto == null) {
...
//                domain.getDomainId(), LDMTypes.oDataStandard, getCurrentUser(request));
        ...
        //增加日志
        addDomainCommonLog(domainExtDto, "domain.log.query", OperationLogType.TABLE_QUERY);
    ...
    public DomainDto createDomain(
            @RequestBody DomainExtDto domain,
        ...
        DomainDto domainDto = domainExtServiceImpl.addDomain(getCurrentUser(request), domain, published);
    ...
    public void updateDomain(@RequestBody DomainExtDto domainDto, HttpServletRequest request) {
        //参数过滤
//        domainDto.filter();
        if(domainDto.getFolderId() == null){
            throw new RuntimeException("业务域不能为空");
        }

        domainExtServiceImpl.updateDomain(domainDto, getCurrentUser(request));

        //增加日志
        addDomainCommonLog(domainDto, "domain.log.modify", OperationLogType.TABLE_MODIFY);
    }
    @Operation(summary = "移动标准至其他目录")
    @Parameters({@Parameter(name = "id", description = "要移动得目录id", in = ParameterIn.QUERY)})
    @PostMapping("/move")
    public Boolean moveCatalog(@RequestParam("id") Long id, @RequestBody List<DomainDto> domainDtos) {
        return domainFolderExtService.moveFolder(domainDtos, id);
    }

    @Operation(summary = "移动参考数据至其他目录")
    @Parameters({@Parameter(name = "id", description = "要移动得目录id", in = ParameterIn.QUERY)})
    @PostMapping("/moveCode")
    public Boolean moveCode(@RequestParam("id") Long id, @RequestBody List<StandardCodeDto> codeDtos) {
        return domainFolderExtService.moveFolderForCode(codeDtos, id);
    }
...
//    )
    ...
    public void deleteDomainByIds(@RequestBody List<String> domainIds, HttpServletRequest request) {
        ...
        domainExtServiceImpl.removeDomainByIds(getCurrentUser(request), domainDtos);
    ...
    public String uploadDomains(@Parameter(name = "file", description = "导入数据标准的excel文件", required = true)
        ...
        CheckFileNameUtil.checkExcelFileName(multipartFile.getOriginalFilename());
        String user  = getCurrentUser(request);

        DataBlauHttpServletRequest dataBlauRequest = new DataBlauHttpServletRequest(request);
        return domainExtServiceImpl.uploadDomain(dataBlauRequest, user, multipartFile, published, categoryId, autoGenCode, ignoreError);
    }

    @PostMapping("/domain/uploadDomainTwo")
    public void uploadDomainTwo(@RequestParam("file") MultipartFile multipartFile, HttpServletRequest request) throws Exception {
        String user  = getCurrentUser(request);
        domainExtServiceImpl.uploadDomainTwo(user, multipartFile);
    }
    ...
    public void exportDomainTemplate(
        ...
        try {
            ...
            try {
                realName = URLEncoder.encode("标准数据元模板", "UTF-8");
...
//    )
    ...
    public StandardCodeDto addCode(@RequestBody StandardCodeFolderDto code, HttpServletRequest request) {
        StandardCodeDto codeDto = standardService.addCode(code, getCurrentUser(request));
    ...
    public StandardCodeDto updateCode(@RequestBody StandardCodeFolderDto code, HttpServletRequest request) {
        StandardCodeDto codeDto = standardService.updateCode(code, getCurrentUser(request));
    ...
    public StandardCodeFolderDto getCodeDetails(@RequestBody StandardCodeQueryDto queryDto) {
        StandardCodeFolderDto codeDto = standardService.getCodeByCodeNumber(queryDto.getCode(), queryDto.getCategoryId());
        //增加日志
        addStandardCommonLog(codeDto, "standard.log.query.base", OperationLogType.TABLE_QUERY);

        return codeDto;
    }
    @PostMapping("/code/codeInfo")
    @Operation(summary = "得到某一个标准代码的内容")
    public StandardCodeDto getCodeInfos(@RequestBody Map<String, Object> queryDto) {
        String code = queryDto.getOrDefault("codeID", "").toString();
        String codeVersionStr = queryDto.getOrDefault("codeVersion", "").toString();
        if(StringUtils.isEmpty(codeVersionStr) || StringUtils.isEmpty(code)) {
            throw new InvalidArgumentException("params is invalid.");
        }
        Integer codeVersion;
        try {
            codeVersion = Integer.valueOf(codeVersionStr);
        } catch (Exception e) {
            LOGGER.error("codeVersion is invalid.");
            return null;
        }

        StandardCodeDto codeDto = domainService.getCodeByCodeNumber(code, DomainService.DOMAIN_CATEGORY_ID);
        if(!Objects.equals(codeDto.getVersion(), codeVersion)) {
            List<StandardCodeDto> allOldCodeVersions = domainService.getAllOldCodeVersions(code, DomainService.DOMAIN_CATEGORY_ID);
            codeDto = allOldCodeVersions.stream()
                    .filter(v -> Objects.equals(v.getVersion(), codeVersion))
...
//        }
    ...
    public void deleteCodes(@RequestBody StandardCodeQueryDto queryDto, HttpServletRequest request) {
        ...
        domainExtServiceImpl.deleteCodeByCodeNumbers(queryDto.getCodes(), getCurrentUser(request),
                        ...
                        try {
                            ...
                            try {
                                if(categoryId == DomainService.DOMAIN_CATEGORY_ID) {
                                    result = standardService.importStandardCode(uploadedFile, published, autoGenCode, ignoreError, user);
                                } else {
    ...
    @PostMapping("/code/uploadDomainTwo")
    public void uploadCodeTwo(@RequestParam("file") MultipartFile multipartFile, HttpServletRequest request) throws Exception {
        String user  = getCurrentUser(request);
        File uploadedFile = DataUtility.uploadFile(multipartFile);
        boolean published = false;
        boolean autoGenCode = true;
        boolean ignoreError = false;
        standardService.uploadCodeTwo(uploadedFile, published, autoGenCode, ignoreError, user);
    }
...
//    )
    ...
    public DomainFolderDto createFolder(@RequestBody DomainFolderExtDto folder, HttpServletRequest request) {
        ...
        if(folder.getParentId() == DomainService.DOMAIN_CATEGORY_ID) { // 基础标准的一级目录校验bizCode
            if(StringUtils.isBlank(folder.getBizCode())) {
                throw new IllegalArgumentException("业务域编码不能为空！");
            }
        }
        String currentUser = getCurrentUser(request);
        return domainFolderExtService.createFolder(folder, currentUser);
    }
...
//    )
    ...
    public DomainFolderDto updateCategory(@RequestBody DomainFolderExtDto folder) {
        if (!AuthTools.hasAnyRole(AuthTools.ROLE_SUPERUSER)) {
            if (!domainCategoryPermissionService.hasPermissionToFolder(AuthTools.currentUsernameFailFast(),
        ...
        return domainFolderExtService.updateFolder(folder);
    }
...
//    )
    ...
    public void deleteCategory(@RequestBody DomainFolderResDto folderResDto) {
        ...
        domainFolderExtService.deleteFolder(folderResDto);
    }
...
//    )
    ...
    /**
     * 标准概览-气泡图：各业务域（一级目录）已发布数据元标准的总数
     */
    @PostMapping(value = "/getBusinessCount")
    @Operation(summary = "标准概览-气泡图：各业务域（一级目录）已发布数据元标准的总数")
    //@PreAuthorize("hasAnyRole('ROLE_SUPERUSER', 'DATA_STANDARD_DASHBOARD')")
    public Map<String, Long> getBusinessCount() {
        return domainServiceExt.getBusinessCount();
    }

    /**
     * 标准概览-气泡图：各业务域（一级目录）已发布数据元标准的总数
     */
    @PostMapping(value = "/getBusinessTermCount")
    @Operation(summary = "业务域发布业务术语top10")
    public Map<String, Long> getBusinessTermCount() {
        return domainServiceExt.getBusinessTermCount();
    }

    @PostMapping(value = "/getBusinessTermPublishCount")
    @Operation(summary = "业务域发布业务术语top10")
    public StatisticsDtoExt getBusinessTermPublishCount() {
        return domainServiceExt.getBusinessTermPublishCount();
    }

    /**
     * 治理数据标准量（个）：已废弃的个数（即被治理的数量），数据标准总数（已发布+已废弃）
     */
    @PostMapping(value = "/getDomainCount")
    @Operation(summary = "治理数据标准量（个）：已废弃的个数（即被治理的数量），数据标准总数（已发布+已废弃）")
    //@PreAuthorize("hasAnyRole('ROLE_SUPERUSER', 'DATA_STANDARD_DASHBOARD')")
    public DomainCountDto getDomainCount() {
        return domainServiceExt.getDomainCount();
    }

    /**
     * 6.	近一年数据元标准发布趋势：折线图展示从上个自然月往前12个月中，每个月发布的数据元标准（发布日期为该月）、
     * 变更的数据元标准（在该月产生的新版本数量）、废弃的数据元标准（在该月废弃的数据元标准）。
     */
    @PostMapping(value = "/getDomainCountByMonth")
    @Operation(summary = "近一年数据元标准发布趋势：折线图展示")
    //@PreAuthorize("hasAnyRole('ROLE_SUPERUSER', 'DATA_STANDARD_DASHBOARD')")
