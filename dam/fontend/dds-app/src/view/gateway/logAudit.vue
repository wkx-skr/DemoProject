      ...
      <template v-if="treeShow">
        ...
        <div class="tree">
          <datablau-tree
            ...
            node-key="id"
            :empty-text="treeList.length === 0 ? '暂无目录信息' : ''"
          ></datablau-tree>
        </div>
      </template>
    </div>
        ...
        <template>
          <div class="top-header-box" ref="topBox" v-if="modelId">
            ...
            <div class="brief-box" v-if="modelInfo.definition">
              ...
              <div class="intro">
                <div class="intro-item" v-if="addressMap.parameterMap">
                  <span>数据源地址：</span>
                  {{ addressMap.parameterMap.HostServer }}:{{
                    addressMap.parameterMap.PortNumber
                  }}
                </div>
            ...
            >
              ...
              >
                <template slot-scope="scope">
                  <template
                    v-if="
                      scope.row.tableLogDtos &&
                      scope.row.tableLogDtos.length > 0
                    "
                  >
                    <div
                      class="log-type"
...
export default {
  ...
  methods: {
    ...
    getAddress() {
      API.logAuditAddressAPI(this.modelId).then(res => {
        const data = res.data.data
        if (data.length > 0) {
          this.addressMap = data[0]
        }
      })
    },
    getModelDetail() {
