<template>
  <div class="overview" style="background: #fff" v-loading="loading">
    ...
    <div class="picTil" v-if="datas.length === 0">
      {{ $t('assetCount.assetsOverview') }}
    </div>

    <lineAndBarCom
      :picTil="$t('assetCount.assetsOverview')"
      :options="options"
      :formatter="true"
      ...
      HTTP.getDataAsset()
        .then(res => {
            ...
            datas[k].forEach(data => {
              ...
              if (findAry.length) {
                ...
                ary.push({
                  ...
                  data: [data.total],
                  type: 'bar',
                  color: data.color || this.color[index],
                  stack: 'Ad',
                  emphasis: {
                    focus: 'series',
                  },
          ...
          this.$set(this.options, 'axisLabel', {
            formatter: item => {
              const name = item.length > 6 ? item.substr(0, 6) + '…' : item
              return name
            },
          })
          // 设置滚动条
