<template>
  <div class="overview" style="background: #fff" v-loading="loading">
    ...
    <div class="title-tip">
      <datablau-tooltip
        effect="dark"
        :content="$t('assetCount.businessTitle')"
        placement="right"
    ...
    <div class="picTil" v-if="datas.length === 0">
      {{ $t('assetCount.assetsSituation') }}
    </div>
    <div class="tabChang">
      <span :class="{ active: tab === 'business' }" @click="tabCh('business')">
        {{ $t('assetCount.businessUnit') }}
      </span>
      <span
      ...
      >
        {{ $t('assetCount.businessSystem') }}
      </span>
    </div>
    <lineAndBarCom
      :picTil="$t('assetCount.assetsSituation')"
      :options="options"
      :clear="clear"
      :formatter="true"
      :yCategory="true"
...
<script>
import lineAndBarCom from '@/view/dataSecurity/echart/lineAndBarCom'
      ...
      HTTP.getbusinessAsset()
        .then(res => {
          ...
          Object.keys(data).forEach(k => {
            ...
            rs.sort((a, b) => a.tagId - b.tagId).forEach(tag => {
              let findAry = ary.filter(item => item.name === tag.tagName)
              max += Number(tag.value)
              if (findAry.length) {
                ary.forEach(item => {
                  item.name === tag.tagName && item.data.push(tag)
                })
              } else {
      ...
      HTTP.getSystemAsset()
        .then(res => {
          ...
          this.datas = res.data.data.map(item => {
            return {
              ...item,
              statistics: item.statistics.sort((a, b) => a.tagId - b.tagId),
            }
          })
