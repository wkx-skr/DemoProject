<template>
  <div class="overview" style="background: #fff" v-loading="loading">
    ...
    <pieChartCom
      v-if="!!datas.length"
      :picTil="$t('assetCount.assetsGradeOverview')"
      :datas="datas"
      :label="false"
      :options="options"
      :co="colors"
    ...
    <div class="picTil" v-if="datas.length === 0">
      {{ $t('assetCount.assetsGradeOverview') }}
    </div>
...
<script>
import pieChartCom from '@/view/dataSecurity/echart/pieChartCom'
    ...
    getClassification() {
      ...
      HTTP.getClassification()
        .then(res => {
          ...
          rs.sort((a, b) => a.tagId - b.tagId).forEach(item => {
            const { color, value } = item
            !!Number(value.split('_')[1]) &&
              this.datas.push({
                value: Number(value.split('_')[1]),
                name: item.tagName,
