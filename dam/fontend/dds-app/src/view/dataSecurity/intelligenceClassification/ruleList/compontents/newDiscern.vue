<template>
  <div class="new-discern-page">
    ...
    <datablau-dialog
      width="480px"
      :isTree="true"
      :title="$t('intelligence.selectSecClass')"
    ...
    <datablau-form-submit v-if="editable" v-loading="detailLoading">
      ...
      <datablau-form :model="formContent" ref="demoForm" :rules="rules">
        ...
        <el-form-item
          ...
          :label="$t('intelligence.idTarget')"
        >
          <datablau-radio v-model="discernTarget" @change="discernTargetChange">
            <el-radio label="AUTH_STANDARD">
              {{ $t('intelligence.infoItem') }}
            </el-radio>
            <el-radio label="SECURITY_CATEGORY">
              {{ $t('intelligence.securityClass') }}
            </el-radio>
          </datablau-radio>
        </el-form-item>
        <el-form-item
          ...
          v-if="
            ...
            discernTarget === 'SECURITY_CATEGORY'
          "
        ...
        <el-form-item
          class="is-required"
          :label="$t('intelligence.selectSecClass')"
          v-if="
            ruleType === ruleTypeEnum.GENERAL_RULE &&
            discernTarget === 'SECURITY_CATEGORY'
          "
        ...
        >
          <datablau-button
            class="iconfont icon-tianjia"
            @click="addClassifyCatalog"
          >
            {{ $t('intelligence.selectSecClass') }}
          </datablau-button>
          <span style="margin-left: 8px">
            {{
              $t('intelligence.selectTip', {
                num: classifyCatalogList.length,
              })
            }}
          </span>
          <div
          ...
          >
            <datablau-tag
              v-for="item in classifyCatalogList"
              :key="item.id"
              closable
              @close="deleteClassify(item)"
            >
              <span class="target-tag">
                <is-show-tooltip
                  :content="item.name"
        ...
        <el-form-item
          ...
          "
          :label="$t('intelligence.idInfoItem')"
          prop="infoItem"
        >
          <datablau-input
            @focus="getInfoItem"
    ...
    >
      ...
      <el-form-item
        v-if="
          ...
          classifyCatalogList.length > 0
        "
        :label="$t('securityModule.securityClassify')"
      >
        {{ classifyCatalogList.map(m => m.name).join(',') }}
      </el-form-item>
      <el-form-item
        v-if="ruleType === ruleTypeEnum.MACHINE_LEARNING"
        :label="$t('intelligence.algorithmStudy')"
