  ...
  methods: {
    ...
    queryCatalog(data = null) {
      if (data) {
        ...
        let catalogPathName = ''
        const pathArr = data.catalogPath
          .split('/')
          .slice(1)
          .filter(i => !!i)
        if (pathArr.length) {
          catalogPathName =
            pathArr.map(p => this.catalogIdMap[p].name).join('/') +
            '/' +
            data.name
        } else {
          catalogPathName = data.name
        }
        data.catalogPathName = catalogPathName
      } else {
        this.sensitiveId = ''
      }
        ...
        .then(res => {
          ...
          if (isLazy) {
            ...
            treeList.forEach(node => {
              this.catalogIdMap[node.id] = node
            })
