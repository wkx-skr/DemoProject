<template>
  <div class="detailSta" v-loading="loading">
    ...
    <div class="fileDetail">
      <div class="enclosure" @click="enclosureDownload">
        {{ $t('statutes.downloadFile') }}
      </div>
      <h2 class="til">
        {{ title }}
        <!--        <p>GBT-35273-2017</p>-->
      ...
      <div class="staDetail">
        <ul class="flex">
          <li>
            <span>{{ $t('statutes.regulationBrief') }}：</span>
            <span class="decText">{{ detail.regulationShortName }}</span>
          </li>
          <li>
            <span>{{ $t('statutes.regulationSource') }}：</span>
        ...
        <ul class="rightUl">
          <li>
            <span>{{ $t('statutes.upLoadDep') }}：</span>
            {{ detail.deptCode }}
          </li>
          <li>
            <span>{{ $t('statutes.publishDep') }}：</span>
            {{ detail.pubDeptCode }}
          </li>
          <li>
            <span>{{ $t('statutes.upLoader') }}：</span>
            {{ detail.uploader }}
          </li>
          <li>
            <span>{{ $t('statutes.upLoadTime') }}：</span>
...
export default {
  ...
  methods: {
    ...
    enclosureDownload() {
      let url = `/datasecurity/datasecurity/regulation/download/${this.detail.fileId}`
