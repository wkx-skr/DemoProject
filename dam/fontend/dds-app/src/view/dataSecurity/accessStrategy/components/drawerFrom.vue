  ...
  <div class="drawerCon" v-loading="loading">
    ...
    >
      ...
      <datablau-detail-subtitle
        :title="$t('securityModule.securityAttr')"
        mb="5px"
      ></datablau-detail-subtitle>
      <div class="security">
        <checkboxCom
      ...
      <!--数据标签-->
      <datablau-detail-subtitle
        :title="$t('securityModule.dataTag')"
        mb="5px"
      ></datablau-detail-subtitle>
      <div class="dataLabel">
        <checkboxCom
...
export default {
  ...
  computed: {
    securityAttributes() {
      return [
        this.$t('securityModule.securityLevel'),
        this.$t('securityModule.importance'),
        this.$t('securityModule.affectedObjects'),
        this.$t('securityModule.reach'),
        this.$t('securityModule.impactLevel'),
      ]
    },
  },
  methods: {
        ...
        .then(res => {
          if (res.data && Array.isArray(res.data)) {
            ...
            ary.forEach(item => {
              ...
              if (item.name === '数据安全等级') {
                item.name = this.$t('securityModule.securityLevel')
              }
