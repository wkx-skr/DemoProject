      ...
      <div class="content">
        <div>
          <datablau-input
            v-model="nameInput"
            :iconfont-state="true"
            :placeholder="$t('accessStrategy.keywordPlaceholder')"
            clearable
            class="nameInput"
            @input="keyWord"
            @clear="inputClear"
          ...
          <span>
            {{
              selectedList.length > 0
                ? $t('accessStrategy.selectNum', {
                    num: selectedList.length,
                  })
                : $t('accessStrategy.noSelect')
            }}
          </span>
...
export default {
  props: {
    ...
    title: {
      type: String,
      default() {
        return this.$t('accessStrategy.addUser')
      },
    },
    currentNode: {
      type: Object,
  ...
  data() {
    return {
      ...
      manageList: [
        {
          label: this.$t('accessStrategy.manage'),
          value: 'MANAGER',
        },
        { label: this.$t('accessStrategy.edit'), value: 'EDIT' },
        { label: this.$t('accessStrategy.visit'), value: 'READ' },
      ],
      delPermision: [],
      checkedKeys: [],
      page: 1,
  ...
  methods: {
    ...
    lazyloading() {
      this.page++
      // this.title === this.$t('accessStrategy.organizationName') &&
    ...
    // 关键字搜
    keyWord(val) {
      clearTimeout(this.firstTimeout) // 防抖6.5.0
      this.firstTimeout = setTimeout(() => {
        if (this.title === this.$t('accessStrategy.addUser')) {
