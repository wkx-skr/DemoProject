...
export default {
  ...
  mounted() {
    this.typeList = [
      {
        type: 'USER_DEFINED',
        value: this.$t('algorithm.custom'),
      },
      {
        type: 'BUILT_IN',
        value: this.$t('algorithm.builtIn'),
      },
    ]
  ...
  methods: {
    ...
    // 下载算法模板
    modelDownload() {
      const url = `/datasecurity/datasecurity/algorithm/download`
      this.$downloadFilePost(url, {}, this.$t('algorithm.algorithmTem'))
    },
    ...
    beforeUpload(file) {
      if (!this.checkFileValidation(file, ['xlsx'])) {
        this.$datablauMessage.error(this.$t('algorithm.selUploadType'))
        return false
      }
    },
    // 导入文件上传失败
    ...
    addAlgorithm() {
      this.selections = []
      this.curName = this.$t('algorithm.newAlgorithm')
      this.setBreadcrumb()
      this.isEdit = false
      this.isView = false
      this.newShow = true
    ...
    editAlgorithm(row) {
      this.selections = []
      this.curName = this.$t('algorithm.editAlgorithm')
          ...
          API.judgeAlgorithm(row.algorithmId)
            .then(res => {
              if (res.data.data) {
                ...
                this.$DatablauCofirm(this.$t('algorithm.modifyTip'))
                  .then(() => {
                    this.editAlgorithm(row)
                  })
                  .catch(() => {})
    ...
    handleView(row) {
      this.selections = []
      this.curName = this.$t('algorithm.viewAlgorithm')
      this.setCurBreadcrumb(row.catalogPath)
      this.algorithmId = row.algorithmId
      this.isView = true
      this.newShow = true
      ...
      API.delAlgorithm(ids)
        .then(async res => {
          ...
          let mapList = [
            {
              tip: '',
              name: this.$t('algorithm.successDel'),
              icon: 'error',
              type: 'number',
              prop: 'sucNums',
            },
            {
              tip: this.$t('algorithm.delTip'),
              type: 'array',
              name: this.$t('algorithm.failDel'),
              icon: 'error',
              prop: 'hasChildren',
            },
            {
              tip: this.$t('algorithm.delTip1'),
              type: 'array',
              icon: 'error',
              name: this.$t('algorithm.failDel'),
              prop: 'ref',
