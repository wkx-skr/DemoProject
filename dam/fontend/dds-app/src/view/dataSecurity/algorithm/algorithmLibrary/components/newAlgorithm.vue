  ...
  <div class="new-algorithm-page">
    ...
    <datablau-form-submit style="top: 10px">
      <datablau-form :model="formContent" ref="demoForm" :rules="rules">
        ...
        <el-form-item
          :label="$t('algorithm.fatherAlName1')"
          class="remote-form-item"
        >
          <datablau-select
            filterable
            clearable
        ...
        <el-form-item :label="$t('algorithm.algorithmType')">
          <datablau-radio v-model="formContent.type">
            <el-radio label="REGEX">{{ $t('algorithm.expression') }}</el-radio>
          </datablau-radio>
        </el-form-item>
        <el-form-item
          :label="$t('algorithm.algorithmContent')"
        ...
        <el-form-item :label="$t('algorithm.test')">
          <datablau-input
            :placeholder="$t('securityModule.input')"
  ...
  async mounted() {
    this.rules = {
      algorithmName: [
        {
          required: true,
          message: this.$t('algorithm.inputAlName'),
          trigger: 'change',
        },
      ],
      catalogName: [
        ...
        if (!valid) {
          ...
          API.testAlgorithm(params)
            .then(res => {
              ...
              if (res.data.data) {
                this.testResult = this.$t('algorithm.accordRule')
                this.isError = false
              } else {
                this.isError = true
                this.testResult = this.$t('algorithm.notAccordRule')
              }
            })
            .catch(e => {
