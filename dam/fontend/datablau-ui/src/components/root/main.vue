...
export default {
  ...
  methods: {
    ...
    setTitle() {
      ...
      let currentProduct = localStorage.getItem('currentProduct')||'dam'
      $('title').text(titleMap[currentProduct]);
      $('#title').text(titleMap[currentProduct]);
    },
    useLastTopMenu() {
    ...
    handleDirectoryData(res) {
      ...
      let currentProduct = localStorage.getItem('currentProduct')||'dam'
      pagesTree = $directory.products[currentProduct].pages
    ...
    updateRoute(routeName) {
      ...
      if (this.isFirstRoute && !location.href.includes('userModal')&& !location.href.includes('searchGlobal')) {
