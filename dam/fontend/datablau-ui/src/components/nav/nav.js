  ...
  methods: {
    handleTopMenuChange({ level1, byUser }) {
      if (!level1) {
        ...
        } else if (location.href.includes('/searchGlobal')) {
          this.$emit('trigger-hide-nav-controller', 'searchGlobal')
        } else {
        ...
        if (this.env.toLowerCase().startsWith('prod')) {
          ...
          if (
            !location.pathname.includes(
              this.pagesMap.get(currentNode).appName
            ) &&
            !this.$parent.isFirstRoute
          ) {
            if (!byUser) {
      ...
      let currentNode = Object.values(level1)[0][0]
      if (
        !location.pathname.includes(this.pagesMap.get(currentNode).appName) ||
        this.$parent.isFirstRoute
      ) {
        // 如果跨app了，需要一下逻辑来匹配当前处于哪个菜单。
        let goal = false
    ...
    openUserModal(args) {
      ...
      if (args === 'searchGlobal') {
        const NewPage = $directory.pagesMap['searchGlobal']
        if (this.env.toLowerCase().startsWith('dev')) {
          location.href = `${location.protocol}//${location.hostname}:${
            this.appsMap.get(NewPage.appName).devPort
          }/#${NewPage.vueRouter}`
        } else if (this.env.toLowerCase().startsWith('prod')) {
          location.href = `${location.origin}/${
            this.appsMap.get(NewPage.appName).productionPath
          }/#${NewPage.vueRouter}`
        }
        this.$emit('trigger-hide-nav-controller', 'searchGlobal')
      } else {
    ...
    handleMenuSelect(index) {
      ...
      if (index) {
        const newPageUrlGenerator = () => {
          if (NewPage.isMapPage) {
            if (this.env.toLowerCase().startsWith('dev')) {
              return `${location.protocol}//${location.hostname}:${
                this.appsMap.get(NewPage.mapAppName).devPort
              }/#${NewPage.mapVueRouter}`
            } else if (this.env.toLowerCase().startsWith('prod')) {
              return `${location.origin}/${
                this.appsMap.get(NewPage.mapAppName).productionPath
              }/#${NewPage.mapVueRouter}`
            } else {
        ...
        const isIdenticalProductByUrl = () => {
          if (this.env.toLowerCase().startsWith('dev')) {
            return (
              location.port ===
              String(this.appsMap.get(NewPage.appName).devPort)
            )
          } else if (this.env.toLowerCase().startsWith('prod')) {
        ...
        if (
          this.oldPage &&
          $directory.pagesMap[this.oldPage].appName === NewPage.appName
        ) {
          // 同app
    ...
    hideInMenuItemValues(level1Value) {
      return level1Value.filter(
        (i) => this.pagesMap.get(i) && !this.pagesMap.get(i).hideInMenu
      )
    },
