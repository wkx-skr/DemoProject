            ...
            <div v-else-if="col.colType === 'customCol'">
              ...
              >
                <p
                  :class="
                    col.customClass &&
                    typeof col.customClass === 'function' &&
                    col.customClass(scope.row)
                  "
                >
                  {{
                    col.customColName &&
                    typeof col.customColName === 'function' &&
...
export default {
  ...
  methods: {
    ...
    initSelection(selections, prop) {
      if (selections && selections.length > 0) {
        selections.forEach(item => {
          const data = this.tableData.find(td => td[prop] === item)
          data && this.$refs.deTable.$refs.table.toggleRowSelection(data, true)
        })
      }
    },
