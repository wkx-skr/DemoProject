  ...
  <div class="db-richtip" v-loading="showLoading">
    ...
    >
      ...
      <span
        ...
        slot="reference"
        :style="{ color: canClick ? '409EFF' : '#555' }"
      >
        {{ label }}
      </span>
    </el-popover>
...
export default {
  ...
  computed: {
    canClick() {
      return this.dataType === LDMTypes.CODE
        ? this.$auth.STANDARD_CODE_VIEW
        : this.dataType === LDMTypes.Domain
        ? this.$auth.DATA_STANDARD_VIEW
        : false
    },
  },
  methods: {
    jumpTo() {
      if (this.dataType === LDMTypes.CODE && this.dataId) {
        if (!this.$auth.STANDARD_CODE_VIEW) return
        let query = {
          name: 'code',
          query: {
            code: this.dataId,
        ...
        if (!this.$auth.DATA_STANDARD_VIEW) return
