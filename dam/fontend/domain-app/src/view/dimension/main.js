...
export default {
  data() {
    return {
      ...
      dialogVisible: false,
      dialogFunction: null,
      dialogPlaceholder: this.$t('domain.common.pleaseEnter'),
  ...
  methods: {
    ...
    treeIconFunction(data, node) {
      ...
      if (data.type) {
        ...
        result = 'tree-icon dim'
      }
      return result
    },
    dataOptionsFunction(data) {
      this.anotherDim = data
      const options = []
      if (this.$auth.DATA_STANDARD_DIM_CATALOG_ADD && data.type === 'FOLDER') {
        options.push({
          icon: 'iconfont icon-tianjia',
          label:
            this.$t('domain.common.add') +
            this.$t('domain.common.spacing') +
            this.typeLabel,
          callback: () => {
        ...
        if (this.$auth.DATA_STANDARD_DIM_CATALOG_VALUE_ADD) {
          options.push({
            icon: 'iconfont icon-tianjia',
            label: this.$t('domain.dimension.addValue'),
            callback: () => {
              this.callDialog('value', 'add')
            },
            args: 'folder',
          })
        }
        if (this.$auth.DATA_STANDARD_DIM_CATALOG_RENAME) {
          options.push({
            icon: 'iconfont icon-revise',
            label: this.$t('domain.common.rename'),
            callback: () => {
        ...
        if (this.$auth.DATA_STANDARD_DIM_CATALOG_DELETE) {
          options.push({
            icon: 'iconfont icon-delete',
            label: this.$t('domain.common.delete'),
            callback: () => {
              this.deleteDims()
            },
            args: 'folder',
    ...
    handleRenameCategory() {
      this.showCodePop = false
      this.categoryName = this.anotherDim.catalog
      this.categoryId = this.anotherDim.catalogId
      this.showPop = false
    ...
    renameCategory() {
      if (!this.categoryName) {
        this.$message.warning(
          this.$t('domain.dimension.categoryNamePlaceholder')
        )
      } else {
        if (this.currentType === 'MONITOR') {
          this.$http
            .post(this.$url + '/service/me/monitors/catalogs', {
              catalogId: this.anotherDim.catalogId,
          ...
          this.$http
            .post(this.$domain_url + '/me/dims/catalogs', {
    ...
    addCategory() {
      if (!this.categoryName) {
        ...
        this.$message.error(
          this.$t('domain.dimension.codeDuplication', { code: this.categoryId })
        )
      } else {
        if (this.currentType === 'MONITOR') {
          this.$http
    ...
    deleteDim(row) {
      this.$DatablauCofirm(
        this.$t('domain.common.deleteConfirm'),
        ...
        .then(() => {
          let requestUrl = this.$domain_url + '/me/dims/'
          if (this.currentType === 'MONITOR') {
    ...
    deleteDims() {
      this.$DatablauCofirm(
        this.$t('domain.dimension.dimDeleteConfirm'),
        this.$t('domain.common.delete'),
        {
          type: 'warning',
        }
        ...
        .then(() => {
          this.showCodePop = false
          const catalogId = this.anotherDim.catalogId
          const values = this.dims

          const length = values.length
          let cnt = 0
          // this.dims = []
          this.anotherDim = null
          if (values.length === 0) {
    ...
    deleteDimCatalog(catalogId) {
      let requestUrl = this.$domain_url + '/me/dims/catalogs/'
      if (this.currentType === 'MONITOR') {
        requestUrl = this.$domain_url + '/me/monitors/catalogs/'
      }
      this.$http
    ...
    callDialog() {
      ...
      if (arguments[0] === 'value') {
        ...
        if (this.currentType != 'MONITOR') {
          typeText +=
            this.$t('domain.common.spacing') + this.$t('domain.dimension.value')
        }
        this.dialogPlaceholder =
          this.$t('domain.common.pleaseEnter') +
          this.$t('domain.common.spacing') +
          typeText
        ...
        this.dialogFunction = () => {
          if (this.currentType === 'MONITOR') {
            ...
            requestBody = {
              catalog: arguments[2] ? this.currentDim : this.anotherDim,
            }
            ...
            if (arguments[1] === 'add' && this.dimsIdSet.has(this.dialogId)) {
              this.$message.error(
                this.$t('domain.dimension.codeDuplication', {
                  code: this.dialogId,
                })
              )
              return
            ...
            this.$http
              .post(this.$domain_url + '/me/dims', requestBody)
              .then(res => {
                this.$message.success(this.$version.common.operationSucceed)
                this.getDims()
                this.abortHandleDialog()
    ...
    handleDialog() {
      ...
      if (!dialogName) {
        this.$message.warning(
          this.$t('domain.dimension.labelCannotEmpty', {
    ...
    codeSelectedNew(selectedCode) {
      ...
      this.$http
        .post(this.$domain_url + '/me/dims/catalogs', requestBody)
        .then(res => {
          this.codeUploading = true
          this.$http
            .get(this.$meta_url + '/domains/codes/content?codeNumber=' + code)
            .then(res => {
              ...
              const errMap = {}
              if (length === 0) {
                this.$message.success(this.$t('domain.common.importSuccessful'))
                this.codeUploading = false
                this.refresh()
              }
              values.forEach(item => {
                this.$http
                  .post(this.$domain_url + '/me/dims', {
                    catalog: requestBody,
                  ...
                  .then(res => {
                    ...
                    if (cnt === length) {
                      this.$message.success(
                        this.$t('domain.common.importSuccessful')
                      )
                      this.codeUploading = false
                      this.refresh()
                    }
              ...
              this.timer = setTimeout(() => {
                let showHTML = `<div>${this.$t(
                  'domain.dimension.errorCountMessage',
                  {
                    length,
                    errorCnt,
                  }
                ...
                for (const key in errMap) {
                  if (errMap[key].length === 1) {
                    source = `<br><div>${this.$t(
                      'domain.dimension.errorMessageDetailSingle',
                      {
                        key,
                      }
                    )}</div>`
                    ...
                    source = `<br><div>${this.$t(
                      'domain.dimension.errorMessageDetailMultiple',
                      {
                        length: errMap[key].length,
                        key,
                      }
                    )}</div>`
                  }
    ...
    codeSelected() {
      ...
      this.$http
        ...
        .then(res => {
          this.codeUploading = true
          this.$http
            .get(this.$domain_url + '/domains/codes/content?codeNumber=' + code)
            .then(res => {
              values = res.data.values
              const length = values.length
              const r_cnt = 0 // request count
    ...
    valueValidator(rule, value, callback) {
      ...
      if (value && value.indexOf('/') !== -1) {
        const errorString = this.$t('domain.dimension.codeErrorCharacter')
    ...
    valueValidator2(rule, value, callback) {
      value = this.dialogId
      if (value && value.indexOf('/') !== -1) {
        callback(this.$t('domain.dimension.codeErrorCharacter'))
      } else {
        callback()
      }
    },
  ...
  computed: {
    typeLabel() {
      ...
      if (type === 'MONITOR') {
        return this.$t('domain.dimension.observationObject') // '观测对象'
      } else if (type === 'NORMAL') {
        return this.$t('domain.dimension.dimension') // '维度'
      } else {
        return this.$t('domain.dimension.timePeriod') // '时间周期'
      }
    },
  },
  watch: {
