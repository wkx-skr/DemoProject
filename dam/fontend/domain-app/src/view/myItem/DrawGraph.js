  ...
  drawStep (msg, graph, parent) {
    ...
    if (!this.param.showColumn && !this.param.showModel && !msg.properties.databaseName) {
      ...
      if (this.param.showColumn) {
        if (!msg.properties.$input_table && !msg.properties.$output_table && self.param.collapseSteps) {
          tableHtml += '<i class="fa fa-plus step-icon" aria-hidden="true" id="step-' + msg.id + '"> </i>'
        } else {
          tableHtml += '<i class="fa fa-minus step-icon" aria-hidden="true" id="step-' + msg.id + '"></i>'
        }
      }
    ...
    if (this.param.showColumn) {
      if (msg.columnsLength === 0) {
        ...
        result.stepCell = graph.insertVertex(parent, buildCellId(msg.id), tableHtml, 0, 0, STD.TABLE_WIDTH, STD.COL_HEIGHT * msg.columnsLength + 50, stepStyle)
      }
