...
export default {
  ...
  methods: {
    getData() {
      this.$http
        .post(
          `/domain/metricManagement/metricWarning/list?metricId=${this.metricId}`
        )
        .then(res => {
    ...
    removeAlert(row) {
      this.$DatablauCofirm(`确定要删除预警信息“${row.name}”吗？`, '')
        .then(() => {
          this.$http
            .post(
              `/domain/metricManagement/metricWarning/delete?metricWarningId=${row.id}`
            )
            .then(() => {
    ...
    getCurrentSchedule() {
      this.$http
        .post(
          `/domain/metricManagement/metricWarning/getSchedule?metricId=${this.metricId}`
        )
        .then(res => {
    ...
    handleScheduleChange(schedule) {
      if (schedule) {
        this.$http
          .post(
            `/domain/metricManagement/metricWarning/updateSchedule?metricId=${this.metricId}&schedule=${schedule}`
          )
          .then(() => {
