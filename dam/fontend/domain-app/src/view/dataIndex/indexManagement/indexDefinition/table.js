...
import udps from '@/view/newDataStandard/udps.vue'
...
export default {
  ...
  mounted() {
    ...
    this.$nextTick(() => {
      this.searchFormData.domainName = this.$route.query.keyword
    })
    // this.getData()
  },
  beforeDestroy() {
  ...
  methods: {
    ...
    exportSelection() {
      const url = `/domain/metricManagement/exportMetric`
    ...
    initTableData() {
      ...
      HTTP.getFolderListService(this.prepareSearchCondition())
        .then(res => {
    ...
    downloadTemplate() {
      const url = `/domain/metricManagement/downloadMetricTemplate?categoryId=${
    ...
    deleteStandard(selectionFromOtherFunction) {
      ...
      this.$DatablauCofirm(
        ...
        .then(() => {
          ...
          let postMethod = this.$postLongList
          postMethod({
            method: 'post',
            requestUrl: `${this.$domain_url}/domains/domain/deleteDomainByIds`,
      ...
      selection.forEach(item => {
        ...
        promises.push(
          new Promise((resolve, reject) => {
            this.$http
              .post(`${this.$domain_url}/domains/domain/updateDomain`, item)
              .then(res => {
                resolve()
              })
              .catch(e => {
