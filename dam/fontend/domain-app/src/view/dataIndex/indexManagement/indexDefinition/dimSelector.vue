...
export default {
  ...
  methods: {
    ...
    getDimensionPage() {
      this.loading = true
      const requestUrl = `/domain/dimension/page`
      const requestBody = {
        categoryId: null,
        currentPage: 1,
        keyword: '',
    ...
    // 右侧列表
    getValueTree(id) {
      let url = `/domain/dimension/tree/get?id=${id}`
      this.$http
        .post(url)
        .then(res => {
    ...
    changeSel(sel) {
      ...
      if (!this.multiple && sel.length > 1) {
        ...
        this.$refs.selectDimTable.toggleRowSelection(del_row, false)
      }
    },
    handleSelect() {
