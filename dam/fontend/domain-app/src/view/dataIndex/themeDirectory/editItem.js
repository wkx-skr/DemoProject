...
export default {
  ...
  methods: {
    ...
    // 获取详情
    getFormData() {
      let url = `/domain/querytheme/get?id=${this.id}`
      this.$http
        .post(url)
        .then(res => {
    ...
    onSubmit() {
      this.$refs.form.validate(valid => {
        if (!valid) {
          ...
          let url = '/domain/querytheme/create'
          ...
          if (this.iscreate) {
            ...
            url = '/domain/querytheme/create'
          } else {
            // 修改
            url = '/domain/querytheme/update'
          }
          this.$http
            .post(url, body)
            .then(res => {
    ...
    // 获取目录树
    getTreeData() {
      let url = `/domain/categories/tree?type=${this.type}`
      this.$http
        .post(url)
        .then(res => {
    ...
    // 获取目录列表
    getTreeList() {
      let url = `/domain/categories/get?type=${this.type}`
      this.$http
        .post(url)
        .then(res => {
