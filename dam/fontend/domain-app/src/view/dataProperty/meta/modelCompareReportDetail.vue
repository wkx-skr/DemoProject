...
export default {
  ...
  methods: {
    ...
    renderContent(h, { node, data, store }) {
      const iconClass = data.name.toLowerCase()
      let iconArr = null
      let contentStr = ''
      ...
      const map = {
        Entity: this.$t('meta.DS.treeSubOperation.table'),
        Relationship: this.$t('meta.DS.treeSubOperation.relationship'),
        Attribute: this.$t('meta.DS.treeSubOperation.column'),
    ...
    dataInit() {
      this.getShowData({ pageSize: 20, currentPage: 1 })
    },
    getShowData(para) {
