  ...
  <div>
    ...
    >
      <el-button
        type="text"
        size="mini"
        @click="goToStandard(currentDomain)"
        :disabled="!$auth.DATA_STANDARD_VIEW"
      >
...
export default {
  ...
  methods: {
    ...
    unbind() {
      this.$http
        .delete(
          `${this.$url}/service/domains/${this.currentDomain.domainId}/columns/${this.objectId}`
        )
        .then(res => {
          this.currentDomain = {}
    ...
    handleBind(domain, body) {
      this.$http
        .post(this.$url + '/service/domains/entities', body)
        .then(res => {
