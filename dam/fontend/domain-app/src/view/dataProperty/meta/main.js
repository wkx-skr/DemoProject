...
export default {
  ...
  methods: {
    ...
    handleModelClick(data, map, name = null) {
      ...
      if (data && typeof data === 'object') {
        if (data.type === 'SCHEMA') {
          ...
          this.currentModel = {
            id: this.modelId,
            name: data.id.split('_')[1] + ' / ' + data.name,
            schema: data.name,
          }
        } else {
    ...
    skip2Job() {
      ...
      window.open(
        baseUrl +
          `main/systemManage/jobManagement?jobType=2&dataSourceId=${this.sourceData.id}`
      )
    },
    loadTags() {
      var self = this
    ...
    handleItemClickFromDetail({ objectId, type }) {
      ...
      if (type === 'REPORT') {
        window.open(
          baseUrl + `main/reportFormManage?id=${objectId}&blank=true`,
          '_blank'
        )
      } else {
        window.open(
