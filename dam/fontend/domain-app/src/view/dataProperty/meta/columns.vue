  ...
  <div>
    ...
    >
      <datablau-input
        ...
        :placeholder="$t('meta.DS.tableDetail.column.placeholder')"
        :iconfont-state="true"
    ...
    >
      ...
      >
        ...
        <el-table-column
          :label="$t('meta.DS.tableDetail.column.dataVisitPermission')"
          :min-width="160"
          v-if="$versionFeature['metadata_SampleData'] && $authServerEnabled"
        >
          <template v-if="scope.row.access != undefined" slot-scope="scope">
            <span style="color: #66bf16" v-if="scope.row.access">
              <i
                class="iconfont icon-content-qx"
                style="vertical-align: middle"
              ></i>
              {{ $t('meta.DS.tableDetail.column.hasPermission') }}
            </span>
            <span v-else style="color: #ff4b53">
              <i
                class="iconfont icon-menu-aqwg"
                style="vertical-align: middle"
              ></i>
              {{ $t('meta.DS.tableDetail.column.noPermission') }}
            </span>
          </template>
        </el-table-column>
      </datablau-table>
      <!-- <div :style="style.more" v-if="data.length > 5">
      <el-button type="text" v-if="!showFullData" @click="handleExpand">{{$version.tableHeader.expandColumns.format(data.length-5)}}</el-button>
      <el-button type="text" v-else="showFullData" @click="handleCollapse">{{$version.tableHeader.collapse}}</el-button>
    </div> -->
      <div
        style="
          width: 100%;
...
export default {
  ...
  methods: {
    ...
    modifyColumns(data) {
      const requestUrl = this.$url + `/service/entities/${data.id}/summary`
      const requestBody = {
        id: data.objectId,
    ...
    getColumns() {
      this.$http
        ...
        .then(res => {
          ...
          // this.shortTableData.forEach(item => {
          //   this.getAccessible(item.objectId, item)
          // })
    ...
    getAccessible(columnId, item) {
      this.$http
        .get(this.$url + '/service/auth/check/' + columnId + '/80000005')
        .then(res => {
          this.$set(item, 'access', res.data)
        })
        .catch(e => {
          this.$showFailure(e)
        })
    ...
    callTagSelector() {
      // TODO
      /* this.$bus.$once('tagSelectedArr', arrList => {
        console.log('arrList', arrList)
        if (arrList) {
          this.currentTag = arrList
        }
      }) */
