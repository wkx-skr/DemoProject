      ...
      if (Array.isArray(buckets) && buckets.length) {
        buckets.forEach(suggest => {
          let basicObj = {
            ...
            id: suggest.id || suggest.assetsId,
            api: suggest.api,
...
export const typeStyleMap = (data, _this) => {
  ...
  if (data.typeId) {
    ...
    switch (`${data.typeId}`) {
      ...
      case `DATA_OBJECT`:
        return {
          dataType: data.isLogical ? 'logicalcolumn' : 'column',
          bg: 'rgba(75, 92, 196, 0.1)',
...
export const getSearchTypes = _this => {
  return [
    ...
    _this.$versionFeature.metadata_Report
      ? {
    ...
    _this.$versionFeature.metadata_File
      ? {
...
export const getVisitParams = item => {
  ...
  return {
    ...
    objectId,
    secName: item.secName || item.alias,
    typeId: item.typeId,
    icon: item.icon,
...
export const toBrowsing = (data, _this) => {
  ...
  if (query.typeId === 'REPORT' && _this.$auth.METADATA_REPORT_VIEW) {
    ...
    window.open(
      baseUrl +
        `main/reportFormManage?objectId=${data.itemId}&keyword=${_this.keywords}&blank=true&isAssets=true`
    )
  }
  if (query.typeId === 'DATA_SERVICE' && _this.$auth.API_DEVELOP_ADMIN) {
    ...
    window.open(
      baseUrl +
        `main/apiOverview?apiId=${data.objectId || ''}&blank=true&isAssets=true`
    )
  }
