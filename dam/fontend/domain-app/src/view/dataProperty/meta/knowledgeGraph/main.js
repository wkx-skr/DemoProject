  ...
  methods: {
    ...
    async goDetail(detail) {
      ...
      if (detail.typeId === '80010000') {
        ...
        window.open(baseUrl + `main/accessControl`)
      } else {
        console.log(detail)
      }
    },
      ...
      if (max && list.length > 0) {
        ...
        if (RightRelation.length > 0) {
          ...
          RightRelation.forEach(item => {
            ...
            if (LevelCountMap.get(currentLevel) < max && LevelDisplayMap.get(currentLevel).includes(item.startNode.businessId)) {
              if (!LevelDisplayMap.get(endLevel)) {
                LevelDisplayMap.set(endLevel, [])
              }
              LevelCountMap.set(currentLevel, LevelCountMap.get(currentLevel) + 1)
        ...
        if (LeftRelation.length > 0) {
          ...
          LeftRelation.forEach(item => {
            ...
            if (LevelCountMap.get(currentLevel) < max && LevelDisplayMap.get(currentLevel).includes(item.endNode.businessId)) {
                    ...
                    this.$confirm(`确定要删除自定义类型的关系吗？`, '提示', {
                      ...
                      .then(() => {
                        this.$http
                          .post(
                            this.$graph_url +
                            `/graph/removeCustomizedRelation`,
                            element.detail
                          )
                          .then(_ => {
