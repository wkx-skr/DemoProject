<template>
  <div>
    <div v-show="hasLogicalRelation">
      <!-- <div style="margin-bottom: 1em">逻辑关联</div> -->
      <div id="logical-relation">
        <canvas width="800" height="0"></canvas>
      </div>

      <!--{{objectId}}{{tableName}}-->
    </div>
    <div class="noresult" v-show="!hasLogicalRelation" style="height: 200px">
      <div class="noresult-img">
        <img src="static/kgimg/noresult.svg" alt="" />
        <p>
          {{ $t('meta.DS.tableDetail.relationMessage.noLogicalRelation') }}
        </p>
      </div>
    </div>
  </div>
  ...
  methods: {
    initEventListener() {
      ...
      $(this.$el).on('click', '.diagram-box', function () {
        const baseUrl = HTTP.getDdmLoginUrl()
        window.open(baseUrl + `#/main/list?id=${self.data.modelId}`)
      })
    },
    getData() {
      this.$http
        .get(this.$url + `/service/entities/tables/${this.objectId}/ddminfo`)
        .then(res => {
...
${name}</div></div>`
    ...
    drawEdge(centerOffset, eachWidth) {
      ...
      this.right.forEach((t, i) => {
        if (this.left.length === 0) {
          this.drawOneEdge(left, top, leftEnd, topEnd + 50 * i)
        } else {
          const [left, top, leftEnd, topEnd] = [
            eachWidth + 350,
            centerOffset + 12,
