    ...
    return {
      ...
      formContent: {
        standard: [
          {
            name: '',
            catalog: this.standardCode
              ? this.$t('domain.domain.codeProp')
              : this.$t('domain.domain.symbolProp'),
            dataType: 'String',
            candidates: null,
          },
        ],
    ...
    initCatalogOptions() {
      ...
      if (this.standardCode) {
        this.catalogOptions.push({
          label: this.$t('domain.domain.codeInfo'),
          value: this.$t('domain.domain.codeProp'),
        })
      } else {
        if (this.categoryTypeId === 1) {
          this.catalogOptions.push(
            ...[
              {
                label: this.$t('domain.domain.symbolInfo'),
                value: this.$t('domain.domain.symbolProp'),
              },
              {
                label: this.$t('domain.domain.defineInfo'),
                value: this.$t('domain.domain.defineProp'),
              },
              {
                label: this.$t('domain.domain.relaInfo'),
                value: this.$t('domain.domain.relaProp'),
              },
              {
                label: this.$t('domain.domain.expressInfo'),
                value: this.$t('domain.domain.expressProp'),
              },
              {
                label: this.$t('domain.domain.newManageInfo'),
                value: this.$t('domain.domain.newManageProp'),
              },
              {
                label: this.$t('domain.domain.additionInfo'),
                value: this.$t('domain.domain.additionProp'),
              },
            ]
          )
        } else {
          this.catalogOptions.push(
            ...[
              {
                label: this.$t('domain.domain.baseInfo'),
                value: this.$t('domain.domain.standardProp'),
              },
              {
                label: this.$t('domain.domain.manageInfo'),
                value: this.$t('domain.domain.manageProp'),
              },
              {
                label: this.$t('domain.domain.techInfo'),
                value: this.$t('domain.domain.techProp'),
              },
            ]
          )
          if (this.categoryTypeId !== 4) {
            this.catalogOptions.push({
              label: this.$t('domain.domain.businessInfo'),
              value: this.$t('domain.domain.businessProp'),
            })
          }
        }
      }
    ...
    titleTooltip() {
      if ([5, 6].includes(this.categoryTypeId)) {
        return this.$t('domain.domain.udpTooltip').replace(
          this.$t('domain.domain.domain'), // '数据标准',
          this.$t('domain.domain.domainIndex') // '数据指标'
        )
      } else {
