<template>
  <div id="dataStandard-quoto">
    ...
    <div class="top-box" v-if="categoryTypeId < 4" v-show="appName === 'DAM'">
      <div class="data-box">
        ...
        <datablau-button
          ...
          v-show="
            tableData && tableData.length > 0 && !Boolean($route.query.isAssets)
          "
          type="text"
          size="mini"
          @click="unbindAll"
      ...
      <div class="data-box" v-if="$versionFeature['domain_Verification']">
    ...
    <div class="table-container" v-if="appName === 'DAM' && categoryTypeId < 4">
      <datablau-table :data="tableData" :height="441">
        <el-table-column
          prop="columnName"
        ...
        >
          <template slot-scope="scope">
            <a
              @click="onMetadataClick(scope.row.columnObjectId)"
              href="javascript:;"
              class="blue"
            >
        ...
        <el-table-column
          ...
          show-overflow-tooltip
          min-width="100"
          :label="$t('domain.domain.columnAlias')"
        ></el-table-column>
        <el-table-column
        ...
        >
          <template slot-scope="scope">
            <a
              @click="onMetadataClick(scope.row.tableObjectId)"
              href="javascript:;"
              class="blue"
            >
              {{ scope.row.tableName }}
        ...
        <el-table-column
          ...
          show-overflow-tooltip
          min-width="100"
          :label="$t('domain.domain.tableAlias')"
        ></el-table-column>
        <el-table-column
        ...
        <el-table-column
          :label="$t('domain.domain.mappingResult')"
          v-if="$versionFeature['domain_Verification']"
          width="200"
        >
          <template slot-scope="scope">
    ...
    <div class="top-box" v-if="ddmEnabled">
      <div class="data-box">
        <p class="fl">{{ $t('domain.domain.ddmQuoteCount') }}</p>
        <div class="num">
          {{ ddmCnt }}
        </div>
      </div>
    ...
    <div class="table-container" v-if="ddmEnabled">
  ...
  data() {
    return {
      ...
      statusPromise: new Promise((resolve, reject) => {
        return this.$http
          .get(
            `${this.$meta_url}/service/domains/domainVerifyInfo?currentPage=1&pageSize=100000&keyword=&status=&theme=&modelCategoryId=&orderBy=updateTime&sort=desc&domainId=${this.domainId}`
          )
          .then(res => {
            resolve(res)
          })
      ...
      ddmEnabled: this.$ddmConnectable,
      // useDam: true,
    }
  },
  methods: {
    getPiedata2Percent(pieData) {
      ...
      let part = pieData[this.$t('domain.verification.partialMapping')] || 0
      let cannot = pieData[this.$t('domain.verification.canNotMap')] || 0
      if (!isNaN(full - 0)) {
        full = full - 0
      } else {
        full = 0
      ...
      const total = full + part + cannot
      if (full > 0 && total > 0) {
        result = ((full / total) * 100).toFixed(2) + '%'
      }
      return result
    },
    ...
    getDomainVerifyPie() {
      this.$http
        .get(
          `${this.$meta_url}/service/domains/domainVerifyPie/${this.domainId}`
        )
        .then(res => {
    ...
    getData() {
      ...
      this.tableData = null
      this.$http
        .post(`/metadata/domains/domain/getDomainUsagesPage`, {
          domainId: this.domainId,
    ...
    unbind(objectId, domainId) {
      this.$http
        .delete(
          `${this.$meta_url}/service/domains/${domainId}/columns/${objectId}`
        )
        .then(res => {
          this.getData()
        })
    ...
    unbindAll() {
      this.$DatablauCofirm(this.$version.domain.long.unbind, '', {
        ...
        .then(() => {
          this.$http
            .post(
              `/metadata/domains/unbindDomainById?domainId=${this.domainId}`
            )
            .then(_ => {
              this.getData()
            })
