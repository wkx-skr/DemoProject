<template>
  <div class="top-count-item" :class="{ 'en-page': $i18n.locale !== 'zh' }">
    <div class="domain-top-box" @click="skip2">
      <p class="domain-count-item-title">{{ showData.title }}</p>
      <div class="bottom-part">
        <div class="info-container">
          <div class="left-box">
            <div class="text-box">
              <p class="info-label left-label">{{ dataObj.label }}</p>
              <p class="info-value left-value">{{ dataObj.value }}</p>
            </div>
          </div>
          <div class="right-box">
            <div class="text-box">
              <p class="info-label left-label">{{ dataObj.label2 }}</p>
              <p class="info-value left-value">{{ dataObj.value2 }}</p>
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    skip2() {
      if (
        this.showData.name === 'indexTopCount' &&
        JSON.parse(localStorage.getItem('allServers')).includes('METRIC')
      ) {
        let pageUrl =
          this.BaseUtils.RouterUtils.getFullUrl('indexDefinitionNew')
        window.open(pageUrl)
      } else {
        let para = {
          name: 'domain',
