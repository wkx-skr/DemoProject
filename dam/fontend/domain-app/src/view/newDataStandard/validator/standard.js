...
const checkDomainCodeConflicts = (rule, value, callback) => {
  ...
  if (!value) {
    callback(
      new Error(
        _this.$t('domain.common.itemRequiredInput', {
    ...
    _this.$http
      .post(
        _this.$domain_url +
          `/domains/domain/checkDomainCodeConflicts?domainCode=${encodeURIComponent(
            value
          )}`
      )
      .then(res => {
...
const checkBusinessNameConflicts = (rule, value, callback) => {
  ...
  if (!value) {
    ...
    value &&
    __this &&
    __this.detailInitial &&
    __this.detailInitial.chineseName === value
  ) {
    callback()
  } else {
    _this.$plainRequest
      .post(
        _this.$domain_url +
          `/domains/domain/checkBusinessNameConflicts?categoryId=${currentCategoryId}`,
        value
      )
      .then(res => {
        if (res.data) {
          callback(
            new Error(_this.$t('domain.domain.domainBusinessNameExists'))
          )
        } else {
          callback()
        }
