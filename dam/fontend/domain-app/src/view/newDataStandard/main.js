...
import dataRuleManage from './dataRuleManage.vue'
...
export default {
  ...
  data() {
    ...
    var nameValidatePass1 = (rule, value, callback) => {
      if (this.changeObj.name === '') {
        ...
        this.nameArr.includes(this.changeObj.name) &&
        this.changeObj.name !== this.currentClassName
      ) {
        callback(new Error(this.$t('domain.domain.classificationNameExisted')))
      } else {
        callback()
  ...
  created() {
    ...
    if (this.useDam) {
      this.getDataTypeOptionsPromise = HTTP.getSelectionOptions({
        requestBody: {
          category: 'DOMAIN',
          names: ['数据类型'], // TODO i18n 作为数据存入后台，需要后续处理
        },
      })
    } else {
  ...
  mounted() {
    ...
    if (this.$route.query.domain || this.$route.query.domainId) {
      this.contentStatus = this.$route.query.contentStatus || 'scan'
      this.nowDomain = {}
    }
  ...
  methods: {
    toMove() {
      ...
      if (nodes.length === 1) {
        const selectedMoveFolder = nodes[0]
        if (selectedMoveFolder.foldId === 1) {
          this.$message.error('不能移动至根目录下')
          return
        }
        this.$refs.folderDetail.toMove(selectedMoveFolder, () => {
          this.moveDomainDialog = false
        })
      }
    },
    openDialogDomainRule(type, data, domainCode) {
      this.domainCodeBind = domainCode
      this.getBigClassListAndBusinessTypeList()
      this.dialogDomainRuleVisible = true
      this.dataRuleType = type
      if (data) {
        this.domainRuleForm = data
      } else {
        this.$refs.domainRuleRules.resetFields()
        this.domainRuleForm = {}
        this.domainRuleForm = {
          name: '',
          content: '',
    ...
    closeDialogDomainRule() {
      this.$refs.domainRuleRules.clearValidate()

      this.dialogDomainRuleVisible = false
    },
    createDomainRule() {
      this.$refs.domainRuleRules.validate(valid => {
        if (valid) {
          let obj = {
            name: this.domainRuleForm.name,
            content: this.domainRuleForm.content,
            techRuleType: this.domainRuleForm.techRuleType,
            dataRuleType: 'REGEXP',
            description: this.domainRuleForm.description,
          }
          if (this.domainRuleForm.id) {
            // obj.id = this.domainRuleForm.id
            this.$http
              .post(
                this.$domain_url + `/domains/dataRule/updateDataRule`,
                this.domainRuleForm
              )
              .then(res => {
                this.$datablauMessage.success('修改成功')
                this.dialogDomainRuleVisible = false
                if (this.dataRuleType === 'manage') {
                  this.$refs.dataruleManage.getTableData()
                } else {
                  this.$refs.scandetail.getListData()
                }
              })
              .catch(e => {
                this.$showFailure(e)
            ...
            this.$http
              .post(this.$domain_url + `/domains/dataRule/createDataRule`, obj)
              .then(res => {
    ...
    domainRuleBind(res) {
      this.$http
        .post(this.$domain_url + `/domains/dataRule/bind`, {
          domainCode: this.domainCodeBind,
          dataRuleIds: [res.data.id],
        })
        .then(res => {
          this.$refs.scandetail.getListData()
        })
    ...
    handleChangeClassOptions(val) {
      this.domainRuleForm.bigClassSelectOption = val[0]
      this.domainRuleForm.smallClassSelectOption = val[1] || null
    },
      ...
      return new Promise(resolve => {
        this.$http
          .post(`${this.$url}/select/option/get`, {
            category: 'TR',
            // names: ['SelectOption.businessType', 'SelectOption.categories'],
            names: ['规则大类'],
          })
          .then(res => {
            let tempList = []
            const classList = res.data.filter(
              e =>
                e.optionName ===
                (this.$i18n.locale === 'zh' ? '规则大类' : 'Rule Category')
            )
            const typeList = res.data.filter(
              e =>
                e.optionName ===
                (this.$i18n.locale === 'zh' ? '业务类型' : 'Business Type')
            )
            if (!classList.length) {
    ...
    closeRuleManage() {
      this.ruleManage = false
    },
    openManage() {
      this.ruleManage = true
    },
    elRadioSelect(state) {
      if (this.typeIds !== 2) {
    ...
    dataOptionsFunction(data, node) {
      ...
      if (!data.code) {
        ...
        options.push({
          ...
          label: this.$t('common.button.scan'),
          callback: () => {
    ...
    standardImportAction() {
      if (this.formFile.length === 0) {
        this.$datablauMessage.warning('请选择文件')
        return
      }
    ...
    addSubmit() {
      ...
      this.$refs.rulePrams.validate(val => {
        if (val) {
    ...
    changeSubmit() {
      this.$refs.rulePram.validate(val => {
        if (val) {
          if (this.buttonDisabled) return
          this.buttonDisabled = true
  ...
  computed: {
    ...
    standardUploadUrl() {
      ...
      let url = `${HTTP.domainUploadUrl()}?categoryId=${
        this.typeIds
      }&autoGenCode=${this.autoCode}&ignoreError=${this.exportType}`
