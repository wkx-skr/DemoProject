...
import treeCatalogue from './newTree.vue'
...
export default {
  ...
  data() {
    return {
      ...
      showList: true, // 是否从元数据或者资产页面跳转过来的
      exportType: false,
    }
  },
  ...
  computed: {
    ...
    useDam() {
      ...
      return !!this.$store.state.damConnectable
    },
    useDdm() {
      // return this.headerProduction && this.headerProduction === 'ddm'
      return !!this.$store.state.ddmConnectable
    },
    standardUploadUrl() {
  ...
  beforeMount() {
    ...
    if (this.blank) {
      ...
      HTTP.getCodeContent({
        ...
        this.currentCodeName = res.data.name
  ...
  methods: {
    ...
    treeNodeClick(data, node) {
      ...
      this.$refs.codeList.resetData(data.foldId)
    },
    ...
    handleEditNode(type, row) {
      if (!this.currentFolder || !this.currentFolder.foldId) {
        this.$message.error('请先选择所属目录')
        return
      }
    ...
    editSucess(data) {
      this.$refs.codeList.resetData(this.currentFolder.foldId)
    ...
    handleAddCode() {
      this.handleEditNode('add')
      // if (
      //   this.currentFolder &&
      //   this.currentFolder.foldId &&
      //   this.currentFolder.foldId !== 1
      // ) {
      // } else {
      //   this.$message.error('根目录不能添加参考数据')
      // }
    },
    getFindState() {
