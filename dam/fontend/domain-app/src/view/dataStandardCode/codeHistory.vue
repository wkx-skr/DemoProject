...
export default {
  ...
  methods: {
    ...
    async versionCompare() {
      ...
      HTTP.codeCompareVersionService({
        ...
        .then(res => {
          ...
          this.allCompareResultTableData = res.data.standardCompareResult
          const codeArr = []
          ...
          this.allCompareResultTableData2 = codeArr
          this.compareResultTableData2 = codeArr
    ...
    handleCurrentChange(val) {
      const allList = this.onlyShowChange2 ?
        this.allCompareResultTableData2.filter(item => item.type !== this.$t('domain.code.changeTypeEmpty')) :
        this.allCompareResultTableData2
      try {
        this.compareResultTableData2 = _.clone(allList).slice(
          (val - 1) * this.pageSize,
          (val - 1) * this.pageSize + this.pageSize
        )
      } catch (e) {
        this.compareResultTableData2 = _.clone(allList).slice(val * this.pageSize)
      }
    },
    exportFile(isAll) {
      const ids = []
  ...
  watch: {
    onlyShowChange1(value) {
      if (value) {
        this.compareResultTableData = _.clone(this.allCompareResultTableData.filter(e => e.srcValue !== e.tagValue))
      } else {
        this.compareResultTableData = _.clone(this.allCompareResultTableData)
      }
    },
    onlyShowChange2(value) {
      if (value) {
        const arr = _.clone(this.allCompareResultTableData2).filter(e => e.type !== this.$t('domain.code.changeTypeEmpty'))
        this.total = arr.length
        // this.compareResultTableData2 = arr
        this.handleCurrentChange(1)
      } else {
        // this.compareResultTableData2 = _.clone(this.allCompareResultTableData2)
        this.total = this.allCompareResultTableData2.length
        this.handleCurrentChange(1)
      }
    },
    onlyShowChange3(value) {
      if (value) {
        this.compareResultTableData3 = _.clone(this.allCompareResultTableData3).filter(e => e.srcValue !== e.tagValue)
      } else {
        this.compareResultTableData3 = _.clone(this.allCompareResultTableData3)
      }
    },
