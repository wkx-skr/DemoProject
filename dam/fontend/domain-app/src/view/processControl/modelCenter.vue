...
export default {
  ...
  methods: {
    ...
    async lazyLoad(tree, treeNode, resolve) {
      if (tree.reRender) {
        let res = await window.$wHttp.post(
          `${window.wBase}workflow/process/list?type=${tree.name}&parentId=${tree.parentId}`
        )
        ...
        let res = await window.$wHttp.post(
          `${window.wBase}workflow/process/list?type=${tree.name}&parentId=${tree.treeId}`
        )
        if (Array.isArray(res.data) && res.data.length > 0) {
          resolve(res.data)
        } else {
    ...
    async getTableData() {
      let res = await window.$wHttp.post(
        `${window.wBase}workflow/process/type/info?appName=${HTTP.$appName}&keyword=${this.keyword}`
      )
      if (Array.isArray(res.data) && res.data.length > 0) {
