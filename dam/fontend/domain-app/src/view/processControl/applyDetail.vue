    ...
    <datablau-tabs v-model="activeName">
      ...
      >
        <div>
          <div
            class="apply-reason"
            v-if="formData.reason || formData.applyReason"
          >
          ...
          <el-row :gutter="10">
            <el-col :span="12">
              ...
              >
                <el-form-item
                  style="width: 40%"
                  v-for="item in Object.keys(formData.oldCatalogData)"
                  label-width="100"
                  class="jjjjjj"
                  :class="{
                ...
                >
                  ...
                  >
                    <is-show-tooltip
                      ...
                      :style="{
                        width:
                          180 - catalogChangeLabelMap[item].length * 20 + 'px',
                      }"
                    ></is-show-tooltip>
                  </span>
                </el-form-item>
            ...
            <el-col :span="12">
              ...
              >
                <el-form-item
                  style="width: 40%"
                  v-for="item in Object.keys(formData.newCatalogData)"
                  label-width="100"
                  class="jjjjjj"
                  :class="{
      ...
      >
        ...
        <div
          class="apply-form"
          v-else-if="typeId && ['5', '6'].includes(typeId.value)"
        >
          <metrics-apply-form :raw-data="applyDetailData"></metrics-apply-form>
        </div>
        <div class="apply-form catalog-apply-form" v-else>
          <div
            class="apply-reason"
            v-if="formData.reason || formData.applyReason"
          >
          ...
          <datablau-detail :column="1" labelWidth="120px">
            ...
            >
              <span
                :style="{
                  display: 'inline-block',
                  width: 410 - item.name.length * 20 + 'px',
                }"
              >
                <is-show-tooltip
                  :content="item.value"
                  :style="{ width: 410 - item.name.length * 20 + 'px' }"
                ></is-show-tooltip>
              </span>
            </el-form-item>
            <!-- <el-form-item
...
import standardScan from '@/view/newDataStandard/standardScanBasic.vue'
import UserInformationService from '@service/UserInformationService'
import { AssetsTypeEnum } from '@/view/dataProperty/meta/utils/Enum'
import MetricsApplyForm from '@/view/dataIndex/indexManagement/indexDefinition/entrance/applyForm.vue'
...
export default {
  ...
  data() {
    return {
      ...
      techRule: {
        applyReason: this.$t('userPane.myTodo.noReason'),
        applyFile: '',
        applyFileName: this.$t('userPane.myTodo.noAttachment'),
      },
  ...
  mounted() {
    ...
    this.dataInit().then(() => {
      ...
      this.isNotNull =
        ...
        JSON.parse(this.formData.notNull.value)
          ? '是'
          : '否'
  ...
  methods: {
    ...
    goTechRuleDetail(row) {
      this.$http
        .post(this.$url + '/service/quality/rule/tech/' + row.ruleId + '/check')
        .then(res => {
    ...
    gobuRuleDetail(row) {
      this.$http
        .post(this.$url + '/service/quality/rule/bu/' + row.ruleId + '/check')
        .then(res => {
          if (row.hasAuthToView) {
            var pos = location.href.indexOf('#/')
            var baseUrl = location.href.slice(0, pos + 2)
      ...
      return new Promise(resolve => {
        ...
        HTTP.getApplyDetail(para)
          .then(res => {
            ...
            if (
              ...
              this.techRule.applyReason =
                data.formDtos.filter(item => item.name === '申请原因')[0]
                  ?.value || this.$t('userPane.myTodo.noReason')
                  ...
                  .then(res => {
                    this.$nextTick(() => {
                      this.techRule.applyFileName =
                        res.data[0]?.fileOrginalName || this.$t('userPane.myTodo.noAttachment')
                    })
                  })
              }
            }
            ...
            // const applyReasonDto = data.formDtos.find(
            //   item => item.code === 'applyReason'
            // )
            // if (applyReasonDto && task[0]) {
            //   task[0].param.opinion = applyReasonDto.value
            // }
            // this.taskDtos = task
            this.taskDtos = task
            // 通过登录名获取姓名
    ...
    convertDimCodes(list) {
      if (!list) return
      this.$http
        .post(`${this.$url}/service/me/dims/catalogs/codes`, JSON.parse(list))
        .then(res => {
          const arr = []
          res.data.forEach(e => {
    ...
    formatCatalogValue(key, value) {
      ...
      if (key === 'catalogExtensions') {
        return (
          value &&
          value.map(item => `${item.proName}: String(${item.proValue || ' '})`)
        )
      }
    ...
    formDtoFormatter(formDtos) {
      ...
      if (this.applyDetailData.processType === '变更目录申请') {
        ...
        if (oldCatalogData.catalogExtensions) {
          ...
          catalogExtensions.forEach(item => {
            ...
            this.$set(this.catalogChangeLabelMap, item.proName, item.proName)
          })
        }
      ...
      if (
        ...
        if (formData.catalogExtensions && formData.catalogExtensions.value) {
          const catalogExtensions = JSON.parse(formData.catalogExtensions.value)
          catalogExtensions.forEach(item => {
            formData[item.proName] = {
              name: item.proName,
              value: item.proValue,
            }
            this.catalogChangeLabelMap[item.proName] = item.proName
          })
        }
        delete formData.catalogExtensions
      }
...
.apply-detail-page {
  ...
  .details-box {
    .detail {
      /*width: 450px !important;*/
      .value {
        /*此处影响到指标详情了,如果需要以下5行,请使用更精确的css选择器*/
        /*height: 28px !important;*/
