...
export default {
  ...
  methods: {
          ...
          .then(res => {
            ...
            result2.forEach(item => {
              ...
              if (item.processType.includes('数据标准')) {
                if (
                  item.formPropertyDtos.length &&
                  item.formPropertyDtos[0].value
                ) {
                  if (`${item.formPropertyDtos[0].value}` === '1') {
                    item.businessType = 'shujubiaozhun'
                    item.processTypeName = this.applyTypesMap['数据标准'].label
                  } else if (`${item.formPropertyDtos[0].value}` === '2') {
                    item.businessType = 'zhibiaobiaozhun'
                    item.processTypeName = this.applyTypesMap['指标标准'].label
                  } else if (`${item.formPropertyDtos[0].value}` > '4') {
    ...
    setGetAllData(para) {
      this.getAllData = this.getAllData = HTTP.getMyDone({
