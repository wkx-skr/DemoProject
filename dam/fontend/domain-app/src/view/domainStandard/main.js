...
import updatedDomain from '../newDataStandard/updatedDomain.vue'
...
export default {
  ...
  created() {
    this.hasEditAuth = !!this.$auth.DATA_STANDARD_FIELD_MANAGE
    this.getAllOrgs()
    if (this.useDam) {
  ...
  methods: {
    goSelect() {
      this.expandedKeys2.push(this.treeData2[0].domainId)
      this.handleItemClicked2(this.treeData2[0])
      this.$nextTick(() => {
        this.$refs.mainTree2.setCurrentKey(this.treeData2[0].domainId)
      })
    },
    goTree() {
      this.activeType = 'tree'
    },
    addInheriDomain(data) {
      this.nowDomainId = data.domainId
      this.$http
        .post(`${this.$domain_url}/domains/domain/getDomainById`, {
          domainId: data.domainId,
        })
        .then(res => {
          this.nowDomain2 = res.data
          // this.nowDomain2.parentDomainId = data.parentDomainId
    ...
    handleClickActiveType(type) {
      if (this.activeType === 'field') {
        this.getDomainInheritTree()
      } else {
        this.$refs.mainTree2.setCheckedKeys([])
        this.innerLoadStandard()
      }
    },
        ...
        HTTP.getDomainInheritTree()
          .then(res => {
            ...
            if (res.data.children) {
              this.treeData2 = res.data.children
            } else {
              this.treeData2 = []
            }
            // this.expandedKeys2 = [res.data.domainId]
            if (this.$route.query.field) {
              this.expandedKeys2.push(this.$route.query.field)

              this.$nextTick(() => {
                const node =
                  this.$refs.mainTree2 &&
                  this.$refs.mainTree2.getNode(this.$route.query.field).data
                this.handleItemClicked2(node)
                this.$refs.mainTree2.setCurrentKey(this.$route.query.field)
              })
            } else {
              this.$nextTick(() => {
                let defaultNode = ((res.data || {}).children || [])[0]
                if (defaultNode) {
                  this.expandedKeys2.push(defaultNode.domainId)
                  this.handleItemClicked2(defaultNode)
                  this.$nextTick(() => {
                    this.$refs.mainTree2.setCurrentKey(defaultNode.domainId)
                  })
                }
              })
            }
            ...
            //     this.$nextTick(() => {
            //       this.$refs.mainTree2.setCurrentKey(this.nowFolder.foldId)
            //     })
            //   } else {
            //     this.$nextTick(() => {
            //       this.$refs.mainTree2.setCurrentKey(res.data.foldId)
            //     })
            //     this.nowFolder = {
            //       data: res.data,
            //     }
    ...
    dataOptionsFunction(data, node) {
      const options = []
      if (!data.code) {
        if (this.$auth.DATA_STANDARD_FIELD_CATALOG) {
          options.push({
    ...
    dataOptionsFunction2(data) {
      ...
      if (data.domainId !== '_root') {
        options.push({
          icon: 'iconfont icon-see',
          label: this.$t('domain.domain.derive'),
          callback: () => {
            this.commandHandle('addField', data)
          },
          args: 'folder',
        })
      }
    ...
    standardImportAction() {
      ...
      // this.$refs.standardImport.$el.click()
    },
    goToDomain(domainObj, type) {
      setTimeout(() => {
        ...
        if (type) {
          this.$nextTick(() => {
            this.$refs.mainTree2.setCurrentKey(domainObj.domainId)
          })
        }
      })
    },
    ...
    showBegain() {
      ...
      this.$bus.$emit('showUploadProgress', {
        name: this.$t('domain.domain.domain', {
          typeName: this.labelText.name,
        }),
        time: 10,
      })
    },
  ...
  computed: {
    ...
    labelText() {
      ...
      } else if (this.typeIds === 4) {
        obj = {
          typeName: this.$t('domain.domainStandard.domainStandard'),
          ...
          name: this.$t('domain.domainStandard.domainStandard'),
      ...
      if (this.activeType === 'field') {
        setTimeout(() => {
          this.keywordSetTimeout = setTimeout(() => {
            this.$refs.mainTree2.filter(value)
          }, 800)
        })
      } else {
        setTimeout(() => {
