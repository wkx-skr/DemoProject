...
export default {
  ...
  methods: {
    ...
    initData() {
      ...
      if (this.data) {
        this.innerLoadStandardDetails(
          this.fromField ? this.data.domainId : null
        )
      }
    },
    getNode() {
    ...
    getDomainVote(id) {
      ...
      HTTP.getDomainVote(id, 80010066)
        .then(res => {
          this.vote = res.data
        })
        .catch(err => {
    ...
    async innerLoadStandardDetails(domainId) {
      ...
      if (this.details.referenceCode !== '') {
        HTTP.getDomainItemDetail(this.data.domainId).then(res => {
          this.details.referenceCodeState = res.data.referenceCodeState
        })
      }
      this.getUdpCurrent()
      this.$nextTick(() => {
