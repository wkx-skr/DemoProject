<template>
  <div>
    <datablau-dialog title="日志" size="xl" :visible.sync="logDialogVisible" custom-class="log-dialog">
      <div class="item-container">
        <div v-for="i in logContent" class="item">
          <span v-if="i.startsWith('\t')" style="margin-left: 4em">{{i}}</span>
          <span v-else>{{ i }}</span>
        </div>
      </div>
      <span slot="footer">
    ...
    >
      ...
      <el-table-column label="操作" width="100" align="center" header-align="center">
        <template slot-scope="scope">
          <datablau-button @click="showLog(scope.row)" type="text">日志</datablau-button>
        </template>
      </el-table-column>
    </datablau-table>
  </div>
...
export default {
  ...
  methods: {
    scanJobResult() {
      ...
      this.$http.post(`/job/main/query/jobResults/byCriteria`,{
        ...
        let isRunning  = false
        if (['INIT','CREATED', 'PREPARED', 'RUNNING'].includes(this.jobHistory[0].status)) {
          isRunning = true
        } else {
          isRunning = false
        }
    ...
    showLog(row) {
      this.$http.post(`/job/main/findLogByInstanceId`, {
