...
import '@/../../base-components/http/utils/public_path.js'
...
// base--P 简版； advanced--E 高级版； demo版--A
This.$ddmVersionLevel = 'P'
// 微服务相关
...
This.$bus.$on('changePasswordStrong', strong => {
  This.$strongPassword = strong
})
...
This.$getVersionLevel = () => {
  This.$http
    ...
    .then(res => {
      ...
      console.log($versionFeature)
      /* window.vueThis.$showFailure = () => {}
        if (this.$customerId !== res.data.customer) {
         this.$router.push({
           name: 'damEmptyPage',
           query: { judgeBy: 'version' },
         })
         return
       } */
    })
...
Vue.prototype.$getCatAndTags = (resolve, reject) => {
  ...
  This.$http
    ...
    .then(res => {
      ...
      This.$http
        .get(`${This.$url}/service/tags/auth`)
        .then(res => {
          res.data.sort((a, b) => a.order - b.order)
...
Vue.prototype.$timeFormatter = function () {
  if (typeof arguments[0] === 'number') {
    ...
    // moment(arguments[0]).format('YYYY-MM-DD HH:mm:ss')
    return arguments[0]
  }
  if (arguments[0][arguments[1].property]) {
...
Vue.prototype.$downloadFile = (url, para) => {
  ...
  if (
    ...
    /\/download$/.test(url) &&
    !url.includes('mask/rule/download') &&
...
const pwEncrypt = pwd => {
  ...
  crypt.setPublicKey(public_string)
...
This.$skip2Domain = (domainId, isAssets = '') => {
  ...
  HTTP.getDomainDetailByIdService({ domainId })
    .then(res => {
      ...
      if (data.categoryId === 5) {
        ...
        // 基础标准
        if (HTTP.$appName === 'DAM') {
          This.$skip2({ name: 'dataStandard', query: { domainId, isAssets } })
        } else {
          This.$skip2Ddm({
            name: 'domain',
            query: { domainId: domainId, isAssets },
          })
        }
      } else if (data.categoryId === 4) {
        // 域标准
        if (HTTP.$appName === 'DAM') {
          This.$skip2({ name: 'domainStandard', query: { domainId, isAssets } })
        } else {
          This.$skip2Ddm({
            name: 'domainStandard',
            query: { domainId, isAssets },
          })
        }
      } else {
        // 领域标准 判断权限
            ...
            if (res.data !== 'NONE') {
              if (HTTP.$appName === 'DAM') {
                This.$skip2({
                  ...
                  query: { domainId, categoryId: data.categoryId, isAssets },
                })
              } else {
                This.$skip2Ddm({
                  name: 'dataStandardField',
