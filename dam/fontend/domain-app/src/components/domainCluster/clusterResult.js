...
export default {
  ...
  mounted() {
    ...
    // this.$http.get(this.$url + '/service/modelCategories/')
  ...
  methods: {
    resizeTable() {
      this.$nextTick(() => {
        this.tableHeight = $('.cluster-result-tab .table-row')[0].offsetHeight
      })
    },
    getCategory() {
      // 系统
    ...
    dataInit(callback) {
      ...
      this.$http
        .get(
          this.$meta_url +
            `/service/domains/cluster_result?currentPage=${
              this.currentPage - 1
    ...
    handleDeleteItem() {
      this.$DatablauCofirm(
        this.$t('domain.domainCluster.confirmDeleteResultItem'),
          ...
          if (arr && Array.isArray(arr) && arr.length > 0) {
            ...
            arr.forEach(element => {
              ...
              this.$http
                .post(
                  this.$meta_url +
                    `/service/domains/cluster_result/delete?clusterId=${data.clusterId}`
                )
                .then(res => {
                  // arr.shift()
                  // removeArr0()
                  flagNum++
                  if (flagNum === arr.length) {
                    if (
                      arr.length ===
