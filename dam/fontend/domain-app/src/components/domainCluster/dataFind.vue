<template>
  <div>
    ...
    <div class="citic-card-tabs fit-old-code datablau-new-tabs data-find">
      <div v-if="showHome">
        <div class="find">
          <div class="left">
            ...
            >
              <img src="../inteligentFind/ring.png" alt="" />
            </div>
            ...
            <div class="right-box">
              <p class="result-head">
                <span v-if="showStopJobBtn">
                  {{
                    $t('domain.dataFind.scanProgress', {percent: jobSta.percent}
                    )
                  }}
                </span>
                <span v-else-if="jobNeverRun">{{ $t('domain.dataFind.taskNeverRunning') }}</span>
              ...
              <p class="finish-count">
                ...
                <span>
                  <span v-if="showStopJobBtn">{{ $t('domain.dataFind.scanningProgressCount') }}：</span>
                  <span v-else>{{ $t('domain.dataFind.totalNumberOfReferrals') }}：</span>
                  <datablau-button
                    type="text"
              ...
              <p class="set-line">
                <span class="grey-word">{{ $t('domain.dataFind.scanSettings') }}：</span>
                <datablau-button
                  type="text"
        ...
        <el-tab-pane
          ...
          :label="
            item === 'showJobDetail'
              ...
              : item === 'showDadaSource'
              ? $t('domain.dataFind.dataRange')
              : item === 'reReCommendation'
              ? $t('domain.dataFind.reRecommend')
              : $t('domain.domainCluster.recommendResult')
          "
          :name="item"
        >
          <!--TODO i18n-->
          ...
          <!-- 结果 -->
          <cluster-result
            v-show="currentShowPageName === 'jobResult'"
            v-else-if="item === 'jobResult' && currentShowPageName === 'jobResult'"
            ref="jobResult"
          ...
          <!-- 重新推荐 -->
          <redomainFind
            v-else-if="item === 'reReCommendation' && currentShowPageName === 'reReCommendation'"
            ref="reReCommendation"
            @readdItemTab="readdItemTab"
            :getColumnsData="getColumnsData"
            @reRefreshResultOk="reRefreshResultOk"
            :getPrecision="getPrecision"
          ></redomainFind>
        </el-tab-pane>
      </datablau-tabs>
      <!-- 推荐结果查看详情 -->
      <div v-if="currentShowPageName === 'clusterItem' && currentTab === 'jobResult'">
        <cluster-item
          v-if="currentShowPageName === 'clusterItem' && showHome === false && currentTab === 'jobResult'"
      ...
      <!-- 重新推荐查看详情 -->
      <div v-if="currentShowPageName === 'reClusterItem' && currentTab === 'reReCommendation'">
        <reClusterItem
          v-if="currentShowPageName === 'reClusterItem' && showHome === false && currentTab === 'reReCommendation'"
          :clusterData="recurrentShowPageData.clusterData"
          :ref="recurrentShowPageData.name"
          :key="'clus' + recurrentShowPageData.clusterId"
          :id="recurrentShowPageData.clusterId"
          :propDomainId="recurrentShowPageData.propDomainId"
          :allDataLoading="allDataLoading"
          :columnsSort="columnsSort"
          :getColumnsData="getColumnsData"
          :getPrecision="getPrecision"
      ...
      <div v-if="currentShowPageName === 'createDomain' && currentTab === 'jobResult'">
        <create-domain
          v-if="currentShowPageName === 'createDomain' && currentTab === 'jobResult'"
          :ref="currentShowPageData.name"
...
.data-find {
  ...
  .left {
    ...
    .background-box {
      ...
      height: 240px;
      border-radius: 50%;
      background: #fff url('../inteligentFind/bolang-line.png');
  ...
  .right {
    ...
    .right-box {
      ...
      .finish-count {
        ...
        .checking-icon {
          background: url('../inteligentFind/checking-icon.png') center/cover;
...
<style lang="scss">
@import '../inteligentFind/animation.scss';
</style>
