  ...
  methods: {
    ...
    addItemTab(clusterData) {
      ...
      this.currentShowPageData = {
        ...
        label: `"${column.alias || column.name}" ${this.$t(
          'domain.domainCluster.clusterResult'
        )}`,
    ...
    clearRemoveArr(callback) {
      ...
      if (this.removeColumnsArr.length > 0) {
        const removeCol0 = () => {
          ...
          this.$http
            .get(this.$meta_url + '/service/domains/cluster_result')
            .then(res => {
              let resultArr = []
    ...
    postNewResult(data) {
      return this.$http.post(
        this.$meta_url + '/service/domains/cluster_result',
        data
      )
    },
    handleRunJob() {
    ...
    getJobStatus() {
      if (!this.cluJob || !this.cluJob.jobId) {
        setTimeout(() => {
        ...
        criteria: [
          {
            ...
            fieldName: 'jobId',
            compareValue: this.cluJob.jobId,
            notEqual: false,
          },
        ],
        page: {
      ...
      this.$http
        // .get(this.$url + '/service/datablau_jobs/' + this.cluJob.jobId)
