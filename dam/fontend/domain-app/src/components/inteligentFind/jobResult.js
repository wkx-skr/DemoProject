  ...
  methods: {
    tableDatInit() {
      ...
      if (Object.keys(obj).length > 0) {
        const url = this.$meta_url + '/service/entities/domains/candidates'
        let str = ''
        this.$nextTick(() => {
          str += '?'
    ...
    gettableDataShow(para, callback) {
      this.showLoading()
      let url = this.$meta_url + '/service/entities/domains/candidates'
      let str = ''
      if (para) {
        str += '?'
        if (para.pageSize) {
    ...
    handleBindDom(scope) {
      ...
      this.$http
        .post(this.$meta_url + '/service/domains/entities', json)
        .then(res => {
          this.$message.success({
            message: this.$t('domain.common.modifySuccessfully'),
          })
          if (this.total - this.pageSize * (this.currentPage - 1) === 1) {
    ...
    confirmPost() {
      this.showLoading()
      const arr = _.cloneDeep(this.selection)
      const arrLength = arr.length
      const showSucces = () => {
        this.$message.success({
          message: this.$t('domain.common.modifySuccessfully'),
        })
        if (arrLength === this.total - this.pageSize * (this.currentPage - 1)) {
          if (this.currentPage > 1) {
            this.currentPage--
          }
        }
    ...
    getCloumnDetail(objectId) {
      this.$http
        .get(this.$meta_url + '/service/entities/' + objectId + '/summary')
        .then(res => {})
        .catch(e => this.failureCallback(e))
    },
    ...
    getLogicalInfo(logicalInfoArr) {
      this.$http
        ...
        .then(res => {
          this.tableDataShow.forEach(element => {
