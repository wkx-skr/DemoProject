  ...
  methods: {
    ...
    callOptionsMenu(row, evt, idx) {
      ...
      if (
        row &&
        row.connectionInfo &&
        row.connectionInfo.parameterMap &&
        row.connectionInfo.parameterMap.biType
      ) {
    ...
    goToJob(row, jobType) {
      ...
      if (
        row.connectionInfo.parameterMap &&
        row.connectionInfo.parameterMap.biType &&
        row.connectionInfo.parameterMap.jobId
      ) {
        this.$router.push({
          name: 'jobManagement',
          query: {
            jobId: row.connectionInfo.parameterMap.jobId,
            from: 'dataSource',
            update: update,
          },
        })
    ...
    ediDS(dsform, para) {
      ...
      if (formatPara.isEditing) {
        ...
        if (ds.dbtype === 'GBASE') {
          json.parameterMap.GBaseMppVcName = ds.GBaseMppVcName
        }
        ...
        if (ds.dbtype === 'GBASE') {
          request.parameterMap.GBaseMppVcName = ds.GBaseMppVcName
        }
