...
export default {
  data() {
    ...
    return {
      ...
      keyword: '',
      keywordFilterProps: ['sourceName', 'type'],
      emptyPara: {
        keyword: '',
        currentPage: 1,
        sortData: {
  ...
  mounted() {
    ...
    this.$bus.$on('changeDs', ({ dsform, para }) => {
      console.log('changeDs134', dsform, para, this.insertFlag)
  ...
  methods: {
    ...
    handleTest(row, index) {
      if (row.type === 'AGENT') {
        ...
        this.$http
          .post(this.$baseurl + `/datasources/testConnection?dsId=${row.id}`)
          .then(res => {
            row.isError = false
            row.isSuccess = true
            this.testBtnKey++
    ...
    handleEdit(rowInfo) {
      ...
      console.log('123row', row, this.displayData)
      var self = this
      let isJDBC = this.$allPlugins.some(all => {
        return all.dbType === row.type
      })
      this.isEditing = true
      if (isJDBC) {
      ...
      environmentParams.forEach(item => {
        if (row.parameterMap[item]) {
          ds[item] = row.parameterMap[item]
        } else {
          ds[item] = ''
        }
      }) */
        ds.connectType = row.connectType
        ds.hostname = row.parameterMap.HostServer
        ds.dbport = row.parameterMap.PortNumber
        ds.dbname = row.parameterMap.DatabaseName
        /* ds.delimiter = row.parameterMap.Delimiter
      const is = row.parameterMap.IsFirstRowColumnName
      if (is === 'true') {
      ...
      ds.modelCategoryId = row.categoryId */
        /* if (
        row.parameterMap.CommentToLogicalName &&
        row.parameterMap.CommentToLogicalName == 'true'
      ) {
        ds.CommentToLogicalName = true
      } else {
        ds.CommentToLogicalName = false
      } */
        /* ds.versioning = row.versioning
      if (ds.connectType == 'WebLogic') {
        ds.JNDIName = row.parameterMap.JNDIName
        ds.dbname = [row.parameterMap.DatabaseName]
      } */
        ...
        // zl add
        if (this.isEditing) {
          let AuthenticationType = row.parameterMap.AuthenticationType - 0
          ds.AuthenticationType = AuthenticationType
          if (AuthenticationType === 1) {
            ds.KeyTabPath = row.parameterMap.KeyTabPath
            ds.Krb5Path = row.parameterMap.Krb5Path
          }
        }
        // end
        this.dsform = ds
    ...
    delmodel() {
      ...
      this.jobStaData = []
      this.$http
        .post(this.$baseurl + `/datasources/deleteDatasources`, datasourceIds)
        .then(res => {
    ...
    ediDS(dsform, para) {
      ...
      if (this.isEditing) {
        ...
        let url = this.isEditing
          ? this.$baseurl + '/datasources/updateDatasource'
          : this.$baseurl + '/datasources/createDatasource'
        this.$http
          .post(url, json)
          .then(() => {
            this.showSuccess(
    ...
    innerLoadDataSources() {
      ...
      this.$http
        .post(this.$baseurl + '/datasources/findDatasources', param)
        .then(res => {
          if (!res.data || !Array.isArray(res.data)) {
            return
          }
