<template>
  <div>
    <datablau-dialog
      :title="$t('component.branchSelect.personnel')"
      ...
      <div class="content">
        <div>
          <datablau-input
            v-model="nameInput"
            :iconfont-state="true"
            :placeholder="$t('assets.permissionSettings.searchPeople')"
            clearable
            class="nameInput"
            @input="keyWord"
            @clear="inputClear"
          ...
          <div v-show="!flag">
            <div class="menuTop">
              <span @click="backClick">
                ...
                {{ $t('assets.permissionSettings.return') }}
              </span>
              {{ selectListName }}
            </div>
            <div v-show="!search">
              <div v-show="!onlyPeople">
                <datablau-checkbox
                  :label="$t('assets.permissionSettings.selectAll')"
                  class="checkbox"
                  :checkboxType="'single'"
                  v-model="checkbox"
                  ...
                  v-show="
                    selectListName ===
                    $t('assets.permissionSettings.institutional')
                  "
                ></datablau-checkbox>
                <datablau-checkbox
                  :label="$t('assets.permissionSettings.selectAll')"
                  ...
                  v-show="
                    selectListName === $t('assets.permissionSettings.role')
                  "
                ></datablau-checkbox>
                <datablau-checkbox
                  :label="$t('assets.permissionSettings.selectAll')"
                  ...
                  v-model="checkbox2"
                  v-show="
                    selectListName === $t('assets.permissionSettings.user')
                  "
                ></datablau-checkbox>
              </div>
        ...
        <div class="tagiconBox">
          <p>{{ $t('assets.permissionSettings.chosen') }}</p>
...
<script>
import HTTP from '@/view/dataProperty/meta/utils/api.js'
export default {
  /**
  ...
  data() {
    return {
      ...
      flag: true,
      menu: [],
      selectListName: this.$t('assets.permissionSettings.results'),
      defaultProps: {
        children: 'children',
        label: 'fullName',
        isLeaf: 'isLeaf',
      ...
      list: [
        {
          name: this.$t('assets.permissionSettings.institutional'),
          icon: 'department',
          id: 1,
        },
        {
          name: this.$t('assets.permissionSettings.role'),
          icon: 'role',
          id: 2,
        },
        {
          name: this.$t('assets.permissionSettings.user'),
          icon: 'group',
          id: 3,
        },
      ],
  ...
  methods: {
    ...
    keyWord(val) {
      this.flag = false
      this.search = true
      this.selectListName = this.$t('assets.permissionSettings.results')
      this.loading = true
      this.page = 1
    ...
    selectClick(item) {
      ...
      this.loading = true
      this.setChecked([])
      if (item.name === this.$t('assets.permissionSettings.institutional')) {
        this.mechanism()
      } else if (item.name === this.$t('assets.permissionSettings.role')) {
        this.getRole()
      } else if (item.name === this.$t('assets.permissionSettings.user')) {
        this.getGroup()
      }
      this.flag = false
    },
    ...
    handleNodeClick(data) {
      ...
      if (
        this.selectListName ===
        this.$t('assets.permissionSettings.institutional')
      ) {
        this.mechanismAcquisition(data)
      } else if (
        this.selectListName === this.$t('assets.permissionSettings.role')
      ) {
        this.roleAcquisition(data)
      } else if (
        this.selectListName === this.$t('assets.permissionSettings.user')
      ) {
