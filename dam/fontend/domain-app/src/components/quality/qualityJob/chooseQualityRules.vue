  ...
  <div class="chooseQualityRules">
    ...
    >
      ...
      >
        ...
        <!--						<span>{{bigRuleFormatter(displayRules[scope.$index].bigClassSelectOption)}}</span>-->
        <!--					</template>-->
      </el-table-column>
      <el-table-column
        prop="smallClassSelectOption"
      ...
      >
        <!--					<template slot-scope="scope">-->
        <!--						<span>{{smallRuleFormatter(displayRules[scope.$index].smallClassSelectOption)}}</span>-->
        <!--					</template>-->
      </el-table-column>
      <el-table-column
        prop="bizTypeSelectOption"
    ...
    <div class="dialog-bottom">
      <el-checkbox v-model="notUsedByJob" @change="handleUsedByJobChange" style="float:left;margin-top: 13px;margin-right: 20px;">只显示未被任务引用的规则</el-checkbox>
      <datablau-pagination
...
export default {
  ...
  methods: {
    getBigClassListAndBusinessTypeList() {
      ...
      // this.$http.post(`${this.$url}/service/select/option/query`, {category: 'TR', names: ['业务类型', '规则大类']}).then(res => {
      this.$http
        .post(`${this.$url}/service/quality/rules/tech/page`, obj)
        .then(res => {
    ...
    getSmallClassList() {
      ...
      this.$http
        .get(`${this.$url}/service/select/option/query?parentId=${pId}`)
        .then(res => {
          res.data.forEach(e => {
            const obj = {
              lable: e.optionValue,
    ...
    loadAllRules() {
      ...
      if (this.allRules) {
        ...
        let requestUrl = this.$url + '/service/quality/rules/tech/report'
        if (this.categoryId) {
