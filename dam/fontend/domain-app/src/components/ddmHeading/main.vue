...
export default {
  ...
  created() {
    ...
    //   })
    // }
    this.getDdmUserInfo = HTTP.getDdmUserInfo()
  ...
  mounted() {
    ...
    this.$bus.$on('getTaskJobResult', (res, type) => {
      this.iframePostMessage({ taskId: res, type })
    })
    ...
    document.addEventListener('click',e => { // 关闭任务详情弹窗
      this.iframePostMessage({ hideTaskInfoPopover: true })
    })
  },
  beforeDestroy() {
      ...
      if (HTTP.$ddmWebOrigin.includes(e.origin)) {
        if (e.data) {
          try {
            ...
            if (data.name === 'dataCatalogDdm') {
              this.$bus.$emit('dataCatalogDdmDigDDD')
            }
