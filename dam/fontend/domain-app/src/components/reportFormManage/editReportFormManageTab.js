...
export default {
  ...
  mounted() {
    this.action = `${this.$url}/service/files/upload` // 上传图片地址
    self = this
    this.dataInit()
    this.getDbData()
  ...
  methods: {
    ...
    getUdp() {
      this.$http
        .get(this.$url + '/service/entities/udps/82800002')
        .then(res => {
          const udpValuesMap = {}
    ...
    addReportFormManage() {
      ...
      const successHandle = () => {
        const addpara = {
          method: 'put',
          getUrl: fileIds => {
            return `${this.$url}/service/files/commit?fileIds=${fileIds}`
          },
          list: addImgList,
          failureCallback: e => {
            this.$showFailure(e)
        ...
        const delResPara = {
          method: 'delete',
          getUrl: fileId => {
            return `${this.$url}/service/files/${fileId}`
          },
          list: removeImgList,
          failureCallback: e => {},
          skipError: true,
      ...
      this.reportDetailPro
        .then(res => {
          ...
          if (this.reportFormManage.type === 'Analysis' && false) {
            ...
            console.log('res.data.resultArea', res.data.resultArea)
          }
          this.reportFormManage.description =
            this.reportFormManage.description || ''
    ...
    showEditPathDialog() {
      console.log(this.customPathArr)
      const path = this.customPathArr || ['']
      console.log(path)
      this.pathFroCustomEdit = [...path]
      console.log(this.pathFroCustomEdit)
