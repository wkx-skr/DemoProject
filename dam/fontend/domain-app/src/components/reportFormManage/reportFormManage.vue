...
// import importJobManage from './importJobManage.vue'
...
export default {
  ...
  data() {
    const reportFormManageUrl = this.$url + '/service/dataReport/'
    return {
      initShow: true,
      hasAccess: false,
  ...
  methods: {
    ...
    showReportDetail(para) {
      ...
      if (this.$route.query.isAssets) {
        this.editReportFormManage(0, { id: para.id })
      } else {
        this.editReportFormManage(0, { addName: para.name })
      }
    },
    createdJob(res) {
    ...
    editReportFormManage(index, para) {
      ...
      this.$nextTick(() => {
        if (this.$route.query.isAssets) {
          this.$refs.reportFormManageTab.getTableDataAll({ curId })
        } else {
          this.$refs.reportFormManageTab.getTableDataAll({ addName: addName })
        }
      })
    },
    removeTab(name) {
    ...
    resetGetTreeData() {
      this.getTreeData = this.$http.get(`${this.$url}/service/dataReport/tree`)
      this.initTreeData()
    },
    initTreeData() {
      this.getTreeData
