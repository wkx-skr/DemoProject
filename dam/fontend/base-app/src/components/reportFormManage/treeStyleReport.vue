...
export default {
  ...
  methods: {
    ...
    export(data) {
      ...
      const url = `${this.$meta_url}/dataReport/file/report`
      const params = {
        categorys: newArr,
      }
    ...
    getQueryidArr(objectId) {
      this.$http
        .get(this.$meta_url + '/service/dataReport/' + objectId)
        .then(res => {
    ...
    tableLayout() {
      this.$nextTick(() => {
        this.$refs.treeWithList?.$refs.rightList?.tableDolayout()
      })
    },
    // refreshTree() {
