<template>
  <datablau-dialog
    :title="$t('meta.allApply.processDetails')"
      ...
      >
        ...
        >
          <div
            ...
            "
          >
            {{ $t('meta.allApply.initiateApplication') }}
          </div>
          <div style="margin-bottom: 5px">
            {{ $t('meta.allApply.applicant') }}{{ applyDisplayName }}
          </div>
        ...
        >
          <div
            ...
            "
          >
            {{ $t('meta.allApply.approvalProcessing') }}
          </div>
          <div style="margin-bottom: 5px">
            {{ $t('meta.allApply.approvedBy') }}{{ assigneeDisplayName }}
          </div>
          <div style="margin-bottom: 5px">
            {{ $t('meta.allApply.residenceTime') }}
            {{ getStayTime(statusDetailList[0].startTime) }}
          </div>
          <div>
            {{ $t('meta.allApply.urgentStatus')
            }}{{
              statusDetailList[statusDetailList.length - 1].tasks[0].urge
                ? $t('meta.allApply.urgedToProcess')
                : $t('meta.allApply.notUrgedToHandle')
            }}
          </div>
        </div>
        <div
        ...
        >
          <div
            ...
            "
          >
            {{ $t('meta.allApply.approvalResults') }}
          </div>
          ...
          <div v-else>{{ $t('meta.allApply.noResultsYet') }}</div>
        </div>
      </div>
  ...
  methods: {
    ...
    getStayTime(startTime) {
      ...
      let years = moment.duration(value).years()
        ? moment.duration(value).years() + ' ' + this.$t('meta.process.year')
        : ''
      let months = moment.duration(value).months()
        ? moment.duration(value).months() + ' ' + this.$t('meta.process.month')
        : ''
      let days = moment.duration(value).days()
        ? moment.duration(value).days() + ' ' + this.$t('meta.process.day')
        : ''
      let hours = moment.duration(value).hours()
        ? moment.duration(value).hours() + ' ' + this.$t('meta.process.hour')
        : ''
      let minutes = moment.duration(value).minutes()
        ? moment.duration(value).minutes() +
          ' ' +
          this.$t('meta.process.minute')
        : ''
      let seconds = moment.duration(value).seconds()
        ? moment.duration(value).seconds() +
          ' ' +
          this.$t('meta.process.second')
        : ''
