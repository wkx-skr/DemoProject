...
export default {
  ...
  methods: {
    ...
    getDateTemplate() {
      this.$http
        .post(this.$url + `/job/dateTemplate/list`)
        .then(res => {
    ...
    refreshCheck(jobId) {
      ...
      if (self.jobInRunning == true) {
        self.jobLogFresh.listRefresh = true
        self.loadCurrentJob(jobId)
      } else {
        if (self.jobLogFresh.listRefresh == true) {
          self.loadJobHistory(jobId)
          self.jobLogFresh.listRefresh = false
          clearInterval(self.jobLogFresh.interval)
          clearInterval(self.jobLogFresh.interval1)
          this.currentStep = ''
        }
      }
    ...
    preRunAtOnce() {
      const run = () => {
        ...
        this.$http
          ...
          .then(res => {
            //			self.$http.put(this.$url+'/service/datablau_jobs/' + jobId, requestBody).then(res=>{
            //				self.runAtOnce()
            self.jobInRunning = true
            // 聚合推荐任务启动时接收信号
            this.$emit('runJob')
            if (self.jobInRunning == true) {
              self.jobLogFresh.interval = setInterval(() => {
                self.refreshCheck(jobId)
              }, 3000)
              self.refreshJobStatus()
            } else {
              clearInterval(self.jobLogFresh.interval)
    ...
    runAtOnce() {
      ...
      self.$http
        ...
        .then(res => {
          const nextRunTime = res.data // only get the response but didn't do anything for now

          // set job initial status and update in every 3 sec.
          self.jobInRunning = true
          if (self.jobInRunning == true) {
            self.jobLogFresh.interval = setInterval(() => {
              self.refreshCheck(jobId)
