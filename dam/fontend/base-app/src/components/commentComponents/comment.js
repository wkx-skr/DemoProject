...
export default {
  ...
  methods: {
    ...
    createMessage(message) {
      ...
      this.$http
        .post(
          this.$url + `/thread/updateThread?threadId=${threadId}`,
          requestBody
        )
        .then(res => {
          this.getThread()
    ...
    getThread() {
      ...
      const url =
        typeId === 80010066
          ? `/thread/getItems?itemId=${this.typeId}&objectId=${objectId}&size=1000`
          : `/thread/getThreadMessageByObject?size=1000&objectId=${objectId}&typeId=${typeId}`
      this.$http
        .post(this.$url + url)
        .then(res => {
    ...
    createThread() {
      ...
      this.$http
        .post(this.$url + '/thread/createThread', requestBody)
        .then(res => {
          this.getThread()
        })
        .catch(e => {
