  ...
  <div class="choose-table-item tab-page">
    ...
    <div class="table-row" style="top: 60px">
      ...
      >
        ...
        <el-table-column prop="type" :label="$t('domain.dataFind.type')" sortable="custom">
          <template slot-scope="scope">
            <database-type
              :key="scope.row.type"
        ...
        <el-table-column
          ...
          sortable="custom"
        ></el-table-column>
        <el-table-column :label="$t('domain.dataFind.taskDetail')" min-width="180" v-if="domainVertify">
          <template slot-scope="scope">
        ...
        <el-table-column :label="$t('domain.dataFind.referenceProperty')" min-width="200" v-if="!domainVertify">
          <template slot-scope="scope">
            <datablau-checkbox2
              :min="1"
        ...
        <el-table-column :label="$t('domain.dataFind.enable')" width="100" v-if="!domainVertify">
          <template slot-scope="scope">
            <datablau-switch
              :key="switcherKey"
...
export default {
  ...
  props: ['choosedModels', 'compareConfigObj', 'domainVertify'],
  components: {
    DatabaseType,
  },
  computed: {},
  ...
  methods: {
    ...
    versionFormat: function (row) {
      return row.connectionInfo.versioning ? this.$t('domain.common.true') : this.$t('domain.common.false')
    },
    showSuccess(msg) {
      this.$message({
        title: 'Success',
    ...
    innerLoadDataSources() {
      ...
      self.loadingDS = true
      self.$http
        .get(self.BASE_URL + 'models/fromre/range')
        .then(res => {
