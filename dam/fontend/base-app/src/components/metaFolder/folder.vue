    ...
    <div class="folder-right-content">
      <div class="folder-header">
        <datablau-input
          ...
          :placeholder="$t('meta.file.fileNamePlaceholder')"
        ></datablau-input>
        <datablau-button
          type="normal"
...
export default {
  ...
  methods: {
    ...
    // 导出全部文件资产
    exportFolder(data) {
      const url = this.$meta_url + `/service/filetree/${data.id}/export`
    ...
    getFolderTree() {
      ...
      this.treeData = []
      this.$http
        .get(this.$meta_url + '/service/filetree/tree')
        .then(res => {
          // 过滤根目录root
    ...
    showExportTable() {
      ...
      const url = this.$meta_url + `/service/filetree/export`
      this.$downloadFilePost(url, this.checkArray, this.$t('meta.file.file'))
    },
    // 勾选选择框时的事件
    selectChange(val) {
    ...
    getTableData() {
      ...
      this.$http
        .post(this.$meta_url + '/service/shareFile/folder/page', requestBody)
        .then(res => {
