...
export default {
  data() {
    return {
      ...
      showHome: true,
      breadcrumbData: [],
      breadcrumbData1: [this.$t('domain.verification.domainLandingVerification')],
      breadcrumbData2: [this.$t('domain.verification.domainLandingVerification'), this.$t('domain.common.preview')],
  ...
  methods: {
    ...
    nodeClick(node) {
      ...
      if (clickNode === this.$t('domain.verification.domainLandingVerification')) {
        this.showHome = true
      }
    },
    tabClick(node) {
    ...
    setPieData() {
      const url = `${this.$url}/service/domains/domainVerifyPie`
      this.getPieData = this.$http.get(url)
    },
    setEsystemData() {
      const url = `${this.$url}/service/domains/domainVerifyBar`
      ...
      this.$http
        ...
        .then(res => {
          ...
          if (jobData && jobData.lastRunEndTime) {
            this.jobLastEndTime =
              this.$timeFormatter(jobData.lastRunEndTime) || this.$t('domain.common.none')
          }
          this.verJobData = jobData
