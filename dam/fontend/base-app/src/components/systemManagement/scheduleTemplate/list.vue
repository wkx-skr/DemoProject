  ...
  <div class="container" v-loading="isLoading">
    <datablau-list-search>
      ...
      <template slot="buttons">
        <datablau-button @click="addItem" style="" class="iconfont icon-add">
...
export default {
  ...
  methods: {
    ...
    editAccess(item) {
      const requestUrl = `/job/dateTemplate/update`
    ...
    handleExport(item) {
      this.$downloadFilePost(
        `/job/dateTemplate/exportDates?id=` + item.id
      )
    },
    preDelete(item) {
      this.$DatablauCofirm(
    ...
    deleteTemplate(item) {
      this.$http
        .post(`/job/dateTemplate/delete?id=${item.id}`)
        .then(() => {
          this.getTemplateList()
        })
        .catch(e => {
