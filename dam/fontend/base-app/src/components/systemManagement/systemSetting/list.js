  ...
  methods: {
    ...
    preDeleteRows() {
      ...
      if (this.multipleSelection.length === 1) {
        text = `删除“${this.multipleSelection[0].name}”为不可逆操作，是否确定删除？`
      } else {
        text = `删除已选中的${this.multipleSelection.length}个参数为不可逆操作，是否确定删除？`
      }
      this.$DatablauCofirm(text, '删除参数', {
        type: 'warning',
      })
        .then(() => {
          this.deleteRows()
    ...
    deleteRow(id, length) {
      this.$http
        .delete(this.$quality_url + '/parameters/' + id)
        .then(res => {
          this.deleteCounter++
    ...
    handleTest(index) {
      ...
      this.$http
        .get(
          this.$quality_url + '/parameters/' + detail.paramId + '/value/test'
        )
        .then(res => {
          this.$alert(
