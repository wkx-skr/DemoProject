    ...
    drawEcharts({ legendData, seriesData, catalogArr }) {
      ...
      if (container) {
        this.$nextTick(() => {
          ...
          //     this.$router.push('/main/accessControl?id=' + params.data.categoryId)
          //   } else {
      ...
      this.getAllTages
        ...
        .then(res => {
          ...
          for (const key in data) {
            ...
            const obj = {
              catalog: this.muluIdToName[key],
              catalogName: this.muluIdToName[key],
              categoryId: key,
              total: total,
              dataArr: arr,
              tagCountMap: cataData,
            }
            // if(!this.muluIdToName[key]) {
            //   break;
            // }
            formatData.push(obj)
            totalShow.push(0)
          }
          this.$bus.$emit('folder-size', formatData.length)
          formatData.sort((a, b) => {
