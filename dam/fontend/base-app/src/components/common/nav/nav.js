  ...
  methods: {
    ...
    getMenuMap() {
      ...
      const menuL1 = {
        ...
        gatewayManager: ['dataSecurityGateway', 'gatewayAudit'],
        ...
        dataControler: ['dataControl', 'whiteList'],
        dataDesensitization: [
          // 'dataDesensitization/staticRule',
          'datamaskingRule',
        ],
      ...
      const licRequired = {
        ...
        dataControl: ['FE_SECURITY'], // 数据申请单
        whiteList: ['FE_SECURITY'], // 白名单
      ...
      const authRequired = {
        ...
        // 分类分级工具
        dataControl: ['DATA_SECURITY_DATA_REQUISITION'],
        whiteList: ['DATA_SECURITY_WHITE_LIST_MANAGE'],
        ...
        modelCategory: ['BASE_SYSTEM_MANAGE'],
        ...
        domainStandard: ['DATA_STANDARD_FIELD_MANAGE'],
  ...
  watch: {
    ...
    $route(val) {
      ...
      if (this.isIE() || this.isIE11()) {
        if (['devApi', 'apiOverview'].includes(val.name)) {
      ...
      //   '/main/dashboard/dataCatalogDashboard'
      // ) {
      //   this.$bus.$emit('clearHighlightTopNav')
      //   this.$router.push({
      //     path: '/',
      ...
      // }
      /* // 资产统计大屏不展示左侧菜单
      if (this.$router.currentRoute.path === '/homePageScreen') {
        this.$bus.$emit('clearHighlightTopNav')
      } */
      this.resetDefault()
    },
