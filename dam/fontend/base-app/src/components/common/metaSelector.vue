...
export default {
  ...
  methods: {
    ...
    handleNodeClickWithoutSearchTables(data, node) {
      if (data.type === 'MODEL') {
        this.schema = null
        this.modelId = data.modelIds
        this.modelName = data.name
    ...
    getModelTree() {
      ...
      this.$http
        .get(
          this.$meta_url +
            '/service/models/modeltree?includeLogicalEntity=false&dataConnection=true'
        )
        .then(res => {
    ...
    searchTables() {
      const requestUrl = `${this.$meta_url}/entities/searchMetadata`
      const requestBody = {
        currentPage: 1,
    ...
    handleNodeClick2(data) {
      ...
      if (!this.modelId) {
        const modelId = data.parentId
        const treeNode = this.$refs.tree2.getNode(modelId)
        this.handleNodeClickWithoutSearchTables(treeNode.data, treeNode)
      }
    },
    searchColumns(tableId) {
      const requestUrl = `${this.$meta_url}/entities/${tableId}/columns`
      this.$http
        .get(requestUrl)
        .then(res => {
    ...
    saveTableDialog() {
      ...
      // const node = this.$refs.tree2.getNode(this.modelId)
