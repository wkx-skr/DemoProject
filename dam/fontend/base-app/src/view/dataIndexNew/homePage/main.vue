    ...
    <div class="home-page-top">
      <div class="top-main">
        ...
        <div class="search-result">
          ...
          <el-button
            type="text"
            v-for="(val, index) in currentKw"
            :key="val + index"
            @click.native="handleCurrentKw(val)"
          >
            {{ val }}
          </el-button>
        </div>
...
export default {
  ...
  methods: {
    ...
    search() {
      this.$router.push({
        name: 'search',
        query: { keyword: this.keyword, types: LDMTypes.Index },
      })
      this.setHomePageKw(this.keyword)
    },
    getApplyPend() {
      HTTP.getApplyPend()
        .then(res => {
    ...
    handleCurrentKw(val) {
      this.$router.push({
        name: 'search',
        query: { keyword: val, types: LDMTypes.Index },
      })
    },
    getCurrentIndex() {
      const obj = {
      ...
      HTTP.getFolderListServiceNew(obj)
        .then(res => {
