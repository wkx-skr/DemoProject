...
export default {
  ...
  methods: {
    ...
    // 获取详情
    getFormData() {
      let url = `${this.$metric_url}querytheme/get?id=${this.id}`
      this.$http
        .post(url)
        .then(res => {
    ...
    onSubmit() {
      this.$refs.form.validate(valid => {
        if (!valid) {
          ...
          // let url = this.$meta_url+ '/service/';
          let url = `${this.$metric_url}querytheme/create`
          ...
          if (this.iscreate) {
            ...
            body.creatUser = this.$user.username
            // body.tableId = this.tableId;
            if (!body.tableId) {
              this.$message.error(this.$t('meta.report.selDataSource'))
              return false
            }
            url = `${this.$metric_url}querytheme/create`
          } else {
            // 修改
            url = `${this.$metric_url}querytheme/update`
          }
          this.$http
            .post(url, body)
            .then(res => {
    ...
    // 获取目录树
    getTreeData() {
      let url = `${this.$metric_url}categories/tree?type=${this.type}`
      this.$http
        .post(url)
        .then(res => {
