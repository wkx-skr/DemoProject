...
export default {
  ...
  methods: {
    ...
    // 获取详情
    getFormData() {
      let url = `${this.$metric_url}dimension/get?id=${this.id}`
      this.$http
        .post(url)
        .then(res => {
    ...
    // 查询全部维度值
    getValue() {
      let url = `${this.$metric_url}dimension/list/get?id=${this.id}`
      this.$http
        .post(url)
        .then(res => {
          if (res.data) {
    ...
    onSubmit() {
      this.$refs.form.validate(valid => {
        if (!valid) {
          ...
          let url = `${this.$metric_url}dimension/create`
          ...
          if (this.iscreate) {
            // 新建
            url = `${this.$metric_url}dimension/create`
          } else {
            // 修改
            url = `${this.$metric_url}dimension/update`
          }
          this.$http
            .post(url, body)
            .then(res => {
              ...
              let url = `${this.$metric_url}dimension/value/create`
    ...
    // 获取目录树
    getTreeData() {
      let url = `${this.$metric_url}categories/tree?type=82800023`
      this.$http
        .post(url)
        .then(res => {
    ...
    getTreeList(callback) {
      let url = `${this.$metric_url}categories/get?type=82800023`
      this.$http
        .post(url)
        .then(res => {
