<template>
  <div>
    ...
    <div slot="footer" style="position: absolute; right: 20px; bottom: 0">
      <datablau-button @click="closeItem" type="secondary">
...
export default {
  ...
  methods: {
    ...
    getUdpsByType() {
      ...
      this.$http
        .post(`${this.$metric_url}dimension/udp/get?category=${this.typeId}`)
        .then(res => {
          this.tabLoading = false
          this.udps = res.data.sort((a, b) => a.udpOrder - b.udpOrder)
          if (
            this.typeId.includes('@@') ||
            [LDMTypes.TimeModifierType, LDMTypes.ModifierType].includes(
              parseInt(this.typeId)
            )
          ) {
          ...
          this.udps.forEach(item => {
            if (item.udpOrder > this.maxOrder) {
              this.maxOrder = item.udpOrder
            }
            dupUdpNamMap[item.name] = item
          })
          !this.udps.length && (this.maxOrder = 0)
    ...
    deleteRow(row) {
      this.$DatablauCofirm(
        ...
        .then(() => {
          let body = [row.id]
          this.$http
            .post(`${this.$metric_url}dimension/udp/delete`, body)
            .then(res => {
    ...
    swap(prev, next) {
      let num = this.udps[prev].udpOrder
      this.udps[prev].udpOrder = this.udps[next].udpOrder
      this.udps[next].udpOrder = num
      this.udps.sort((a, b) => b.udpOrder - a.udpOrder)
    ...
    changeOrder(requestBody, callback) {
      this.$http
        .post(`${this.$metric_url}dimension/udp/update`, requestBody)
        .then(res => {
          if (callback) {
            callback()
          }
