...
<script>
import emptyPage1 from '@/view/dataIndexNew/indexManagement/indexDefinition/emptyPage1'
...
export default {
  ...
  methods: {
    getData () {
      this.$http
        .post(
          `${this.$metric_url}metricManagement/dataPreview?metricId=${this.metricId}&currentPage=${this.currentPage}&pageSize=${this.pageSize}`
        )
        .then(res => {
          const tableData = []
