...
export default {
  ...
  methods: {
    ...
    getTreeData () {
      this.$http
        .post(this.$metric_url + `domains/tree/getTree`, {
    ...
    processCreateCategory () {
      let requestUrl = this.$metric_url + `domains/folder/createFolder`
      if (this.category.id) {
        requestUrl =
        this.$metric_url + `domains/folder/updateFolder?folder=${this.category.id}`
      }
    ...
    removeCategory () {
      this.$DatablauCofirm(this.$t('meta.report.delConfirm'), '')
        .then(() => {
          this.$http
            .post(`${this.$metric_url}domains/folder/deleteFolder`, {
