  ...
  <div>
    ...
    <datablau-form :label-width="$i18n.locale === 'zh' ? '6em' : '8em'">
      <el-form-item :label="$t('indicator.mapping.dim')">
        ...
        <datablau-button type="text" @click="addDim" v-if="!isAssets">
      ...
      <el-form-item :label="$t('indicator.mapping.metadata')">
        ...
        <datablau-button type="text" @click="setTable" v-if="!isAssets">
          {{ $t('meta.DS.tableDetail.security.set') }}
        </datablau-button>
      </el-form-item>
      <el-form-item
      ...
      >
        <template slot-scope="scope">
          <datablau-input
            ...
            @change="addOrUpdateMapping(scope.row, $event)"
            :disabled="!scope.row.objectId || isAssets"
          ></datablau-input>
        </template>
      </el-table-column>
      <el-table-column
...
export default {
  ...
  methods: {
    getMappingList() {
      this.$http
        .post(
          `${this.$metric_url}metricManagement/mapping/list?metricId=${this.metricId}`
        )
        .then(res => {
    ...
    getTableDetail() {
      if (this.tableId) {
        this.$http
          .get(this.$meta_url + `/service/entities/${this.tableId}/summary`)
          .then(res => {
            this.tableName = res.data.physicalName
            this.modelName = res.data.modelName + ' / ' + res.data.schema
    ...
    addOrUpdateMapping(row, newValue) {
      ...
      if (row.metricMappingId) {
        // update
        this.$http
          .post(`${this.$metric_url}metricManagement/mapping/update`, {
        ...
        this.$http
          .post(`${this.$metric_url}metricManagement/mapping/add`, {
            attrInfo: row.attrInfo,
            attrType: row.attrType,
            itemId: row.itemId,
            dimensionLevelId: row.dimensionLevelId,
    ...
    bindNullObject(requestBody) {
      return new Promise(resolve => {
        this.$http
          .post(`${this.$metric_url}metricManagement/mapping/add`, requestBody)
          .then(() => {
            resolve()
          })
          .catch(e => {
    ...
    realUnbindMapping(row) {
      this.$http
        .post(`${this.$metric_url}metricManagement/mapping/delete`, [
          row.metricMappingId,
        ])
        .then(() => {
          if (row) {
