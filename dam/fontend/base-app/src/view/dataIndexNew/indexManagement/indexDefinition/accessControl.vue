<template>
  <div>
    ...
    <datablau-table :data="tableData">
      ...
      >
        <template slot-scope="scope">
          <datablau-button type="text" from-table @click="viewItem(scope.row)" :disabled="isAssets">
...
<script>
import accessFormRule from '@/view/dataIndexNew/indexManagement/indexDefinition/accessFormRule'
...
export default {
  ...
  methods: {
          ...
          this.$http
            .post(
              formData.id || formData.id === 0
                ? `${this.$metric_url}metricManagement/metricAuth/updatePermission`
                : `${this.$metric_url}metricManagement/metricAuth/createPermission`,
              [formData]
            )
            .then(res => {
              this.visible = false
    ...
    getPermissionList () {
      this.$http
        .post(
          `${this.$metric_url}metricManagement/metricAuth/getPermissionsList?metricId=${this.metricId}`
        )
        .then(res => {
    ...
    removePermission (row) {
      this.$DatablauCofirm(this.$t('indicator.access.sure'), '')
        .then(() => {
          const id = row.id
          this.$http
            .post(`${this.$metric_url}metricManagement/deleteMetricAuth`, [id])
            .then(() => {
              this.getPermissionList()
            })
            .catch(e => {
