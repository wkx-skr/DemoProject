...
export default {
  ...
  methods: {
    exporttUdp() {
      if (this.standardCode === false) {
        const url = `${this.$metric_url}domains/exportDomainUdp?categoryId=${this.categoryTypeId}&type=1`
        this.$downloadFilePost(url)
      } else {
        const url = `${this.$metric_url}domains/exportDomainUdp?categoryId=${this.categoryTypeId}&type=2`
        this.$downloadFilePost(url)
      }
    },
    exportTemplate() {
      if (this.standardCode === false) {
        const url = `${this.$metric_url}domains/downloadDomainUdpTemplate?categoryId=${this.categoryTypeId}&type=1`
        this.$downloadFilePost(url)
      } else {
        const url = `${this.$metric_url}domains/downloadDomainUdpTemplate?categoryId=${this.categoryTypeId}&type=2`
        this.$downloadFilePost(url)
      }
    },
    importUdp() {
    ...
    submit() {
      this.$refs.ruleForm.validate(valid => {
        if (valid) {
          ...
          // let requestUrl = this.$meta_url+ '/service/domains/udps'
          // requestUrl += `?clear=true&categoryId=${this.categoryTypeId}`
          // this.$http
          //   .post(requestUrl, requestBody)
          HTTP.setDomainUdpsServiceMetric({
