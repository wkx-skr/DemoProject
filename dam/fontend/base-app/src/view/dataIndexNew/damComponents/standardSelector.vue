        ...
        >
          <div style="height: 100%; width: 100%; overflow: auto">
            ...
            >
              ...
              >
                <template slot-scope="scope">
                  {{ scope.row.path[scope.row.path.length - 1] }}
                </template>
              </el-table-column>
              <el-table-column
...
export default {
  ...
  methods: {
    ...
    initData() {
      ...
      HTTP.getOrgTree()
        .then(res => {
          ...
          this.descriptionDepartmentTree = [res]
          this.defaultExpandList = res.children.map(e => e.pbm)
        })
        .catch(e => {
          this.treeLoading = false
          this.$showFailure(e)
    ...
    innerLoadStandardTree() {
      ...
      return new Promise(resolve => {
        ...
        if (this.folderValue === '1') {
          url = `${this.$domain_url}/domains/tree/getTree`
        } else {
          url = `${this.$metric_url}domains/tree/getTree`
        }
        this.$http
          .post(url, obj)
    ...
    innerLoadStandard() {
      ...
      let url = ''
      if (this.folderValue === '1') {
        url = `${this.$domain_url}/domains/domain/getPublicPage`
      } else {
        url = `${this.$metric_url}domains/domain/getPublicPage`
      }
      this.$http
        .post(url, obj)
    ...
    tableSelectionChanged(selection) {
      ...
      if (!this.multiple && selection.length > 1) {
        ...
        this.$refs.selectDomainTable.toggleRowSelection(delRow, false)
      }
    },
    resetQuery() {
      this.searchFormData = {
