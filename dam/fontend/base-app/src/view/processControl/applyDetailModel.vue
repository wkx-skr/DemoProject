...
export default {
  ...
  methods: {
    ...
    transLateTabsName(item) {
      let obj = {
        applyList: this.$t('userPane.myTodo.applyForm'),
        approRecord: this.$t('userPane.myTodo.approRecord'),
        approOpinion: this.$t('userPane.myTodo.ApproOpinion'),
        applyInfo: this.$t('userPane.myTodo.applyInfo'),
        ruleApplyList: this.$t('userPane.myTodo.applyForm'),
        domainCompare: this.$t('userPane.myTodo.domainCompare'),
        catalogInfo: this.$t('userPane.myTodo.catalogInfo'),
        assetsInfo: this.$t('userPane.myTodo.assetsInfo'),
        domainCodeDetail: this.$t('userPane.myTodo.applyForm'),
        codeApplyData: this.$t('userPane.myTodo.approveData'),
      }
      return obj[item.name] || item.label
    },
    dataInit() {
      return new Promise(resolve => {
        ...
        HTTP.getApplyDetail(para)
          .then(res => {
            ...
            let id = data.formDtos.find(
              item => item.code === 'projectId'
            )?.value
            let projectId = ''
              try {
                projectId = (id && JSON.parse(id)[0]) || (idAry && JSON.parse(idAry)[0])
              } catch(e) {
                console.log(e);
              }
                  ...
                  .then(res => {
                    this.$nextTick(() => {
                      this.techRule.applyFileName =
                        res.data[0]?.fileOrginalName ||
                        this.$t('userPane.myTodo.noAttachment')
                    })
                  })
              }
            }
