  ...
  >
    <datablau-tabs v-model="activeName">
            ...
            >
              <span
                ...
                :style="{
                  ...
                  width:
                    410 -
                    (item.name || catalogProcessAttrLabel[item.code]).length *
                      20 +
                    'px',
                }"
              >
      ...
      <el-tab-pane
        ...
        label="申请表单"
        name="catalogApply"
        v-if="applyDetailData.processType === '资产目录权限申请'"
      >
        <div class="item-title"></div>
        ...
        <div class="apply-form">
          ...
          <datablau-detail :column="1" labelWidth="120px">
            <el-form-item
              ...
              style="width: 45%"
            >
              <span v-if="item.code === 'authorityType'">{{ '可读' }}</span>
              <span
                v-else-if="item.name || catalogProcessAttrLabel[item.code]"
                :style="{
                  display: 'inline-block',
                  width:
                    410 -
...
export default {
  ...
  methods: {
    ...
    handleAssetSizeChange(val) {
      ...
      this.getList(this.applyDetailData.processInstanceId)
    },
    ...
    getList(processInstance) {
      ...
      this.$http
        ...
        .then(res1 => {
          ...
          this.tableFormData = res1.data.data.content
        })
    },
    transState(type) {
      if (type === 'UNPUBLISHED') {
        ...
        HTTP.getApplyDetail(para)
          .then(res => {
            ...
            if (
              ...
              this.applyDetailData.processType === '资产目录权限申请'
            ) {
              ...
              // 资产发布为批量发布
              this.getList(this.applyDetailData.processInstanceId)
              formDtos = this.handleAssetsProcessAttr(data.formDtos)
            } else {
              formDtos =
