...
export default {
  ...
  mixins: [Resource],
  props: ['summary', 'name', 'currentNode', 'permission'],
  data() {
    const hierarchy = []
    for (let i = 1; i <= 100; i++) {
      hierarchy.push({
  ...
  methods: {
        ...
        this.searchFormData.nodeOptions.forEach(element => {
          ...
          if (this.typeNameArr[element.value]) {
            this.$set(
              element,
              'label',
              this.summary.logicalElement && element.value === '80000004'
                ? '实体'
                : this.summary.logicalElement && element.value === '80000005'
                ? '属性'
                : this.typeNameArr[element.value]
            )
          }
        })
      }
    ...
    typeNameArrName(type, nodesArrayDetail) {
      if (
        nodesArrayDetail.properties &&
        nodesArrayDetail.properties.logicalElement === 'true'
      ) {
        if (nodesArrayDetail.typeId === '80000004') {
          return '实体'
        } else if (nodesArrayDetail.typeId === '80000005') {
          return '属性'
        } else {
        ...
        if (RightRelation.length > 0) {
          ...
          RightRelation.forEach(item => {
            ...
            if (
              ...
              LevelDisplayMap.get(currentLevel) &&
    ...
    imageNode(element, type) {
      // function checkImageUrl(url, timeout = 500) {
        ...
        this.nodesArray.forEach(element => {
          ...
          if (this.nodeExtendArr.length !== 0 && element.level) {
            ...
            this.nodesArr.push({
              ...
              size: 24,
              scope: element.startNode.scope,
              logicalElement: element.startNode.properties.logicalElement
                ? element.startNode.properties.logicalElement
                : '',
            })
            this.nodesArr.push({
              detail: element.endNode,
          ...
          if (element.startNode.level === 1 || element.startNode.level === 0) {
            this.searchFormData.nodeOptions.push({
              ...
              label:
                this.summary.logicalElement &&
                element.startNode.typeId === '80000004'
                  ? '实体'
                  : this.summary.logicalElement &&
                    element.startNode.typeId === '80000005'
                  ? '属性'
                  : this.typeNameArr[element.startNode.typeId],
            })
          }
              ...
              label:
                ...
                element.endNode.typeId === '80000004'
                  ...
                  : this.summary.logicalElement &&
                    element.endNode.typeId === '80000005'
                  ? '属性'
                  : this.typeNameArr[element.endNode.typeId],
            })
          }
        })
      }
