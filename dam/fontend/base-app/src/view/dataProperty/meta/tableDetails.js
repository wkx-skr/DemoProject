...
export default {
  ...
  methods: {
    ...
    saveLogicalName() {
      ...
      const requestUrl = this.$meta_url + `/entities/${this.summary.id}/summary`
      const requestBody = {
    ...
    saveDefinition() {
      ...
      if (this.object.type === 'REPORT') {
        ...
        const requestUrl =
          this.$meta_url + `/entities/${this.summary.id}/summary`
        const requestBody = {
    ...
    bindTagToObject() {
      if (this.objectType === 'FILE') {
        ...
        this.$http
          .post(
            this.$meta_url +
              `/tag/tags/choose?objectId=${this.objectId}&typeId=${this.summary.properties.TypeId}`,
    ...
    getObjectBindTags() {
      const id = this.summary.id
      const url = `${this.$url}/tags/getTagsOfItem?itemId=${id}&withoutCategory=true&type=82800008`
      this.$http
        .post(url)
        .then(res => {
