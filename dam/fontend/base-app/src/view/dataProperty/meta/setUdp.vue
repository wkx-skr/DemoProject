...
export default {
  ...
  data() {
    ...
    if (this.from === 'modelCategory') {
      ...
      } else if (this.groupId && this.groupId === 1) {
        typeMap = TypeMap.logical
      } else {
        typeMap = TypeMap.meta
      }
    }
  ...
  methods: {
    ...
    getUdpsByType() {
      ...
      const pre = this.from === 'modelCategory' ? this.$url : this.$meta_url
      this.$http
        .post(
          `${pre}/udps/getUdpsOfType?typeId=${typeId}&groupId=${
    ...
    deleteRow(row) {
      this.$DatablauCofirm(
        ...
        .then(() => {
          ...
          this.$http
            .post(pre + `/udps/deleteUdp?udpId=${row.id}`)
            .then(res => {
    ...
    changeOrder(requestBody, callback) {
      ...
      this.$http
        .post(
          pre + `/udps/updateUdpOfType?typeId=${this.currentTypeId}`,
          requestBody
        )
        .then(res => {
          if (callback) {
