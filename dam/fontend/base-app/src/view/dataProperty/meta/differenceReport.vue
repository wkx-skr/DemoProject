<template>
  <div class="tab-page model-compare-page" v-loading="loading || !schemasReady">
    ...
    <div style="margin-left: 10px; font-size: 16px; font-weight: bold">
      {{ $t('meta.DS.treeSubOperation.modelBind') }}
    </div>
    <div
      id="diff-tree-box"
      v-loading="!showTree"
    ...
    >
      ...
      >
        ...
        <span>
          {{ item.originPath || $t('meta.DS.treeSubOperation.historyData') }}
        </span>
        <br />
        <span>{{ $t('meta.DS.treeSubOperation.branch') }}：</span>
        <span>
          {{ item.originBranch || $t('meta.DS.treeSubOperation.historyData') }}
        </span>
        <br />
        <span>{{ $t('meta.DS.treeSubOperation.version') }}：</span>
        <span>
          {{ item.versionName || $t('meta.DS.treeSubOperation.historyData') }}
        </span>
    ...
    >
      <div class="message">
        ...
        >
          <el-form-item
            :label="$t('meta.DS.treeSubOperation.dasourceSchema')"
            v-if="hasSchemas"
          >
            <el-select
          ...
          <datablau-tabs v-model="curTab" @tab-click="handleTabChange">
            <el-tab-pane
              :label="$t('meta.DS.treeSubOperation.modelCompare')"
              name="compare"
            ></el-tab-pane>
            <el-tab-pane
              :label="$t('meta.DS.treeSubOperation.modelSync')"
              name="sync"
            ></el-tab-pane>
            <el-tab-pane
              :label="$t('meta.DS.treeSubOperation.modelRelation')"
              name="relation"
            ></el-tab-pane>
          </datablau-tabs>
          ...
          <ul v-if="curTab === 'sync'" style="margin-bottom: 30px">
            <div style="margin-bottom: 10px">
              <span>{{ $t('meta.DS.treeSubOperation.runSync') }}</span>
              <datablau-switch
      ...
      <div class="preview">
        <div v-if="curTab === 'compare'" style="margin-bottom: 10px">
          <span>{{ $t('meta.DS.treeSubOperation.runCompare') }}</span>
          <datablau-switch
            style="display: inline-block; margin-left: 20px"
        ...
        <div v-if="curTab === 'relation'" style="margin-bottom: 10px">
          <span>{{ $t('meta.DS.treeSubOperation.runRelation') }}</span>
          <datablau-switch
            style="display: inline-block; margin-left: 20px"
          ...
          <span style="vertical-align: middle">
            <i class="iconfont icon-tips">
              {{ $t('meta.DS.treeSubOperation.runRelationTips') }}
            </i>
          </span>
        </div>
        <div class="sub-title part-title" v-if="curTab !== 'relation'">
          {{
            curTab === 'sync'
              ? $t('meta.DS.treeSubOperation.syncRes')
              : curTab === 'compare'
              ? $t('meta.DS.treeSubOperation.compareRes')
              : ''
          }}
        </div>
