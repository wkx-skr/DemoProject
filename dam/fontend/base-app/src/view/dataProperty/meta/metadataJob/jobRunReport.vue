...
export default {
  ...
  methods: {
    ...
    formateJobSta(row, column, cellValue, index) {
      const sta = cellValue
      const staMap = {
        INFO: this.$t('meta.job.INFO'),
        ERROR: this.$t('meta.job.ERROR'),
        STOP: this.$t('meta.job.STOP'),
        COMPLETE: this.$t('meta.job.COMPLETE'),
        RUNNING: this.$t('meta.job.RUNNING'),
      }
