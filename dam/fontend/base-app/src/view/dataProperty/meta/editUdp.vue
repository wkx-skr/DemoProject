    ...
    >
      <el-form-item :label="$t('meta.DS.udp.udpName')" prop="name">
        <datablau-input
          ...
          class="formItem maxlengthInput max3"
          v-model="body.name"
          show-word-limit
          maxlength="100"
        ></datablau-input>
      ...
      <el-form-item :label="$t('meta.DS.udp.desc')">
        <datablau-input
          ...
          :minRow="3"
          :maxRows="5"
          style="max-height: 100px; width: 500px"
          v-model="body.desc"
          show-word-limit
        ></datablau-input>
      </el-form-item>
...
import { BasePrefix } from '../../../../../base-components/http/baseController/BaseController'
export default {
  props: [
    'typeId',
    'currentRow',
  ...
  computed: {
    disabledSave() {
      ...
      let typeData = true
      if (this.body.type === 'ENUM') {
        typeData = _.trim(this.body.typeData)
      } else {
        typeData = true
      }
      return (
  ...
  methods: {
    selectBlur(e) {
      // 意见类型
      if (e.target.value !== '') {
        this.body.catalog = e.target.value.slice(0, 50)
    ...
    getUdpList() {
      ...
      if (this.from === 'modelCategory') {
        ...
        pre = this.$meta_url
        if (this.groupId) {
          url = pre + `/entities/udp/select?groupId=${this.groupId}`
        } else {
          url = pre + `/entities/udp/select`
        }
        method = 'get'
      }
    ...
    createOrUpdateUdp() {
      ...
      requestBody.groupId = this.groupId ? this.groupId : ''
      let pre = this.from === 'modelCategory' ? this.$url : this.$meta_url
      this.$http
        .post(`${pre}/udps/updateUdpOfType?typeId=${this.typeId}`, requestBody)
        /* UserDefinedPropertyController.createOrUpdateUdp({
        typeId: this.typeId,
        requestBody: requestBody,
      }) */
        /* this.$http
        .post(this.$url + `/entities/udps/${this.typeId}`, requestBody) */
        .then(res => {
          this.$emit('refresh')
          this.$emit('close')
        })
