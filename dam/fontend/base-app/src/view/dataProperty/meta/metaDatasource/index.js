...
import reDetail from './reDetail.vue'
import reMain from './reMain.vue'
import MetadataJob from '@/view/dataProperty/meta/metaDatasource/MetadataJob.vue'
...
export default {
  ...
  mounted() {
    this.$bus.$on('showMetadataJob', this.showMetadataJob)
  ...
  methods: {
    showMetadataJob(args) {
      this.metadataJobArgs = args
      this.metadataJobVisible = true
    },
    openDefaultPage() {
      if (this.$route.query.resourceId && this.$route.query.jobType) {
        this.metadataJobArgs = {
    ...
    showRe(obj) {
      this.isEdit = !obj.isAdd
      this.isReport = obj.reType === 'report'
      this.isFile = obj.reType === 'file'
      this.isShareFile = obj.reType === 'shareFile'
      this.isMetaModel = obj.reType === 'metaModel'
      if (!obj.isAdd) {
        this.editRow = obj.row
      } else {
        this.editRow = null
      }
  ...
  data() {
    return {
      ...
      isShareFile: false,
      isMetaModel: false,
      metadataJobVisible: false,
      metadataJobArgs: null,
      defaultKeyword: this.$route.query.keyword || '',
    }
  },
}
