      ...
      <div class="compare-message left-cont">
        <div class="message-name">
          <p style="display: flex">
            ...
            <datablau-button
              v-if="
                $auth['METADATA_EDIT'] || $auth['METADATA_EDIT_CURRENT_SYSTEM']
              "
              type="icon"
              @click="handleClick(compareIds[0])"
          ...
          <p>
            ...
            <span>
              {{ $timeFormatter(compareIds[0].createTime) }}
            </span>
          </p>
        </div>
      </div>
        ...
        <div class="message-name">
          <p style="display: flex">
            ...
            <datablau-button
              ...
              "
              type="icon"
              @click="handleClick(compareIds[1])"
              style="margin-left: 8px"
            >
              <i class="iconfont icon-bianji"></i>
            </datablau-button>
          ...
          <p>
            ...
            <span>
              {{
                moment(compareIds[1].createTime).format('YYYY-MM-DD HH:mm:ss')
              }}
            </span>
          </p>
        </div>
      ...
      >
        ...
        >
          <template slot-scope="scope">
            <datablau-icon
              :data-type="
                isLogical && scope.row.type === 'table'
                  ? 'logicaltable'
                  : scope.row.type === 'column' && isLogical
                  ? 'logicalcolumn'
                  : scope.row.type
              "
              :size="18"
      ...
      <div class="details-search">
        ...
        >
          <el-radio-button label="table">
            {{ isLogical ? '实体' : $t('meta.DS.treeSubOperation.table') }}
          </el-radio-button>
          <el-radio-button label="columns">
            {{ isLogical ? '属性' : $t('meta.DS.treeSubOperation.column') }}
          </el-radio-button>
      ...
      <div v-if="radioSearch === 'table'" class="report-details">
        <div class="report-details-first">
          <div class="first-title">
            <datablau-tooltip
              :content="compareIds[0].versionName"
              placement="top"
              :disabled="long"
            >
              <p class="" @mouseover="getTooltip($event)">
                {{ compareIds[0].versionName }}
              </p>
            </datablau-tooltip>
        ...
        <div class="report-details-sencond">
          <div class="sencond-title">
            <datablau-tooltip
              :content="compareIds[1].versionName"
              placement="top"
              :disabled="long"
            >
              <p class="" @mouseover="getTooltip($event)">
...
export default {
  ...
  methods: {
    ...
    downloadDetails() {
      let url = `${this.$meta_url}/entities/version/compare/download`
      let obj = {
    ...
    getCompare() {
      ...
      this.$http
        .post(this.$meta_url + '/entities/version/compare', response)
        .then(res => {
    ...
    getCompareDetail() {
      ...
      this.$http
        .post(
          this.$meta_url + '/entities/version/compare/getCompareDetail',
          response
        )
        .then(res => {
    ...
    getComparePage(typeId) {
      ...
      this.$http
        .post(
          this.$meta_url + '/entities/version/compare/getComparePage',
          response
        )
        .then(res => {
          if (this.radioSearch === 'table') {
  ...
  mounted() {
    this.getReDetail(this.compareIds[0].modelId)
