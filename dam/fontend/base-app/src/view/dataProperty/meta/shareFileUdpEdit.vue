...
export default {
  ...
  methods: {
    ...
    setUdpVal() {
      this.$http
        .post(
          `${this.$meta_url}/udps/getItemUdps?itemId=${this.summary.id}&typeId=82800008`
        )
        .then(res => {
          let result = res.data
          result.forEach(r => {
            this.udps.forEach(udp => {
              if (udp.udpId === r.id) {
                udp.value = JSON.parse(r.value).value
              }
            })
          })
        })
    ...
    /* getUdps() {
      ...
      const url = `${this.$meta_url}/service/shareFileUdpVal/${id}`
    ...
    addPropertyValue(u, i) {
      ...
      this.$http
        .post(
          this.$meta_url +
            `/udps/updateItemUdpValue?itemId=${this.summary.id}&udpId=${u.udpId}&containerType=80010001&containerId=${this.summary.modelId}`,
          body
        )
        .then(res => {
