  ...
  <div class="content" style="position: relative">
    <div id="kg">
      <div class="content-search" style="height: 50px">
        <div class="left-content">
          ...
          >
            ...
            <el-form-item
              :label="$t('meta.DS.tableDetail.knowledgeGraph.sameLevelNum')"
            >
              <datablau-select
                ref="siblingsSelect"
                v-model="maxSibling"
              ...
              >
                <el-option
                  :value="100"
                  :label="$t('meta.DS.tableDetail.knowledgeGraph.noLimit')"
                ></el-option>
                <el-option
                  :value="10"
                  :label="$t('meta.DS.tableDetail.knowledgeGraph.max10')"
                ></el-option>
                <el-option
                  :value="5"
                  :label="$t('meta.DS.tableDetail.knowledgeGraph.max5')"
                ></el-option>
              </datablau-select>
            </el-form-item>
            <el-form-item
      ...
      <div v-show="noresultState === true" v-loading="loadingKg">
        ...
        <div id="detail-box" v-show="false" :class="{ 'is-width': isWidth }">
          ...
          <div class="ul-box">
            ...
            <p class="value" v-if="$route.query.type !== 'META_MODEL'">
              {{
                typeNameArrName(nodesArrayDetail.typeId, nodesArrayDetail)
            ...
            <p class="value" v-else>
              {{
                typeNameArrName(nodesArrayDetail.typeId, nodesArrayDetail) ||
                (metaModelTypeMap &&
                  metaModelTypeMap[nodesArrayDetail.typeId] &&
                  metaModelTypeMap[nodesArrayDetail.typeId].chineseName)
              }}({{ nodesArrayDetail.typeId }})
            </p>
          </div>
          <div class="ul-box">
            <p class="key">
              {{ $t('meta.DS.tableDetail.knowledgeGraph.source') }}
            </p>
        ...
        <div id="detail-box3" v-show="false">
          <p class="title">
            {{ $t('meta.DS.tableDetail.knowledgeGraph.fuzzySearch') }}
          </p>
          <datablau-input
            ...
            <div v-for="(k, index) in typeArr" :key="index" class="legend-item">
              ...
              <p>
                <img
                  ...
                  :src="'static/kgimg/' + k + '1.svg'"
                  alt=""
                  v-if="summary.logicalElement && k === '80000004'"
                />
                <img
                  style="width: 30px; height: 30px; margin-right: 0.5em"
                  :src="'static/kgimg/' + k + '1.svg'"
                  alt=""
                  v-else-if="summary.logicalElement && k === '80000005'"
                />
                <img
                  style="width: 30px; height: 30px; margin-right: 0.5em"
                  :src="'static/kgimg/' + k + '.svg'"
                  alt=""
                  v-else-if="typeNameArr[k]"
                />
                <img
                  v-else
                  style="width: 30px; height: 30px; margin-right: 0.5em"
                  :src="metaModelIconMap[k]"
                  alt=""
                />
                <span v-if="summary.logicalElement && k === '80000004'">
                  实体
                </span>
                <span v-else-if="summary.logicalElement && k === '80000005'">
                  属性
                </span>
                <span v-else>
                  {{
                    ...
                    (metaModelTypeMap &&
                      metaModelTypeMap[k] &&
                      metaModelTypeMap[k].chineseName)
                  }}
                </span>
              </p>
            </div>
      ...
      <list-data
        ...
        @goDetail="goDetail"
        :typeNameArr="typeNameArr"
        :metaModelIconMap="metaModelIconMap"
      ></list-data>
    </div>
  </div>
</template>
