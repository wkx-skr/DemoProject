...
<template>
  ...
  >
    ...
    <div
      ...
      v-if="useWorkflow && headerProduction.toUpperCase() === 'DAM'"
    >
  ...
  methods: {
    handleClose(tag) {
      ...
      this.$http
        .post(
          this.$url +
            `/service/entities/${this.details.code}/tags/delete?typeId=80010098`,
          [
            {
              name: tag.name,
            },
    ...
    getCodeDetail(code) {
      ...
      HTTP.getCodeContent({
        ...
        .then(res => {
          ...
          this.$emit('setPath', [res.data.datasetName, res.data.code])
        })
        .catch(e => {
          this.showLoading = false
          this.$showFailure(e)
