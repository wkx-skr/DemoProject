...
export default {
  ...
  components: { LineageGraphEntrance },
  data() {
    return {
      loading: false,
      param: {
  ...
  methods: {
    ...
    getRawData() {
      ...
      if (this.object.type === 'REPORT') {
        url = `${this.$meta_url}/lineage/report/${this.object.objectId}/type`
      } else {
        url = `${this.$meta_url}/lineage/object/${this.object.objectId}/?onlyImpact=false`
      }
      this.$http
        .get(url)
        .then(res => {
