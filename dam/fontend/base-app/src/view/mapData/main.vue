<template>
  <div class="con">
    ...
    <div class="searchBox">
      <div class="left">
        <datablau-input
          ...
          :iconfont-state="true"
          v-model="name"
          placeholder="搜索系统名称、表名称"
          @input="getDataBySearch"
        ></datablau-input>
      </div>
      <div class="right">
        ...
        <datablau-button
          type="icon"
          class="icon-location iconfont"
          tooltipContent="定位"
          @click="orientation"
        ></datablau-button>
        <datablau-button
          type="icon"
          class="icon-quanping1 iconfont"
          tooltipContent="全屏"
          v-if="!fullScreen"
    ...
    <div
      v-if="showSearchBox"
      :style="{ width: goBackFlag ? '436px' : '406px' }"
      class="searchBoxRight"
    >
      <!--      <div style="padding-left: 20px">搜索结果</div>-->
      <datablau-tabs
        v-model="activeName"
        class="searchListBox intoPane"
        @tab-click="handleTabClick"
      >
        <el-tab-pane
          :label="'系统(' + systemSearList.length + ')'"
          name="first"
        >
          <ul class="systemList">
            <li
              v-for="(item, i) in systemSearList"
              :key="item.id + i"
              @click="orientationNode(item)"
            >
              <div class="titleName">
                ...
                <datablau-tooltip
                  ...
                  placement="top-start"
                  :disabled="item.categoryName && item.categoryName.length < 16"
                >
                  <span>
                    {{
                      item.categoryName && item.categoryName.length < 16
                        ? item.categoryName
                        : item.categoryName?.substring(0, 14) + '...'
                    }}
                  </span>
                </datablau-tooltip>
                <datablau-tooltip
                  class="item"
                  effect="dark"
                  :content="item.categoryAbbreviation"
                  placement="top-start"
                  :disabled="
                    item.categoryAbbreviation &&
                    item.categoryAbbreviation.length < 16
                  "
                >
                  <span>
                    ({{
                      ...
                      item.categoryAbbreviation.length < 16
                        ? item.categoryAbbreviation
                        : item.categoryAbbreviation?.substring(0, 14) + '...'
                    }})
                  </span>
                </datablau-tooltip>
                <!--                ({{ item.categoryAbbreviation }})-->
              ...
              <div style="margin-top: 0.5em">
                ...
                <datablau-tooltip
                  ...
                  :content="item.zone"
                  placement="top-start"
                  :disabled="!item.zone || item.zone.length < 16"
                >
                  <p>
                    {{
                      item.zone
              ...
              <datablau-tooltip
                ...
                placement="top-start"
                :disabled="!item.description || item.description.length < 46"
              >
                  ...
                  描述：{{
                    item.description
                      ? item.description.length < 46
                        ? item.description
                        : item.description?.substring(0, 44) + '...'
                      : '暂无描述'
                  }}
                </div>
              </datablau-tooltip>
        ...
        <el-tab-pane :label="'表(' + modelTable.length + ')'" name="tables">
          <ul class="systemList">
            ...
            >
              ...
              <div style="margin-top: 0.5em">
                <span>所属模型：</span>
                {{ item.zone || item.parentPhysicalName }}
              </div>
              <div class="description" style="margin-top: 0.5em">
                <span>描述信息：</span>
                {{ item.definition ? item.definition : '暂无描述' }}
              </div>
            </li>
          </ul>
        </el-tab-pane>
    ...
    <div class="rightCom" v-show="showRightIcon">
      <div class="comBox" v-if="typeActive === 'overview'">
        <!-- 点击系统 -->
        <div
          class="searchBoxRight"
          :style="{ width: goBackFlag ? '436px' : '406px' }"
        ...
        >
          <div class="top">
            <datablau-tooltip
              class="item"
              effect="dark"
              :content="information.categoryName"
              placement="top-start"
              :disabled="
                information.categoryName && information.categoryName.length < 16
              "
            >
              <div class="title">
                {{
                  information.categoryName &&
                  information.categoryName.length < 16
                    ? information.categoryName
                    : information.categoryName?.substring(0, 14) + '...'
                }}
              </div>
            </datablau-tooltip>
            <datablau-tooltip
              class="item"
              effect="dark"
              :content="information.categoryAbbreviation"
              placement="top-start"
              :disabled="
                information.categoryAbbreviation &&
                information.categoryAbbreviation.length < 16
              "
            >
              <div class="sTil">
                {{
                  ...
                  information.categoryAbbreviation.length < 16
                    ? information.categoryAbbreviation
                    : information.categoryAbbreviation?.substring(0, 14) + '...'
                }}
              </div>
            </datablau-tooltip>
            <datablau-tooltip
              ...
              placement="top-start"
              :disabled="
                !information.description || information.description.length < 46
              "
            >
              <div class="description">
                描述：{{
                  information.description
                    ? information.description.length < 46
                      ? information.description
                      : information.description?.substring(0, 44) + '...'
                    : '暂无描述'
                }}
              </div>
            </datablau-tooltip>
            ...
            <!--<div class="description">
              描述：{{
                information.description ? information.description : '暂无描述'
              }}
            </div>-->
          </div>
          <div class="tableFlex">
                ...
                <ul class="information">
                  <li>
                    ...
                    <datablau-tooltip
                      class="item"
                      effect="dark"
                      :content="information.zone"
                      placement="top-start"
                      :disabled="
                        !information.zone || information.zone.length < 16
                      "
                    >
                      <p>
                        {{
                          information.zone
                            ? information.zone.length < 16
                              ? information.zone
                              : information.zone.substring(0, 14) + '...'
                            : '未指定业务域'
                        }}
                      </p>
                    </datablau-tooltip>
                    <!--{{ information.zone ? information.zone : '未指定业务域' }}-->
                  </li>
                  <li>
                    <span>IT部门</span>
                    {{ information.itDepartment }}
                  </li>
                  <li>
                    <span>负责人</span>
                    {{ information.owner }}
                  ...
                  <li>
                    ...
                    <datablau-tooltip
                      class="item"
                      effect="dark"
                      :content="information.deployment"
                      placement="top-start"
                      :disabled="
                        !information.deployment ||
                        information.deployment.length < 16
                      "
                    >
                      <p>
                        {{
                          information.deployment
                            ? information.deployment.length < 16
                              ? information.deployment
                              : information.deployment.substring(0, 14) + '...'
                            : ''
                        }}
                      </p>
                    </datablau-tooltip>
            ...
            >
              ...
              >
                ...
                <el-table-column
                  ...
                  label="用途"
                ></el-table-column>
                <el-table-column prop="value" label="数据源"></el-table-column>
              </datablau-table>
            </div>
          </div>
        </div>
        ...
        >
          <div class="top">
            <datablau-tooltip
              class="item"
              effect="dark"
              :content="modelDetail.categoryName"
              placement="top-start"
              :disabled="
                modelDetail.categoryName && modelDetail.categoryName.length < 16
              "
            >
              <div class="title">
                {{
                  modelDetail.categoryName &&
                  modelDetail.categoryName.length < 16
                    ? modelDetail.categoryName
                    : modelDetail.categoryName?.substring(0, 14) + '...'
                }}
              </div>
            </datablau-tooltip>
            <datablau-tooltip
              class="item"
              effect="dark"
              :content="modelDetail.definition"
              placement="top-start"
              :disabled="
                modelDetail.definition && modelDetail.definition.length < 16
              "
            >
              <div class="sTil">
                ...
                {{
                  modelDetail.definition && modelDetail.definition.length < 16
                    ? modelDetail.definition
                    : modelDetail.definition?.substring(0, 14) + '...'
                }}
              </div>
            </datablau-tooltip>
            ...
            >
              <el-tab-pane label="基本信息" name="first">
                <ul class="information" v-if="modelDetail.reverseOptions">
                  <li>
                    <span>版本</span>
                    {{ modelDetail.reverseOptions.DBVersion }}
                  </li>
                  <li>
                    <span>IP地址</span>
                    {{ modelDetail.reverseOptions.HostServer }}
                  </li>
                  <li>
                    <span>负责人</span>
                    {{ modelDetail.owner }}
                  ...
                  <li>
                    ...
                    <div>
                      {{
                        modelDetail.reverseOptions.SELECT_SCHEMA ||
                        modelDetail.reverseOptions.DATABASE_NAME
                      }}
                    </div>
                  </li>
                </ul>
                    ...
                    <el-table-column prop="" label="" width="30">
                      <template scope="{row}">
                        <datablau-tooltip
                          ...
                          effect="dark"
                          :content="
                            row.lineageBindStatus === true ? '有血缘关系' : ''
                          "
                          placement="top"
                          :disabled="row.lineageBindStatus !== true"
                        >
                          <div
                            class="drop"
                            :class="{ have: row.lineageBindStatus === true }"
                          ></div>
                        </datablau-tooltip>
                      </template>
                    </el-table-column>
        ...
        >
          <div>
            <div
              class="lingeDetail title oneline-eclipse"
      ...
      <div class="comBox" v-if="typeActive === 'standard'"></div>
      <div class="comBox" v-if="typeActive === 'quality'"></div>
      <div class="comBox" v-if="typeActive === 'security'"></div>
      <div class="comBox" v-if="typeActive === 'earlyWarning'"></div>
      <div class="comBox" v-if="typeActive === 'stakeholders'"></div>
      <!-- <div class="nav">
        <div
          class="nav-li"
          :class="{ 'nav-liActive': typeActive === item.type }"
        ...
        >
          <div class="nav-li-icon">
            <img
              v-if="typeActive === item.type"
              style="width: 24px; height: 24px"
              :src="require(`../img/${item.icon}-1.svg`)"
              alt=""
            />
            <img
              v-else
    ...
    <!--    <div v-if="typeActive === 'overview'"></div>-->
    ...
    <lineageCom
      v-if="routineBlood"
      :propertiesFileData="propertiesFileData"
      @close="close"
    ></lineageCom>
...
export default {
  ...
  computed: {
    showRightIcon() {
      this.typeActive = 'overview'
      return this.information || this.modelDetail || this.edgeClick
    },
  },
  methods: {
    navClick(item) {
    ...
    getModelDetail() {
      this.$http
        .get(`${this.$meta_url}/models/${this.modelId}/plain`)
        .then(res => {
          this.modelTable = []
    ...
    async getPublic() {
      /* const about = await this.$http.post(`gateway/main/about`)
    ...
    handleTabClick() {
      switch (this.activeName) {
        ...
        case 'four':
          ...
          break
        case 'table':
          if (!this.environmentData && !this.modelTable.length) {
            this.getModelTable()
          }
          break
      }
    ...
    getProjectData() {
      if (!this.information.categoryId) return
      this.$http
        .get(`${this.$ddd_url}/service/dataMap/${this.information.categoryId}`)
        .then(res => {
    ...
    getProgramList(type) {
      ...
      this.$http
        .get(`${this.$ddd_url}/service/dataMap/${from}/${to}/${type}`)
        .then(res => {
    ...
    goToProgram(row, type) {
      ...
      if (!id) return
      this.$http
        .get(`${this.$dddUrl}/service/project/auth/devlopment?projectId=${id}`)
        .then(res => {
          this.$http
            .get(
              `${this.$dddUrl}/service/datasource/auth/refresh?projectId=${id}`
            )
            .then(res => {})
          const pos = location.href.indexOf('#/')
          const baseUrl = location.href.slice(0, pos + 2)
          window.open(
            `${baseUrl}sql_editor?projectId=${id}&fileId=${row.codeDetailId}`
          )
        })
        .catch(err => {
    ...
    checkKinship(row) {
      ...
      if (row.projectName) {
        this.$http
          .get(
            `${this.$dddUrl}/service/code/file/${row.codeDetailId}?projectId=${row.projectId}`
          )
          .then(res => {
        ...
        //  dam血缘导入
        this.$http
          .get(`${this.$damUrl}/service/lineage/${row.lineageId}`)
          .then(res => {
            let data = {
              rawData: res.data,
    ...
    getModelData() {
      if (!this.information.categoryId) return
      const url = `${this.$meta_url}/models/ddm/systems/${this.information.categoryId}/models`
      this.$http
        .get(url)
        .then(res => {
    ...
    getEnvironmentData() {
      ...
      const url = `${this.$meta_url}/models/category/${modelCategoryId}`
      this.$http
        .get(url)
        .then(res => {
    ...
    getModelTable() {
      this.modelTable = []
      this.loading = true
      const url = `${this.$meta_url}/entities/searchMetadata`
      this.$http
        .post(url, {
          currentPage: 1,
    ...
    init(container, graph) {
      ...
      this.graph.on('edge:click', e => {
        ...
        nodesAll.forEach(item => {
          ...
          if (
            item.id.toString() === from?.toString() ||
            item.id.toString() === edge.cell?.toString()
          ) {
            this.fromText = text
            this.fromId = item.id
          }
          if (
            item.id.toString() === to?.toString() ||
            item.id.toString() === edge.cell?.toString()
          ) {
            this.toText = text
            this.toId = item.id
        ...
        if (to.indexOf('c') !== -1 && from.indexOf('c') !== -1) {
          ...
          this.blood = false
          this.iface = true
          this.activeName = 'modelEdgeClick'
    ...
    getTableLineage(from, to) {
      ...
      let requestUrl = this.$meta_url + '/lineage/relationships?catId=' + from
      if (to) {
    ...
    getCall(from, to) {
      from = isNaN(from) ? from.toString().substring(1) : from
      to = isNaN(to) ? to.toString().substring(1) : to
      const requestUrl = this.$meta_url + '/systemcall/search'
      const requestBody = {
        pageSize: 500,
        currentPage: 0,
    ...
    getSystemCall() {
      this.$http
        .get(`${this.$meta_url}/systemcall/overview`)
        .then(res => {
          res.data.forEach(item => {
    ...
    getOverview() {
      this.$http
        .get(`${this.$meta_url}/lineage/overview`)
        .then(res => {
          let data = res.data
          Object.keys(data.steps).forEach(item => {
            ...
            data.steps[item].sourceId =
              data.steps[line.sourceStepId].properties.modelCategoryId
            data.steps[item].targetId =
              data.steps[line.targetStepId].properties.modelCategoryId
          })
          ...
          this.allDataSource.forEach(async item => {
            let haveLine = haveLineList.filter(
              v => Number(v.sourceId) === item.categoryId
            )
            if (haveLine.length === 1) {
              ...
              haveLine.forEach((v, i) => {
                let obj = this.allDataSource.find(
                  k => k.categoryId === v.targetId
                )
                obj && (obj.frequency = i)
            ...
            if (!haveLine && !haveLine.length) {
              haveLine = haveLineList.find(
                v => Number(v.targetId) === item.categoryId
              )
            }
            !item.targetId &&
              (item.targetId = haveLine ? haveLine.targetId : '')
            !item.sourceId &&
              (item.sourceId = haveLine ? haveLine.sourceId : '')
          })
          !this.origin && this.addNode()
          this.origin && this.filterLinAndSystem()
        })
        .catch(e => {
          this.$showFailure(e)
        })
    ...
    filterLinAndSystem() {
      ...
      console.log(ary, 'ary', this.system.categoryId)
      let obj = this.allDataSource.find(
        item => String(item.categoryId) === String(this.system.categoryId)
      )
      this.systemData.push(obj)
      this.pushSystemLine(ary, this.system.categoryId)

      console.log(this.systemLine, this.systemData, 'systemLine')
      this.allDataSource = this.systemData
      this.addNode(this.systemLine)
    },
    pushSystemLine(ary, id) {
      id = String(id)
      ary.forEach(item => {
        let obj = this.systemLine.find(
          i => item.sourceId === i.sourceId && item.targetId === i.targetId
        )
        let source = this.allDataSource.find(i => String(i.categoryId) === id)

        let systemDataObj = this.systemData.find(
          i => source && source.categoryId === i.categoryId
        )
        !systemDataObj && source && this.systemData.push(source)

        if (String(item.sourceId) === id && !obj) {
          this.systemLine.push(item)
          this.pushSystemLine(ary, item.targetId)
        }
        obj = this.systemLine.find(
          i => item.sourceId === i.sourceId && item.targetId === i.targetId
        )
        if (String(item.targetId) === id && !obj) {
          this.systemLine.push(item)
          this.pushSystemLine(ary, item.sourceId)
        }
      })
    },
    // 添加节点
    addNode(systemLine) {
      let ary = systemLine || [...this.lineList, ...this.systemCall]
      let x = []
      let y = []
      ary.forEach(item => {
        x = this.allDataSource.filter(v => v.x).map(v => v.x)
        y = this.allDataSource.filter(v => v.y).map(v => v.y)
        let source = this.allDataSource.find(
          v => v.categoryId === Number(item.sourceId)
        )
        let target = this.allDataSource.find(
          v => v.categoryId === Number(item.targetId)
        )
        ...
        if (!source.x) {
          ...
          if (source.categoryId !== target.categoryId && !target.x) {
            ...
            target.y = !isNaN(target.frequency)
              ? source.y + 170 * (target.frequency - source.allTable / 2 + 1)
              : source.y
          ...
          target.y = !isNaN(target.frequency)
            ...
            : source.y
          let yMax =
            Math.max(...source.child) > Math.max(...y)
              ? Math.max(...source.child)
              : Math.max(...y)
      ...
      x = this.allDataSource.filter(v => v.x).map(v => v.x).length
        ? this.allDataSource.filter(v => v.x).map(v => v.x)
        : [228]
      y = this.allDataSource.filter(v => v.y).map(v => v.y).length
        ? this.allDataSource.filter(v => v.y).map(v => v.y)
        : [0]
      this.allDataSource
        .filter(v => !v.x)
        .forEach((v, i) => {
          if (i > 4) {
            ...
            v.x = x.length
              ? Math.min(...x) +
                228 * i -
                (Math.min(...x) + 228 * (5 * Math.floor(i / 5) - 1))
              : 228 * i - (Math.min(...x) + 228 * (5 * Math.floor(i / 5) - 1))
          } else {
            v.x = x.length ? Math.min(...x) + 228 * i : 228 * i
          }
          v.y = y.length
            ? Math.max(...y) + 170 + 170 * Math.floor(i / 5)
            : 170 + 170 * Math.floor(i / 5)
          this.addItemNode(v)
        })
      this.addEdge(systemLine)
      // this.graph.centerContent()
      /* let nodes = this.graph.getNodes()
      let index = Math.floor(nodes.length / 2)
      let position = nodes[index].position()
      this.graph.centerPoint(position.x + 200, position.y) */
      let yAry = this.allDataSource
        .filter(v => v.y)
        .map(v => v.y)
        .sort((a, b) => a - b)
    ...
    addItemNode(item) {
      ...
      node.on('change:data', res => {
        // console.log(res, 'res')
        // 点击系统名称 双击
        if (res.current.showDetail && res.current.db && !this.origin) {
          this.itemEdges = []
          this.tableSystemList = []
          this.nodes = []
          this.edges = []
        ...
        if (res.current.showDetail) {
          ...
          // this.orientation()
          this.keyWords = ''
          this.information = this.allDataSource.find(
            v => v.categoryId === res.current.id
          )
          res.current.type === 'category' &&
            (this.information = this.allDataSource.find(
              v => v.categoryId === Number(res.current.id.slice(1))
            ))
          res.current.type === 'model' &&
            (this.modelId = res.current.id.slice(1))
    ...
    // 添加表单节点
    addTableNode() {
      this.tableSystemList = this.nodes.filter(
        v => v.type === 'category' || v.type === 'model'
      )
      ...
      this.tableSystemList.forEach(item => {
        ...
        item.y = null
        // item.drawEdge = false
        let haveLine = this.itemEdges.filter(
          v => v.sourceId === item.categoryId
        )
        if (haveLine.length === 1) {
          haveLine = haveLine[0]
        } else {
          ...
          // 查找每个终点，并加序号
          haveLine.forEach((v, i) => {
            let obj = this.tableSystemList.find(
              k => k.categoryId === v.targetId
            )
            obj.frequency = i
            if (v.arrows) {
    ...
    setNode(item, node) {
      ...
      if (!source.x) {
        ...
        let maxY = y.length ? yMax : ''
        target.x =
          target.categoryId.toString().indexOf('c') !== -1
            ? source.x
            : source.x + 228
        target.y = !isNaN(target.frequency)
          ? source.y + 170 * (target.frequency - source.allTable / 2 + 1)
      ...
      item.drawEdge = true */
        ...
        if (target.x && target.y === source.y) {
          ...
          if (Math.abs(target.x - source.x) > 228) {
            vertices = [
              { x: source.x + 60, y: source.y - 24 - 21 * x },
              { x: target.x + 60, y: target.y - 24 - 21 * x },
            ]
            this.graph.addEdge({
              source: item.sourceId,
              target: item.targetId,
        ...
        if (target.x === source.x) {
          ...
          if (target.y - source.y === -170) {
            ...
            vertices = [
              { x: source.x + 128 + 21 * i, y: source.y + 60 },
              { x: target.x + 128 + 21 * i, y: target.y + 60 },
            ]
          }
    ...
    addItemEdge(item) {
      ...
      this.itemEdges.forEach(item => {
        let source = this.tableSystemList.find(
          v => v.categoryId === item.sourceId
        )
        let target = this.tableSystemList.find(
          v => v.categoryId === item.targetId
        )
        ...
        if (
          ...
          if (source.x === target.x) {
            ...
            vertices = [
              { x: source.x - 30, y: source.y + 60 },
              { x: target.x - 30, y: target.y + 60 },
            ]
            router = {
              name: 'manhattan',
              args: {
          ...
          this.graph.addEdge({
            ...
            attrs: {
              line: {
                sourceMarker: !item.arrows
                  ? null
                  : {
                      // args,
                      name: 'block',
                    },
                targetMarker: !item.arrows
                  ? null
                  : {
                      // args,
                      name: 'block',
        ...
        if (target.x && target.y === source.y) {
          ...
          if (Math.abs(target.x - source.x) > 228) {
            vertices = [
              { x: source.x + 60, y: source.y - 24 - 10 * x },
              { x: target.x + 60, y: target.y - 24 - 10 * x },
            ]
            this.graph.addEdge({
              source: item.sourceId,
              target: item.targetId,
        ...
        if (target.x === source.x) {
          ...
          if (target.y - source.y === -170) {
            ...
            port2 = 2
            vertices = [
              { x: source.x + 188, y: source.y + 60 },
              { x: target.x + 188, y: target.y + 60 },
            ]
          }
          this.graph.addEdge({
            source: { cell: item.sourceId, port: port1 },
        ...
        .then(res => {
          ...
          if (Object.keys(endpoints).length === 0) {
            this.nodes.push({
              ...
              id: 'c' + catId,
              categoryName:
                obj.categoryName + '(' + obj.categoryAbbreviation + ')',
              label: obj.categoryName + '(' + obj.categoryAbbreviation + ')',
              detail: obj,
              type: 'category',
              group: 0,
            })
          ...
          for (const k in endpoints) {
            ...
            if (!this.categoryMap.hasOwnProperty(v.catId)) {
              ...
              this.nodes.push({
                ...
                id: 'c' + v.catId,
                label: v.cat + '(' + obj.categoryAbbreviation + ')',
                categoryName: v.cat + '(' + obj.categoryAbbreviation + ')',
                detail: v,
                type: 'category',
                group: i,
              })
          ...
          relationships.forEach(r => {
            ...
            let fromItem = this.nodes.find(
              item => item.id === 'm' + endpoints[r.first]?.modelId
            )
            let toItem = this.nodes.find(
              item => item.id === 'm' + endpoints[r.second]?.modelId
            )
            let hasEdge =
              fromItem &&
              toItem &&
              this.itemEdges.find(
                item =>
                  item.sourceId === fromItem.id && item.targetId === toItem.id
              )
    ...
    findSystemCall(from, to, callback) {
      ...
      if (to) {
        ...
        this.$http
          ...
          .then(res => {
            this.call = []
            this.call = this.call.concat(res.data.content)
    ...
    async getDataBySearch() {
      ...
      if (!this.name) {
        ...
        this.modelTable = []
        this.systemSearList = this.allDataSource.filter(
          item =>
...
.rightCom {
  ...
  /*width: 454px;*/
  /*box-shadow: -2px 0px 4px 0px rgba(47, 68, 91, 0.08);*/
  .comBox {
    width: 100%;
    height: 100%;
    /*background: #fff;*/
...
.nav {
  ...
  .nav-li {
    ...
    &.nav-liActive {
      background: rgba(24, 127, 255, 0.1);
      p {
        color: #187fff;
      }
      .nav-li-icon {
