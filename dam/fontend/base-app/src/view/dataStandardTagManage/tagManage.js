...
export default {
  ...
  data() {
    ...
    return {
      ...
      addGroupValue: '',
      currentUsageType: '',
      currentUsageObj: null,
      typeIdMap: {
        datablau_metadata: [
          80000004, 82800002, 82800008, 80000005, 80010118, 80010119, 82800024,
        ],
        datablau_domain: [80010066, 82800003, 80010098],
      },
      pageSize: 20,
      currentPage: 1,
      total: 0,
  ...
  methods: {
    ...
    handleTableData(val) {
      if (this.details.tagId) {
        this.$http
          .post(
            `${this.$url}/tags/getTagUsages?tagId=${this.details.tagId}`,
            this.typeIdMap[val]
          )
          .then(res => {
            if (Array.isArray(res.data) && res.data.length) {
              this.getShowTableData(res.data)
            } else {
              this.tableData = []
            }
          })
    ...
    changeUsage(val) {
      this.currentUsageObj = this.usageList.filter(u => {
        return u.appName === val
      })[0]
    },
    getTagUsage() {
      this.$http
        .post(this.$url + '/usage/getRegisteredUsageByType?type=tag')
        .then(res => {
          this.usageList = res.data
          this.currentUsageObj = res.data[0]
          this.currentUsageType = res.data[0]?.appName
        })
        .catch(e => {
          this.$showFailure(e)
          this.usageList = []
    ...
    getShowTableData(val) {
      if (this.details.tagId) {
        let param = {
          tagId: `${this.details.tagId}`,
          typeIds: this.typeIdMap[val],
          pageSize: this.pageSize,
          currentPage: this.currentPage,
        }
        if (!param) return
        if (!this.currentUsageObj) return
        this.$http
          .post(this.currentUsageObj.url, param)
          .then(res => {
            const result = res.data
            this.total = result.totalItems
            if (this.currentUsageType === 'datablau_metadata') {
              let logicalInfoArr = []
            ...
            if (result.content && Array.isArray(result.content)) {
              result.content.forEach(item => {
                ...
                if (item.typeName) {
                  ...
                  if (this.currentUsageType === 'datablau_metadata') {
                    obj.name = item.object.tableName || ''
                    obj.path = `${item.object.modelName}/${item.object.tableName}`
                    obj.objectId = item.object.tableObjectId
                    obj.type = this.typeNameMap[item.typeId]
                    obj.logical = this.logicalInfoObj[item.typeId]
                  } else if (this.currentUsageType === 'datablau_domain') {
                    obj.name =
                      `${item.typeId}` === '80010098'
                        ? item.object.name
                        : item.object.chineseName || ''
                    obj.path =
                      `${item.typeId}` === '80010098'
                        ? `${item.object.datasetName}`
                        : item.object.pathStr || ''
                    obj.objectId =
                      `${item.typeId}` === '80010098'
                        ? item.object.code
                        : item.object.domainId
                    obj.type = this.typeNameMap[item.typeId]
                  }
    ...
    handleSizeChange(val) {
      this.pageSize = val
      this.currentPage = 1
      this.getShowTableData(this.currentUsageType)
    },
      ...
      if (dropNode && dropNode.level && dropNode.level === 1) {
        dropNode.parent.childNodes.forEach((item, index) => {
          const promise = this.$http.post(
            this.$url +
              `/tags/group/reorder?id=${item.data.id}&newOrder=${index}`
          )
        ...
        dropNode &&
          dropNode.parent.childNodes.forEach((item, index) => {
            const promise = this.$http.post(
              this.$url +
                `/tags/category/reorder?categoryId=${item.data.tagId}&newOrder=${index}`
            )
            promises.push(promise)
          })
      }
    ...
    deleteSingleTag() {
      if (this.editTag.group) {
        ...
        this.$DatablauCofirm(
          ...
          .then(res => {
            this.$http
              .post(this.$url + '/tags/deleteGroup?groupId=' + this.editTag.id)
              .then(res => {
                this.keyword = ''
        ...
        this.$DatablauCofirm(
          ...
          .then(res => {
            this.$http
              .post(this.$url + '/tags/deleteTag?tagId=' + tag.tagId)
              .then(res => {
                this.keyword = ''
                this.$datablauMessage.success('删除成功')
              })
        ...
        .then(() => {
          ...
          const deleteTag = id => {
            return new Promise((resolve, reject) => {
              this.$http
                .post(this.$url + '/tags/deleteTag?tagId=' + id)
                .then(res => {
                  this.keyword = ''
          ...
          const deleteGroup = id => {
            return new Promise((resolve, reject) => {
              this.$http
                .post(this.$url + '/tags/deleteGroup?groupId=' + id)
                .then(res => {
                  this.keyword = ''
                  resolve()
                })
    ...
    conEdi() {
      ...
      if (this.isEdit) {
        if (this.addType === 'group') {
          ...
          this.$http
            .post(this.$url + '/tags/createTagGroup', requestBody)
            .then(res => {
              this.keyword = ''
          ...
          url = this.$url + '/tags/updateTag/?tagId=' + this.editTag.tagId
          data = this.editTag
    ...
    getShowData(para) {
      return new Promise((resolve, reject) => {
        ...
        if (this.details.tagId) {
          this.$http
            .post(
              `${this.$url}/tags/getTagUsages/?tagId=${this.details.tagId}&currentPage=${p}&pageSize=${s}`
            )
            .then(res => {
              const data = res.data
    ...
    dataOptionsFunction(data) {
      const sensitive =
        data.builtInCode === 'DATA_SENSITIVE' ||
        data.builtInCode === 'SENSITIVE2' ||
        data.builtInCode === 'SENSITIVE1'
      const options = []
      let label = ''
      if (data.name) {
        label =
      ...
      if (data.category && this.$auth.BASE_TAG_ADD) {
        options.push({
          icon: 'el-icon-plus',
          label: this.$t('meta.tagManage.tag'),
          disabled: sensitive,
          text: sensitive ? '该标签组暂不支持新增' : '',
          tooltip: sensitive,
          callback: () => {
            this.editTag = {}
            this.initTag(this.editTag, data)
      ...
      if (this.$auth.BASE_TAG_EDIT && !this.isDataRanking) {
        options.push({
          icon: 'el-icon-edit',
    ...
    renderContent(h, { node, data, store }) {
      ...
      const getMoreBtn = () => {
        if (
          this.$auth.BASE_TAG_ADD ||
          this.$auth.BASE_TAG_EDIT ||
          this.$auth.BASE_TAG_DELETE
        ) {
          return h(
            'span',
    ...
    getTagGroupList(callback) {
      this.$http
        .post(this.$url + '/tags/getTagGroup')
        .then(res => {
          let data = res.data
