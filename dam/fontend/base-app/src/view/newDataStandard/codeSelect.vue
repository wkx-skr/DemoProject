<template>
  <div class="en-list-box">
    <datablau-dialog
      :title="$t('domain.domain.chooseCode')"
    ...
    >
      <div style="position: relative; height: 400px">
        ...
        >
          <el-form-item :label="$t('domain.common.keyWord')">
            <datablau-input
              clearable
              maxlength="100"
              style="width: 15vw"
              :placeholder="$t('domain.domain.codeSearchPlaceholder')"
              type="text"
              size="small"
...
export default {
  ...
  mounted() {
    this.$bus.$on('callDomainCodeSelector', typeId => {
      this.visible = true
      if (typeId > 2) {
        this.typeId = typeId
      }
