  ...
  methods: {
    ...
    initTypeTab() {
      if (
        ...
        this.$http
          /* .get(
            this.$quality_url +
              `/dashboard/tech/rule/class/count?start=${this.dateRange1.startTime}&end=${this.dateRange1.endTime}`
          ) */
          .get(
            this.$quality_url +
              `/dashboard/tech/rule/class/count/v2?start=${this.dateRange1.startTime}&end=${this.dateRange1.endTime}`
          )
          .then(res => {
            ...
            this.$bus.$emit('typeData', tempJson, 1, legend)
          })
      } else {
        this.$message.warning(this.$t('quality.page.ruleReport.message.name5'))
      }
    },
    initSystemTab() {
      this.systemValueArr = []
      ...
      if (
        ...
        this.$http
          .get(
            this.$quality_url +
              `/dashboard/tech/rule/category/count?start=${this.dateRange2.startTime}&end=${this.dateRange2.endTime}`
          )
          .then(res => {
            var count = 0
    ...
    initCurveTab() {
      ...
      if (
        ...
        this.$http
          .get(
            this.$quality_url +
              `/dashboard/tech/rule/date/count?start=${this.dateRange3.startTime}&end=${this.dateRange3.endTime}`
          )
          .then(res => {
            res.data.forEach(item => {
              ...
              this.curveDelArr.push(item.delete)
            })
            this.$bus.$emit(
              'curveData',
              this.curveNameArr,
    ...
    changeTypeStart() {
      if (
        ...
        this.$message.warning(this.$t('quality.page.ruleReport.message.name2'))
        this.dateRange1.startTime = null
      } else if (
        moment(this.dateRange1.endTime).valueOf() -
          moment(this.dateRange1.startTime).valueOf() >
        ...
        this.$message.warning(this.$t('quality.page.ruleReport.message.name1'))
        this.dateRange1.startTime = null
      }
    },
    changeTypeEnd() {
      if (this.dateRange1.startTime === null) {
        this.$message.warning(this.$t('quality.page.ruleReport.message.name3'))
        this.dateRange1.endTime = null
      } else if (
        moment(this.dateRange1.startTime).valueOf() >
        moment(this.dateRange1.endTime).valueOf()
      ) {
        this.$message.warning(this.$t('quality.page.ruleReport.message.name4'))
        this.dateRange1.endTime = null
      } else if (
        moment(this.dateRange1.endTime).valueOf() -
          moment(this.dateRange1.startTime).valueOf() >
    ...
    downLoadType() {
      const url =
        this.$quality_url +
        `/dashboard/tech/rule/count/download/v2?start=${this.dateRange1.startTime}&end=${this.dateRange1.endTime}`
      this.$downloadFilePost(url)
    },
    downLoadSystem() {
      const url =
        this.$quality_url +
        `/dashboard/tech/rule/count/download?start=${this.dateRange2.startTime}&end=${this.dateRange2.endTime}&type=2`
      this.$downloadFile(url)
    },
    downLoadCurve() {
      const url =
        this.$quality_url +
        `/dashboard/tech/rule/count/download?start=${this.dateRange3.startTime}&end=${this.dateRange3.endTime}&type=3`
      this.$downloadFile(url)
    },
  },
}
