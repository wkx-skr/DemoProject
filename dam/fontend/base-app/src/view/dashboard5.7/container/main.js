...
export default {
  ...
  data: function () {
    const componentNameMap = {
      dataQuality: {
        allQuestionsNum: '数据质量-全部问题数',
        problemRecordNum: '数据质量-全部问题记录数',
        overdueIssuesNum: '数据质量-逾期问题数',
        overdueProblemRecordNum: '数据质量-逾期问题记录数',
        customModule: '数据质量-自定义模块区域',
        problemArea: '数据质量-问题区域分布',
        problemSystem: '数据质量-问题系统分布',
        importance: '数据质量-重要程度',
        ruleSource: '数据质量-业务类型',
        ruleType: '数据质量-规则类型',
        hisProblemsOverdue: '数据质量-问题数据逾期与即将逾期分布',
        hisOverdueStatistics: '数据质量-历史逾期统计',
        overdueDistribution: '数据质量-逾期分布',
        problemSummary: '数据质量-问题列表汇总',
      },
    }
    return {
      ...
      modelValueSingle: '',
      modelValueName: '请选择',
      // options: [{ label: '资产目录管理', value: '0' }],
      //
    }
  },
  mounted() {
    console.log(this.dashboardProperties, 'dashboardProperties')
    this.initRequestBodyBefore()
    this.initSystem()
    $(window).resize(() => {
      this.calculateWidthBase('resize')
    })
    setTimeout(() => {
  ...
  methods: {
    ...
    removeComponent(component) {
      let componentName = ''
      if (this.componentNameMap[this.dashboardProperties.dashboardName]) {
        componentName =
          this.componentNameMap[this.dashboardProperties.dashboardName][
            component.component
          ]
      }
      this.$DatablauCofirm(
        `确定要移除组件"${componentName || component.title}"吗？`
      )
        .then(() => {
    ...
    addComponent() {
      const getH5List = new Promise(resolve => {
        this.$http
          .post(this.$url + '/widget/getWidgetConfig?id=dashboard-h5-list')
          .then(res => {
            if (res.data.content) {
              resolve(
    ...
    getSettingFromDatabase() {
      return new Promise((resolve, reject) => {
        this.$http
          .post(
            this.$url +
              `/widget/getWidgetConfig?id=dashboard-${this.dashboardProperties.dashboardName}`
          )
          .then(res => {
    ...
    resetAll() {
      this.$confirm(
        this.$t('common.dashboard.resetDashboardTips'),
        this.$t('common.dashboard.resetDashboard')
      )
        .then(() => {
          this.$http
        ...
        .catch(() => {
          this.$message.info(this.$t('common.dashboard.cancelO'))
        })
    },
    handleSave() {
      this.editMode = false
    ...
    afterPrepareComponents(items, callback) {
      ...
      items.forEach(component => {
        ...
        let hasLicense = true
        if (
          (name == 'domain' && !this.$featureMap.FE_DOMAIN) ||
          (name == 'rule' && !this.$featureMap.FE_QUALITY)
        ) {
          hasLicense = false
        }
        if (hasLicense) {
    ...
    prepareData() {
      ...
      urlToNameMap.forEach((componentNames, url) => {
        let prefix = this.$meta_url
        if (url.includes('quality')) {
          prefix = this.$quality_url
        }
        this.$http
          .get(prefix + url)
          .then(res => {
