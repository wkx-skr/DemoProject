<template>
  <div>
    <el-dialog
      :title="$t('common.dashboard.preView')"
      width="70%"
      append-to-body
    ...
    <datablau-form
      ...
      :label-width="$i18n.locale === 'zh' ? '4em' : '5em'"
      v-loading="isLoading"
      ref="form"
      :model="form"
      :rules="rules"
    >
      <el-form-item :label="$t('common.dashboard.name')" prop="name">
...
export default {
  ...
  methods: {
    ...
    getDetail() {
      this.$http
        .post(this.$url + `/widget/getWidgetConfig?id=${this.currentData.id}`)
        .then(res => {
          if (res.data.content) {
