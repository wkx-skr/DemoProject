...
export default {
  ...
  methods: {
    refreshCatalogCount() {
      const url = `${this.$meta_url}/service/entities/statistics`
    ...
    getDetailData() {
      this.$http
        .get(this.$quality_url + '/dashboard/quality/rule')
        .then(res => {
    ...
    getLayout() {
      this.$http
        .post(this.$url + `/widget/getWidgetConfig?id=${this.dashboardName}`)
        .then(res => {
