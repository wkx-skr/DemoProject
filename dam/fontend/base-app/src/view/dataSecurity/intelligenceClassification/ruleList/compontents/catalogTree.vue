<template>
  <datablau-dialog
    ...
    :width="dialogType === 'item' ? '1100px' : '500px'"
    :height="dialogType === 'item' ? 560 : 420"
    :title="dialogType === 'item' ? '选择信息项' : '选择安全分类'"
  ...
  >
    <template v-if="dialogType === 'security'">
      ...
      <div class="tree-box" :style="{ top: selectBox + 'px' }">
        <datablau-select
          filterable
          clearable
          class="filter-input"
    ...
    <template v-if="dialogType === 'item'">
      <div style="float: left; width: 240px; height: 440px; overflow: scroll">
        <datablau-tree
...
export default {
  ...
  data() {
    return {
      ...
      treeData: [],
      selectData: [], // 当前所有已选择的安全分类目录（只有父目录，不包含子目录）
  ...
  methods: {
    ...
    setCheckedTree() {
      this.$refs.tree.$refs.tree.setCheckedKeys(this.realIds)
    },
    ...
    async getTree(id, resolve, isLazy = true) {
      ...
      let treeList =
        (this.dialogType === 'item'
          ? data.catalogVos
          : data.DataSecurityStructureVo.catalogVos) || []
      ...
      if (isLazy) {
        ...
        this.$nextTick(() => {
          this.isResolve = true
          if (this.dialogType === 'item' && !this.currentItemCatalog.id) {
            this.currentItemCatalog = treeList[0]
            this.$refs.itemTree.$refs.tree.setCurrentKey(treeList[0].id)
          }
        })
      }
    },
    setHalfSelect(ids) {
      ids.map(item => {
        const node = this.$refs.tree.$refs.tree.getNode(item)
        if (node) {
          setHalf(node.parent)
        }
      })
    ...
    delChildren(id) {
      const that = this
      const node = this.$refs.tree.$refs.tree.getNode(id)
      setSelected(node)
      function setSelected(data) {
        if (data.childNodes.length > 0) {
          data.childNodes.map(item => {
    ...
    async getItemTableData(defaultParam) {
      ...
      const param = {
        catalogId: this.currentItemCatalog.id,
        data: {
      ...
      try {
        ...
        if (
          queryRes.status === 200 &&
          queryRes.data &&
          param.catalogId === this.currentItemCatalog.id
        ) {
    ...
    sortChange(data) {
      ...
      this.getItemTableData({ id: this.currentItemCatalog.id, data: {} })
    },
    handlePageChange() {},
    handleSizeChange() {},
    ...
    async beforeChange(data) {
      ...
      if (node.parent.checked) {
        this.$refs.tree.$refs.tree.setChecked(data.id, true)
        this.$datablauMessage({
          message: '父级节点已选择，子节点无法操作',
          type: 'warning',
        })
        return
      } else {
  ...
  watch: {
    ...
    selectData: {
      handler(val) {
        this.$nextTick(() => {
          this.searchBoxH = $(this.$refs.selectBox).height()
        })
      },
      deep: true,
      immediate: true,
      ...
      if (val && this.dialogType !== 'item') {
        this.$nextTick(async () => {
          if (this.expandId) {
            ...
            const node = this.$refs.tree.$refs.tree.getNode(this.expandId)
            if (node.checked) {
    ...
    currentItemCatalog: {
      handler(node) {
        if (node.id) {
          ...
          this.$nextTick(() => {
            this.$refs.itemTree.$refs.tree.setCurrentKey(node.id)
          })
        }
      },
      deep: true,
