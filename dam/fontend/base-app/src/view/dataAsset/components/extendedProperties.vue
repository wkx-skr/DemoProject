          ...
          <div class="iconbox" v-if="disabled">
            ...
            <span v-show="!item.algoInuse">
              <i
                ...
                v-show="!publishStatus && !underReviewStatus"
                @click="delWeight(index, item)"
              ></i>
            </span>

            <!--              v-show="currentCompletion.length - 1 === index"-->
            <span v-show="!publishStatus && !underReviewStatus">
              <i
...
export default {
  ...
  watch: {
    flag: {
      ...
      handler(val) {
        ...
        this.underReviewStatus = val.catalogType.underReviewStatus
        this.udpsList = [
          {
            propName: '',
            type: '',
        ...
        if (val.catalogType.udps) {
          this.udpsList = _.cloneDeep(
            val.catalogType.udps.filter(item => {
              return !item.delete
            })
          )
  ...
  methods: {
    ...
    primary() {
      ...
      JSON.stringify(this.oldudp) !== JSON.stringify(ary) && (json.udps = ary)
    ...
    delWeight(index, item) {
      ...
      this.catalogType.inUse &&
        this.$DatablauCofirm(text, this.$t('assets.generalSettings.hint'), {
          ...
          .catch(() => {})
      !this.catalogType.inUse && this.splic(index)
    },
    splic(index) {
      // console.log(this.udpsList, 'this.udpsList')
      let delitem = this.udpsList.splice(index, 1)
      // console.log(delitem, this.udpsList, index)
