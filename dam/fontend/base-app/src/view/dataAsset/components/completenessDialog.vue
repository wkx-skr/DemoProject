  ...
  <div>
    ...
    >
      <p class="tilText" v-show="muluText">
        ...
        {{ muluText || $t('assets.generalSettings.muluText') }}
      </p>
      <div class="content" v-loading="loading">
        ...
        <div class="baseline">
          ...
          <el-tooltip effect="dark" placement="right">
            <i class="iconfont icon-tips"></i>
            <div slot="content">
              {{ $t('assets.generalSettings.fineBaseline') }}
              <br />
              {{ $t('assets.generalSettings.fineBaseline2') }}
            </div>
          </el-tooltip>
        </div>
        <div
...
export default {
  ...
  data() {
    return {
      ...
      formula: this.$t('assets.generalSettings.calculationFormula'),
      textMula: this.$t('assets.generalSettings.muluText'),
      property: [
  ...
  methods: {
    ...
    primary() {
      ...
      if (!flag) {
        ...
        this.muluText = this.$t('assets.generalSettings.notEmpty')
        return
      } else if (w < 100 && w !== 0) {
        this.muluTextFlag = true
      ...
      let json = {
        ...
        publishPercent: this.publishPercent > 100 ? 60 : this.publishPercent,
      }
      ...
      if (JSON.stringify(this.oldCurrent) !== JSON.stringify(ary)) {
        ...
        // (text = this.$t('assets.generalSettings.existingAssetDirectory'))
      }
    ...
    getTextMule() {
      this.textMula = ''
      this.formula = this.$t('assets.generalSettings.calculationFormula')
      this.currentCompletion.forEach(item => {
        item.attribute &&
          (this.textMula += `${this.propertyCorrespond(item.attribute)}*${
            item.weight
          }%+`)
        this.formula = this.$t('assets.generalSettings.formula')
      })
    ...
    getProperties(val) {
      let ary = []
      // this.loading = true
      HTTP.getPropertiesList(val)
        .then(res => {
          let udpDtoList = res.data.udpDtoList
          let cataUdp = this.catalogType.udpDtoList
          let list = [...udpDtoList]
          cataUdp && (list = cataUdp)
          list.forEach(item => {
            ary.push({
              value: item.propName,
          ...
          this.propertyNew = [...this.property, ...ary]
          // this.loading = false
        })
        .catch(e => {
          this.$showFailure(e)
    ...
    getAlgorithm(id) {
      ...
      HTTP.getAlgorithm(id)
        .then(res => {
          let data = res.data
          let ary = []
          data.staticProperties.forEach(item => {
          ...
          if (ary.length != 0) {
            this.currentCompletion = ary
            this.textMula = ''
            this.getTextMule()
          }
          this.oldCurrent = JSON.parse(JSON.stringify(this.currentCompletion))
          this.loading = false
        })
        .catch(e => {
          this.loading = false
  ...
  watch: {
    flag: {
      ...
      handler(val) {
        ...
        this.muluText = ''
        this.textMula = this.$t('assets.generalSettings.muluText')
