  ...
  <div class="settingTab">
    <div class="publicTypeBox alignItem">
      <datablau-detail-subtitle
        ...
        mb="0px"
      ></datablau-detail-subtitle>
      <datablau-radio v-model="currentNode.publicType" class="publicAcess">
        <el-radio
          label="SUB"
          v-model="currentNode.publicType"
          @click.native.prevent="changeRadio('SUB')"
        >
          {{ $t('assets.permissionSettings.allPublicVisit') }}
        </el-radio>
        <el-radio
          label="CURRENT"
          v-model="currentNode.publicType"
          @click.native.prevent="changeRadio('CURRENT')"
        >
          {{ $t('assets.permissionSettings.publicVisit') }}
        </el-radio>
      </datablau-radio>
      <!--      <datablau-checkbox-->
      <!--        :checkboxType="'single'"-->
    ...
    <div class="flex alignItem tabChange spaceBetween">
      ...
      <datablau-button type="important" class="tianjia" @click="handleClick">
        <!--        <i class="iconfont icon-tianjia addPermission"></i>-->
          ...
          >
            <template slot-scope="scope">
              <span v-if="item.prop === 'authType'" class="authTypeSel">
                ...
                <!--                &lt;!&ndash;                {{ authType[scope.row[item.prop]] }}&ndash;&gt;-->
                <!--                &lt;!&ndash;                :disabled="scope.row.delet"&ndash;&gt;-->
                <!--                <datablau-select-->
                <!--                  v-model="scope.row[item.prop]"-->
                <!--                  :placeholder="$t('assets.permissionSettings.selectAuthority')"-->
                <!--                  style="width: 120px; height: 30px"-->
                <!--                  :disabled="true"-->
                <!--                  popper-class="popperClass"-->
                <!--                  :popper-append-to-body="true"-->
                <!--                  @change="authTypeChang(scope.row)"-->
                <!--                >-->
                <!--                  <el-option-->
              ...
              <span v-else-if="item.prop === 'username'">
                {{ scope.row[item.prop] }}
                <img
                  src="/static/images/dataAssets/created.svg"
                  class="created"
                  alt=""
                  v-if="scope.row.creator"
                />
              </span>
              <!--              <span v-else-if="item.prop === 'fullUserName'">-->
              <!--                {{ scope.row[item.prop] }}-->
              <!--                {{ scope.row.creator && '(创建人)' }}-->
              <!--              </span>-->
              <!--              <span v-else-if="item.prop === 'operate'">-->
              <!--                &lt;!&ndash;         @click="delPremision(scope.row)"       { blue: !scope.row.delet, grey: scope.row.delet },&ndash;&gt;-->
              <!--                <i :class="['iconfont', 'icon-delete', 'grey']"></i>-->
              <!--              </span>-->
...
export default {
  ...
  data() {
    return {
      ...
      user: [
        ...
        // {
        //   prop: 'operate',
        //   label: this.$t('assets.permissionSettings.operate'),
        // },
      ],
      group: [
        {
  ...
  methods: {
    // 单选取消选中
    changeRadio(val) {
      this.currentNode.publicType =
        this.currentNode.publicType === val ? 'NONE' : val
      // this.$set(this.currentNode, 'publicType', this.currentNode.publicType)
      this.directoryPublic(this.currentNode.publicType)
      // console.log(this.currentNode.publicType, val, 'val')
    },
    // tab切换
    changeSubtab(val, idx) {
      ...
      if (val.value === 'user') {
        ...
        this.placeholder = this.$t('assets.permissionSettings.namePlaceholder')
    ...
    primary(selectedPerson) {
      ...
      selectedPerson.add.forEach(item => {
        // item.change &&
        //   item.authId &&
        //   this.updatePremision(item.authId, item.permission, 'chan')
        // if (!item.authId) {
        let obj = {}
        obj.authType = item.permission
        obj.name = item.name
    ...
    directoryPublic(val) {
      HTTP.directoryPublic({ id: this.currentNode.id, publicType: val })
        .then(res => {
          val === 'SUB' &&
            this.$message.success(
              this.$t('assets.permissionSettings.publicSucceed')
            )
          val === 'CURRENT' &&
            this.$message.success(this.$t('assets.permissionSettings.current'))
          val === 'NONE' &&
            this.$message.success(
              this.$t('assets.permissionSettings.cancelSucceed')
            )
