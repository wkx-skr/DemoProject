import setUdp from '@/view/dataIndex/indexManagement/dimensionDefinition/setUdp.vue'
...
export default {
  ...
  methods: {
    ...
    DeleteClick(row) {
      this.$DatablauCofirm(
        ...
        .then(() => {
          ...
          let url = '/domain/dimension/delete'
          this.$http
            .post(url, ids)
            .then(res => {
    ...
    handleSortChange(sortData) {
      this.orderIsAsc = sortData.order === 'ascending' ? true : false;
    ...
    exportIndex() {
      const url = `/domain/dimension/export`
      this.$downloadFilePost(url, this.prepareSearchCondition())
    },
    downloadTemplate() {
      const url = `/domain/dimension/downloadTemplate`
