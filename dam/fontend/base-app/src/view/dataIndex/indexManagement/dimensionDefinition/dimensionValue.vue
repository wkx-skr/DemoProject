<template>
  <div>
    <el-upload
      ...
      :action="'/domain/dimension/value/import?dimensionId=' + dataId"
...
export default {
  ...
  methods: {
    ...
    getValueUdp() {
      this.$http
        .post(
          `/domain/dimension/udp/get?category=${
    ...
    // 查询全部维度树
    getValueTree() {
      let url = `/domain/dimension/tree/get?id=${this.dataId}`
      this.$http
        .post(url)
        .then(res => {
    ...
    // 查询全部维度值
    getValue() {
      let url = `/domain/dimension/list/get?id=${this.dataId}`
      this.$http
        .post(url)
        .then(res => {
          if (res.data) {
    ...
    valueDelete(scope) {
      this.$DatablauCofirm(
        ...
        if (arr.length > 0) {
          ...
          let url = `/domain/dimension/value/delete`
    ...
    exportTemplate() {
      const url = `/domain/dimension/value/downloadTemplate?dimensionId=${this.dataId}`
      this.$downloadFilePost(url)
    },
    exportValue() {
      const url = `/domain/dimension/value/export?dimensionId=${this.dataId}`
      this.$downloadFilePost(url)
    },
    importValue() {
    ...
    submitValue() {
      this.$refs.valueForm.validate(valid => {
        if (!valid) {
          ...
          let url = `/domain/dimension/value/maintain?dimensionId=${this.dataId}`
      ...
      this.$refs.addValueForm.validate(valid => {
        if (!valid) {
          ...
          if (this.addValueForm.index !== undefined) {
            ...
            let url = `/domain/dimension/value/update`
