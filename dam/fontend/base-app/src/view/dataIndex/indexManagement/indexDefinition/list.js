...
export default {
  ...
  methods: {
    ...
    getTreeData() {
      this.$http
        .post(this.BASE + `/domains/tree/getTree`, {
    ...
    processCreateCategory() {
      let requestUrl = this.BASE + `/domains/folder/createFolder`
      if (this.category.id) {
        requestUrl =
          this.BASE + `/domains/folder/updateFolder?folder=${this.category.id}`
      }
    ...
    removeCategory() {
      this.$DatablauCofirm(this.$t('meta.report.delConfirm'), '')
        .then(() => {
          this.$http
            .post(`/domain/domains/folder/deleteFolder`, {
