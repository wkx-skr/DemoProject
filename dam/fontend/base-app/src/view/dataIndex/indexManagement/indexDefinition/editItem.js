...
import version from '@/view/newDataStandard/version.vue'
import knowledgeGraph from '@/view/dataProperty/meta/knowledgeGraph'
...
import consanguinityGraph from '@/view/dataProperty/meta/consanguinityGraph'
...
import ModifierSelector from '@/view/dataIndex/indexManagement/indexDefinition/component/ModifierSelector.vue'
import { ModifierCategory } from '@/view/dataIndex/modifier/entrance/Constant'
import {
  IndexPage,
  CategoryId,
} from '@/view/dataIndex/indexManagement/indexDefinition/entrance/Constant'
import relationDomainList from '@/view/newDataStandard/relationDomainList'
export default {
  components: {
    RedAlert,
    AnalyzeAndMonitor,
  ...
  methods: {
    ...
    selectOrganization(event) {
      ...
      this.blurFormItem('desOrgChoose')
    },
    selectOrganization1(event) {
      ...
      this.blurFormItem('desOrgChoose1')
    },
    blurFormItem(name) {
      this.$nextTick(() => {
        if (this.$refs[name] && this.$refs[name].DATA.blur) {
          this.$refs[name].DATA.blur()
        }
      })
    },
    getDataTypeList() {
    ...
    getPermission() {
      this.$http
        .post(
          `/domain/metricManagement/metricAuth/getPermission?metricId=${this.currentItem.domainId}`
        )
        .then(res => {
    ...
    processSubmit(requestBody, closeAfterSuccess) {
      ...
      if (this.viewMode && requestBody.state === 'A') {
        this.processUpdate(closeAfterSuccess)
      } else {
    ...
    processCreate(requestBody, closeAfterSuccess) {
      this.$http
        .post(
          requestBody.state === 'D'
            ? `/domain/domains/domain/updateDomain`
            : '/domain/domains/domain/createDomain',
          requestBody
        )
        .then(res => {
  ...
  computed: {
    ...
    pageName() {
      if (this.indexPage === IndexPage.Basic_and_Derive) {
