<template>
  <div>
    <div v-if="isMultiple">
      <datablau-tabs v-model="currentTab">
        <el-tab-pane name="before" label="修改前">
          <view-detail
            style="margin-top: 20px"
            v-if="decodeReady && requirementReady && udpReady"
            :form-data="formData"
        ...
        <el-tab-pane name="after" label="修改后">
          <view-detail
            style="margin-top: 20px"
            v-if="decodeReadyAfter && requirementReady && udpReady"
...
<script>
import ViewDetail from '@/view/dataIndex/indexManagement/indexDefinition/viewDetail.vue'
import _ from 'lodash'
import { IndexType } from '@/view/dataIndex/indexManagement/indexDefinition/class/Enum'
import HTTP from '@/http/main'
import { CategoryId } from '@/view/dataIndex/indexManagement/indexDefinition/entrance/Constant'
...
export default {
  ...
  computed: {
    isMultiple() {
      console.log(
        this.rawData.processType,
        this.$route.query.currentNav,
        '-=-=-='
      )
      return (
