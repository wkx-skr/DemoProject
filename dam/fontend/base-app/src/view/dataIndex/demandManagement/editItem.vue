...
export default {
  ...
  methods: {
    ...
    // 获取目录树
    getTreeData() {
      let url = `/domain/categories/tree?type=82800021`
      this.$http
        .post(url)
        .then(res => {
    ...
    // 获取目录列表
    getTreeList() {
      let url = `/domain/categories/get?type=82800021`
      this.$http
        .post(url)
        .then(res => {
    ...
    // 获取详情
    getFormData() {
      let url = `/domain/requirementmanagement/get?id=${this.id}`
      this.$http
        .post(url)
        .then(res => {
    ...
    getTableData() {
      if (this.id) {
        let url = '/domain/requirementmanagement/requirementdataobject/get/list'
    ...
    onSubmit() {
      this.$refs.form.validate(valid => {
        if (!valid) {
          ...
          let url = '/domain/requirementmanagement/update'
          let body = this.formData
          if (this.iscreate) {
            // 新建需求
            url = '/domain/requirementmanagement/create'
          ...
          this.$http
            ...
            .then(res => {
              ...
              let url = `/domain/requirementmanagement/requirementdataobject/maintain?requirementId=${requirementId}`
              let body = []
