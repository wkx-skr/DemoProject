<template>
  ...
  <div class="box">
    <div class="module-title">
      <p>{{ $t('quality.page.qualityDashboardNew.businessType') }}</p>
    </div>
    <div
      id="ruleSource"
...
export default {
  ...
  data() {
    return {
      myChart: null,
      bizTypeSelectOptionRatio: null,
    }
  },
  mounted() {
    this.$bus.$on('dashboardDataChange', data => {
      this.bizTypeSelectOptionRatio = data.mainCount.bizTypeSelectOptionRatio
      this.$nextTick(() => {
        this.dataInit()
      })
    })
  ...
  methods: {
    dataInit() {
      ...
      this.myChart = echarts.init(document.getElementById('ruleSource'))
      let chartData = []
      let dataName = []
      this.bizTypeSelectOptionRatio.forEach(element => {
        chartData.push({
      ...
      var option = {
        tooltip: {
          ...
          formatter: function (params) {
            let str =
              _this.$t('quality.page.qualityDashboardNew.businessType') +
              '<br/>'
        ...
        color: ['#38B48B', '#409EFF', '#7582E5', '#8DC78A'],
        series: [
          {
            name: this.$t('quality.page.qualityDashboardNew.businessType'),
            type: 'pie',
            radius: ['30%', '55%'],
