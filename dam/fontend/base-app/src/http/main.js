...
import ResetPasswordController from '../../../base-components/http/gateway/ResetPasswordController'
import UserDefinedPropertyController from '../../../base-components/http/baseController/UserDefinedPropertyController'
import GatewayMainController from '../../../base-components/http/gateway/GatewayMainController'
...
const currentProduct = window.localStorage.getItem('currentProduct')
  ? window.localStorage.getItem('currentProduct').toUpperCase()
  : 'DAM'
// const damEnabled =
...
const HTTP = {
  ...
  META_BASE: $meta_url + '/',
  USER_BASE: $user_url + '/',
  // META_BASE: +'/dam/service/',
  wBase: $workflowUrl + '/',
  STATIC_BASE, // 获取静态文件 url
  DDS: '/dds/',
}
window.wBase = $workflowUrl + '/'
...
export default {
  ...
  /* --------- 元数据浏览相关REST API -------- */
  getUdpByType({ typeId }) {
    // return UserDefinedPropertyController.getTypeUdps({ typeId: typeId })
    return this.$http.post(`${$meta_url}/udps/getUdpsOfType?typeId=${typeId}`)
  },
  getDbDataList(para) {
    ...
    return vThis.$http.post(
      HTTP.META_BASE + 'entities/searchMetadata',
      requestBody
    )
  },
  getTableCols({ objectId }) {
    return vThis.$http.get(HTTP.META_BASE + `entities/${objectId}/columns`)
  },
  /* --------- DDC搜索REST API -------- */
  ...
  getReportStatistics({ reportId }) {
    return vThis.$http.get(
      `${$meta_url}/service/dataReport/${reportId}/resourceStatistics`
    )
  },
  // 添加报表服务器(old)
  ...
  // 添加报表服务器(new)
  addReportDs(para) {
    const url = `${HTTP.META_BASE}models/re/report`
    return vThis.$http.post(url, para)
  },
  ...
  getDomainHistoryMetric(para) {
    return vThis.$http.post(
      `${vThis.$metric_url}domains/domain/getHistory`,
      para
    )
  },
  ...
  getDomainColumnMappingMetric(typeIds) {
    return vThis.$http.post(
      `${vThis.$metric_url}domains/column/mapping?categoryId=${typeIds}`
    )
  },
  ...
  domainHistoryCompareMetric(para) {
    return vThis.$http.post(
      `${vThis.$metric_url}domains/version/compareHistory?domainId=${para.domainId}&srcVersion=${para.srcVersion}&tagVersion=${para.tagVersion}`,
      para
    )
  },
  ...
  // 获取邮件模板列表
  getemailList() {
    return vThis.$http.get(`${USER_BASE_URL}mail/getscenelist`)
  },
  // 根据场景获取邮件模板
  getemail(sceneId) {
    return vThis.$http.get(
      `${USER_BASE_URL}mail/getmailtemplate?sceneId=${sceneId}`
    )
  },
  updateTemplate(para) {
    return vThis.$http.post(`${USER_BASE_URL}mail/updatetemplate`, para)
  },

  // 设置邮件是否启用
  setSceneenable(sceneId, enable) {
    return vThis.$http.post(
      `${USER_BASE_URL}mail/setsceneenable?sceneId=${sceneId}&enable=${enable}`
    )
  },
  ...
  getFolderListServiceNew(para) {
    return vThis.$http.post(`${vThis.$metric_url}domains/domain/getPage`, para)
  },
  getPublicPage(para) {
    return vThis.$http.post(
  ...
  getDomainUdpsService(para) {
    ...
    let param = {
      categoryId: para.categoryId || null,
      domainId: para.domainId || null,
    }
    return vThis.$http.post(url, param)
  },
  getDomainUdpsService2(para) {
    let url = para.standardCode
      ? `${vThis.$metric_url}standards/udp/getUdps`
      : `${vThis.$metric_url}domains/udp/getUdps`
    let param = {
  ...
  setDomainUdpsServiceMetric(
    ...
    let url = para.standardCode
      ? `${vThis.$metric_url}standards/udp/createUdps?clear=true&categoryId=${para.categoryTypeId}`
      : `${vThis.$metric_url}domains/udp/createUdps?clear=true&categoryId=${para.categoryTypeId}`
    return vThis.$http.post(url, para.requestBody)
  },
  ...
  getfindStateNew(para) {
    return vThis.$http.post(`${vThis.$metric_url}generate/find/state`, para)
  },
  ...
  getCodeDetailServiceMetric(codes) {
    return vThis.$http.post(
      `${vThis.$metric_url}domains/domain/getDomainByCodes`,
      codes
    )
  },
  applyMetric(param) {
    return vThis.$http.post(`${vThis.$metric_url}flow/domain/applyAuth`, param)
  },
  ...
  getReportLineage(para = {}) {
    return vThis.$http.get(
      `${$meta_url}/service/lineage/report/${para.objectId}/type`
    )
  },
  ...
  getQualityInfo(para) {
    return vThis.$http.get(
      `${$meta_url}/lineage/object/${para.objectId}/qualityInfo`
    )
  },
  /* --------- MainController -------- */
  ...
  addUserToGroups(param) {
    return ModelCategoryController.addUserToGroups({ requestBody: param })
  },
  ...
  resetPasswordByUniqueId(param) {
    return ResetPasswordController.resetPasswordByUniqueId(param)
  },
  /* --------- ModelController -------- */
  getModelTree(para = {}) {
    return vThis.$http.get(`${HTTP.META_BASE}models/modeltree`)
  },
  ...
  getDriverList(
    ...
    return vThis.$http.post(
      `${$url}/drivers/search?pageSize=${params.pageSize}&currentPage=${params.currentPage}&type=${params.type}`
      // `${$url}/drivers/getDriverByType?type=${params.type}`
    )
  },
  /* --------- 业务术语相关的REST API -------- */
  ...
  getTopUsers(para = {}) {
    return vThis.$http.get(
      `${$meta_url}/service/browse/query/topuser/${para.dataAmount}/${para.objectId}/${para.typeId}`
    )
  },
  /* --------- 日志管理 -------- */
  /* --------- 机构和用户组的rest api -------- */
  getAllOrganizations() {
    return vThis.$http.get(`${HTTP.BASE}org/organization/all`)
  },
  getAllAuth(appName = this.$appName) {
    return vThis.$http.get(`${USER_BASE_URL}role/all/tree?appName=${appName}`)
  },
  getAuthByApp(appName = JSON.parse(localStorage.getItem('allServers'))) {
    return vThis.$http.post(`${USER_BASE_URL}role/app/tree`, appName)
  },
  ...
  initializationDefaultGroup(groupId) {
    return vThis.$http.post(
      `${USER_BASE_URL}usermanagement/groups/resetDefaultGroup?groupId=${groupId}`
    )
  },
  ...
  updateUserPassword(requestBody) {
    return GatewayMainController.updateUserPassword({
      requestBody: requestBody,
    })
  },
  /* --------- ParameterController -------- */
  ...
  getObjectVote(para) {
    return vThis.$http.post(
      $url + '/vote/stars?objId=' + para.objId + '&typeId=' + para.typeId
    )
  },
  getDomainVote(id) {
  ...
  getGatewayUserInfo() {
    const url = `/gateway/main/getUserInfo`
  ...
  getAllDataSource() {
    if (!onceData.allDataSource) {
      ...
      onceData.allDataSource = vThis.$http.get(
        HTTP.META_BASE + 'models/fromre/'
      )
    }
    return onceData.allDataSource
  },
  getMetaAllDataSource() {
    if (!onceData.allMetaDataSource) {
      onceData.allMetaDataSource = vThis.$http.get(
        HTTP.META_BASE + 'models/fromre/'
      )
    }
    return onceData.allMetaDataSource
  },
  refreshDataSource() {
    onceData.allMetaDataSource = vThis.$http.get(
      HTTP.META_BASE + 'models/fromre/'
  ...
  getAllModels() {
    if (!onceData.allModels) {
      onceData.allModels = vThis.$http.get($meta_url + '/models/ddm/models')
    }
  ...
  getCatalogColumns({ succesedCallback, failureCallback, para }) {
    ...
    vThis.$http
      .get($meta_url + '/entities/' + objectId + '/columns')
      .then(res => {
        succesedCallback && succesedCallback(res.data)
      })
      .catch(e => {
  ...
  getAllTablesFromDam({ succesedCallback, failureCallback, para }) {
    ...
    let schema = ''
    if (para) {
      modelIds = para.modelId ? [para.modelId] : null
      schema = para.schema
    ...
    vThis.$http
      .post(HTTP.META_BASE + 'entities/searchMetadata', json)
      .then(res => {
        if (
  ...
  getTablesFromDam({ succesedCallback, failureCallback, para }) {
    vThis.$http
      .post(HTTP.META_BASE + 'entities/searchMetadata', para)
      .then(res => {
        succesedCallback && succesedCallback(res.data)
      })
      .catch(e => {
  ...
  /* ****************** new workflow start ************ */
  getAllWorkflowTypes() {
    const url = `${HTTP.wBase}workflow/process/type?appName=${JSON.parse(
  ...
  getMyApply(para) {
    para.appName = JSON.parse(localStorage.getItem('allServers'))
  ...
  getDomainDetailByCodeMetric(codes) {
    return this.getCodeDetailServiceMetric(codes)
    // const url = `${HTTP.BASE}domains/codes/domain/codes`
    // return vThis.$http.post(url, codes)
  },
  // 获取维度 目录列表
  ...
  getUpdsMetric(
    ...
    return this.getDomainUdpsService2(para)
  },
  // 导入UDP标准
  domainUdpUploadUrl() {
  ...
  sendNotification({ succesedCallback, failureCallback, para }) {
    ...
    */
    ...
    vThis.$http
      .post(HTTP.BASE + 'notifications/createNotification', para)
      .then(res => {
        succesedCallback && succesedCallback(res.data)
      })
      .catch(e => {
  ...
  getIfCollected({ succesedCallback, failureCallback, para }) {
    ...
    vThis.$http
      .post($url + '/favor/getByCondition', {
        typeId: parseInt(para.typeId),
        objId: String(para.objId),
      })
      .then(res => {
        let data = null
  ...
  addFavorite({ succesedCallback, failureCallback, para }) {
    vThis
      .$http_cache('post', $url + '/favor/addToFavorite', para)
      .then(res => {
        succesedCallback && succesedCallback(res.data)
      })
      .catch(e => {
  ...
  removeCollection({ succesedCallback, failureCallback, para }) {
    vThis.$http
      .post($url + '/favor/delete?favId=' + para.favId)
      .then(res => {
        succesedCallback && succesedCallback(res.data)
      })
      .catch(e => {
  ...
  refreshCollectionList({ succesedCallback, failureCallback }) {
    onceData.allFavorite = vThis.$http_cache('post', $url + '/favor/loadAllFav')
    onceData.allFavorite
      .then(res => {
        succesedCallback && succesedCallback(res.data)
      })
  ...
  getMyRate({ succesedCallback, failureCallback, para }) {
    vThis.$http
      .post($url + '/vote/stars?objId=' + para.objId + '&typeId=' + para.typeId)
      .then(res => {
        succesedCallback && succesedCallback(res.data)
      })
      .catch(e => {
  ...
  getSelectionOptions(para) {
    const url = `${$url}/select/option/get`
    const requestBody = para.requestBody || {}
  ...
  async getOrgDetailByBms(bm = ['@ROOT']) {
    let result = null
    let err = null
    try {
      const url = `${USER_BASE_URL}org/organization/byBms`
      result = await vThis.$http.post(url, bm)
    } catch (e) {
      err = e
    }
    return result
  ...
  getGroupsPage(para) {
    return this.getGroupsPageDomain(para)
  },
  async getGroupsPageDomain(para) {
    let result = null
    let err = null
    try {
  ...
  getModelBaseLine(para) {
    ...
    const url = `${$meta_url}/models/ddm/models/${modelId}/baselines`
    return vThis.$http.get(url)
  },
  getModelDetail(para) {
    const modelId = para.modelId
    const url = `${$meta_url}/models/ddm/models/${modelId}`
    return vThis.$http.get(url)
  },
  /**
   * 获取 报表 详细信息
  ...
  getReportDetail({ reportId }) {
    return vThis.$http.get(`${$meta_url}/dataReport/${reportId}`)
  },
  ...
  getTableProfile({
    ...
    vThis.$http
      .get(HTTP.META_BASE + `profile/${objectId}`)
      .then(res => {
        successCallback(res.data)
      })
      .catch(e => {
  ...
  postTableProfile({
    ...
    vThis.$http
      .post(HTTP.META_BASE + `profile/${objectId}`)
      .then(res => {
        successCallback(res.data)
      })
      .catch(e => {
  ...
  getIsTableProfiling({
    ...
    vThis.$http
      .get($meta_url + `/profile/${objectId}/state`)
      .then(res => {
        successCallback(res.data)
      })
      .catch(e => {
  ...
  getTableColumns({
    ...
    vThis.$http
      .get(HTTP.META_BASE + `entities/${objectId}/columns`)
      .then(res => {
        successCallback(res.data)
      })
      .catch(e => {
  ...
  testSql(requestBody) {
    return vThis.$http.post(HTTP.DDS + 'api/testSql', requestBody)
  },
  showFailure(e) {
  ...
  exportRules(ids) {
    return vThis.$http.post(`${HTTP.BASE}datamasking/download?ruleIds=${ids}`)
  },
  ...
  getMetaModelIcon(metaModelCode, objectId) {
    const url = `/metadata/mm/${metaModelCode}/${objectId}`
    return this.$http.get(url, { responseType: 'blob' })
  },
  getMetaModelIconNew(objectId) {
    if (!onceData.metaModelIcon[objectId]) {
      const url = `/metadata/mm/${objectId}`
      onceData.metaModelIcon[objectId] = this.$http.get(url, {
        responseType: 'blob',
      })
    }
  ...
  getMetaModelTypes() {
    const url = `/metadata/mm/getAllList`
