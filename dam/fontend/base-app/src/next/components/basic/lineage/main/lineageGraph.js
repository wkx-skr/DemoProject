...
export default {
  ...
  methods: {
    ...
    showDetail(args) {
      // 将showColumn参数传递给detailDrawer
      args.showColumn = this.param.showColumn
              ...
              if (!prev || prev === item.source) {
                if (inputOrOutputSteps.has(item.targetStepId)) {
                  finalLines.push({
                    ...
                    etlSource:
                      item.etlSource || item?.properties?.etlSource,
                  })
                } else {
    ...
    initFilterSteps(rawData) {
      if (this.filterStepsSet.size === 0) {
        const currentStepId = Object.keys(rawData.steps).find(stepId => {
          return (
            ...
            rawData.steps[stepId].columns.some(
              i =>
                i.id === String(this.objectId) ||
                rawData.steps[stepId].columns.some(
                  i => i.id === String(this.objectId)
                ) ||
                rawData.steps[stepId].properties.typeId == LDMTypes.Index
            )
          )
        })
        ...
        this.filterStepsSet.add(currentStepId)
        const blurStep = new Set()
        const findLastStep = (currentStepId, restSteps) => {
          if (blurStep.has(currentStepId)) {
            return
          }
          blurStep.add(currentStepId)
          if (restSteps === 0) {
            return
          }
          if (
