<template>
  <div style="position: relative" class="db-table">
    ...
    >
      ...
      >
        <template slot-scope="scope">
          <el-radio
            class="radioSel"
            :disabled="
              (scope.row.disabled ? scope.row.disabled : false) ||
              !checkSelect(scope.row, scope.$index)
            "
      ...
      >
        <template slot-scope="scope">
          ...
          <template v-else>
            <slot v-if="c.slotName" :name="c.slotName" :scope="scope" />
            <span v-else>
              {{
                c.formatter
                  ? c.formatter(scope.row, { property: c.prop })
                  : scope.row[c.prop]
              }}
            </span>
          </template>
    ...
    <datablau-button
      v-if="
        hasAccess && showColumnSelection && hasComponentCaseName && allColumns
      "
