  ...
  >
    <div
      ...
      :class="{
        ...
        'json-step-table':
          shape.shapeType === ShapeType.STEP &&
          shape.stepType === StepType.JSON_STEP,
  ...
  methods: {
    ...
    updateInputAndOutputProperties() {
      ...
      this.edges.forEach(({ source, sourceStepId, target, targetStepId }) => {
        const SourceStepId = sourceStepId || source
        const TargetStepId = targetStepId || target
    ...
    updateLineageGraph(
      ...
      this.$http
        .post(
          this.$meta_url +
            `/lineage/updateLinageContent?lineageId=${this.$route.query.id}`,
          {
            steps: steps,
            lines: filteredLines,
          }
      ...
      this.edges.forEach(edge => {
        ...
        if (
          ...
          edgesMap.set(this.getEdgeId(edge), {
            ...
            $indirect_impact: !!edge?.properties?.$indirect_impact
          })
        }
      })
