...
import {VersionContent} from "@/next/service/lineage/types/Version.ts";
...
class LineageGraph {
  ...
  */
  private drawStep(step: Step) {
    ...
    if (step.properties.$json_step === 'true') {
      // 展示为前一个节点的颜色，而不使用特殊颜色
      // stepType = StepType.JSON_STEP
    }
  ...
  private createView(): void {
    ...
    if ([GraphType.TABLE_ONLY, GraphType.SCHEMA_AND_TABLE, GraphType.CATEGORY_AND_TABLE].includes(this.graphType)) {
      ...
      this.lines.forEach(item => {
        if (item.source.startsWith('undefined')) {
          item.source = item.sourceStepId
        }
        if (item.target.startsWith('undefined')) {
