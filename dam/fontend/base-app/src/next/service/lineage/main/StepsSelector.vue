...
export default {
  ...
  computed: {
    realSteps() {
      const realSteps = Object.values(this.allSteps).filter(i => {
        return i.properties.$input_table || i.properties.$output_table
      })
      realSteps.sort((a, b) => {
        if (a.name && b.name) {
          return _.lowerCase(a.name).localeCompare(_.lowerCase(b.name))
        }
      })
      return realSteps
    },
