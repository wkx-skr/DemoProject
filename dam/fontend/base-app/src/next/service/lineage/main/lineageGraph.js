  ...
  methods: {
    ...
    handleMetaSelect({ table }) {
      const getColumns = (tableId, callback) => {
        let columns = []
        if (tableId) {
          const requestUrl = `${this.$meta_url}/service/entities/${tableId}/columns`
          this.$http
            .get(requestUrl)
            .then(res => {
              columns = res.data
    ...
    getData() {
      ...
      if (this.$route.query.id) {
        requestUrl = self.$meta_url + '/service/lineage/' + self.$route.query.id
      } else if (this.$route.query.folderId) {
        requestMethod = 'post'
        requestUrl = `${this.$meta_url}/service/lineage/getLineageOfFolderId?folderId=${this.$route.query.folderId}`
      }
    ...
    paintGraph() {
      ...
      if (Object.keys(this.rawData.steps).length > 50) {
        if (!this.selection || this.selection.length === 0) {
          const keys = Object.values(this.rawData.steps)
            .filter(i => {
              return i.properties.$input_table || i.properties.$output_table
            })
            .map(i => i.id)
          keys.sort((a, b) => {
            if (this.rawData.steps[a].name && this.rawData.steps[b].name) {
              return _.lowerCase(this.rawData.steps[a].name).localeCompare(
                _.lowerCase(this.rawData.steps[b].name)
              )
            }
          })
          this.selection = [this.rawData.steps[keys[0]]]
        }
      } else {
        if (!this.selection || this.selection.length === 0) {
      ...
      }
      {
        ...
        Object.values(this.rawData.steps).forEach(item => {
          item.columns.forEach(column => {
            column.id += '------' + column.stepId
          })
        })
        this.rawData.lines.forEach(item => {
          item.source += '------' + item.sourceStepId
          item.target += '------' + item.targetStepId
        })
      }
      data = this.filterByTable(this.rawData)
      data.lines.forEach(item => {
        if (data.steps[item.targetStepId]?.properties?.$json_step) {
          data.steps[item.targetStepId].properties.$input_table =
            data.steps[item.sourceStepId].properties.$input_table
          data.steps[item.targetStepId].properties.$output_table =
            data.steps[item.sourceStepId].properties.$output_table
          console.log(data.steps[item.sourceStepId])
        }
      })
      if (this.showFullProcess) {
      ...
      lines.forEach(line => {
        ...
        if (tLinesString.indexOf(str) === -1) {
          ...
          tLines.push({ source: source, target: target, properties: line?.properties })
        }
      })
      this.startDraw(data)
    },
