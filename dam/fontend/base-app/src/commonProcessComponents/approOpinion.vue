    ...
    >
      <el-form-item :label="$user.username + ': '">
        <datablau-input
          ...
          :rows="3"
          clearable
          :placeholder="$t('userPane.myTodo.placeholder.approOpinion')"
          type="textarea"
          style="display: block"
        ></datablau-input>
      </el-form-item>
      <el-form-item>
        ...
        >
          {{ item.name === null ? $t("userPane.myTodo.submit") : translateButtonName(item.name)}}
...
export default {
  ...
  methods: {
    ...
    translateButtonName(name){
      switch (name) {
        case '通过':
          return this.$t('userPane.myTodo.pass')
        case '驳回':
          return this.$t('userPane.myTodo.reject')
        case '提交':
          return this.$t('userPane.myTodo.submit')
        default:
          return name
      }
    },
    handleApply(handleItem) {
      ...
      if (
        ...
        this.$message.error(this.$t('userPane.myTodo.pleaseSelectView'))
        return
      }
      ...
      HTTP.completeTask(para)
        .then(res => {
          ...
          this.$message.success(this.$t('userPane.myTodo.handleSuccess'))
