...
export default {
  props: {
    ...
    labelText: {
      default() {
        return {
          domainCode: this.$t('userPame.myTodo.domainCode'),
        }
      },
    },
    isIndex: Boolean,
  ...
  methods: {
    isDiffBase(field) {
      if (this.compareData) {
        if (field === 'path') {
          const value = this.details.path ? this.details.path.join('/') : ''
          const value1 = this.compareData.path
            ? this.compareData.path.join('/')
            : ''
          return this.isDiffValue(value, value1, field)
        }
        const value = _.get(this.details, field, '')
        const value1 = _.get(this.compareData, field, '')
        return this.isDiffValue(value, value1, field)
      }
      return false
    },
    isDiffAddi(field, value) {
      if (this.compareAddiData.length > 0) {
        const value1 = this.compareAddiData.find(v => v.name === field)?.value
        return this.isDiffValue(value, value1, field)
      }
      return false
    },
    isDiffValue(value, value1, prop) {
      // 都是空则相同
      if (!value && value !== 0 && !value1 && value1 !== 0) {
        return false
      }
      if (!value && value !== 0 && value1) {
        return true
      }
      if (!value1 && value1 !== 0 && value) {
        return true
      }
      if (value1 !== value) {
        return true
    ...
    async innerLoadStandardDetails(domainId) {
      if (domainId) {
        ...
        if (this.processInstanceId) {
          ...
          if (
            res.data.checkState &&
            ['UNPASS', 'SKIP'].includes(res.data.checkState)
          ) {
            const similarityDetailRes = await this.$http
              .post(
                '/domain/domains/similarity/getSimilarityDetailByDomainId',
                {
                  domainId: domainId,
                }
              )
            ...
            this.similarityDetail = (similarityDetailRes.data || [])
              .map(v => v.chineseName)
              .join('，')
          }
        }
        ...
        this.getDomainCodesData(res.data.dimCodes ? res.data.dimCodes : [])
