  ...
  <div>
    ...
    <div v-if="commonData.processType === '项目版本发布'">
      ...
      <datablau-button
        ...
        "
      >
        {{ $t('userPane.myTodo.versionDocument') }}
      </datablau-button>
      <datablau-button
        ...
        "
      >
        {{ $t('userPane.myTodo.approvalContent') }}
      </datablau-button>
    </div>
    <div
      v-if="
        commonData.processType === '资产权限申请' &&
        $route.query.currentNav === 'myApply'
      "
      style="float: right"
    >
      <datablau-button
        ...
        :disabled="
          tableData.find(
            data =>
              statusFormatter(data) === $t('userPane.myTodo.inApproval') ||
              statusFormatter(data) === '驳回'
          )
        "
        @click="downloadApplyRecord"
      >
        {{ $t('userPane.myTodo.downloadForm') }}
      </datablau-button>
    </div>
  </div>
</template>
  ...
  methods: {
    ...
    downloadApplyRecord() {
      this.$downloadFilePost(
        `/assets/workflow/receipt?processId=${this.commonData.processInstanceId}`,
        {},
        '数据申请单.docx'
      )
    },
    statusFormatter(row, column, cellValue, index) {
      ...
      if (row.endTime) {
        if (row.param && row.param.result) {
          ...
          result = this.$t('userPane.myTodo.pass')
        }
      } else {
        if (row.startTime) {
          result = this.$t('userPane.myTodo.inApproval')
        } else {
          result = this.$t('userPane.myTodo.awaitApproval')
        }
      }
      return result
    },
