...
import JSONFun from './utils/isJSON'
...
if (sessionStorage.getItem('product') === 'ddc') {
  sessionStorage.removeItem('product')
  location.href = settings.api + '/#/home'
}
Vue.use(Vuex)
...
This.$meta_url = '/metadata'
This.$graph_url = '/graph'
This.$quality_url = '/quality'
This.$job_url = '/job'
This.$asstes_url = '/assets'
This.$metric = '/metric'
...
This.$getAbout = () => {
  const getAbout = This.$http.get(This.$base_url + '/service/main/about')
  getAbout
    .then(res => {
...
Vue.prototype.$showFailure = function () {
  ...
  if (status === 401) {
    ...
    this.$message.error('错误类型' + status)
  } else if (status != 'code') {
    this.$message.error('错误类型' + status)
  } else {
...
Vue.prototype.$downloadFile = (url, para) => {
  ...
  if (
    ...
    This.$http
      .get(`${This.$base_url}/service/files/${fieldId}`)
      .then(res => {
...
This.$fileTypeFormatter = fileType => {
  ...
  if (/(zip|rar|gz|7z)$/.test(fileType)) {
    result = 'zip'
  } else if (/(xlsx|xls|xlsm)$/.test(fileType)) {
    result = 'excel'
  ...
  HTTP.getDomainDetailByIdService({ domainId, index })
    .then(res => {
      ...
      if (data.categoryId <= 3) {
        ...
        // 域标准
        if (HTTP.$appName === 'DAM') {
          This.$skip2({ name: 'domainStandard', query: { domainId } })
        } else {
        ...
        This.$skip2({
          ...
          query: { id: domainId, blank: true, isAssets: true },
        })
