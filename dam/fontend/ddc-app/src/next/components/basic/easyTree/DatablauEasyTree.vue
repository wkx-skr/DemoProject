...
export default {
    ...
    treeRenderContent() {
      if (this.renderContent) {
        ...
        return (h, { _self, node, data, store }) => {
          const noClickLeft = -40 - 18 * (node.level - 1)
          const style = {
            outer:
              'position:relative;flex:1;display: flex;align-items: center;justify-content: space-between;height:100%',
            ...
            noClick:
              ...
              noClickLeft +
              'px; bottom: 0;z-index: 2; cursor: no-drop',
            icon: 'color: #409eff',
          }
          const HAS_ICON =
  ...
  methods: {
    ...
    callLockedTips(data) {
      ...
      Object.keys(this.showLockedTips).forEach(i => {
        this.showLockedTips[i] = false
      })
