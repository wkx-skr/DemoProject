...
export default {
  ...
  methods: {
    ...
    handleSpe() {
      ...
      const handler = () => {
        const contentHeight = parseFloat($(this.parentNode).css('height'))
        const innerHeight = parseFloat(
          $(this.parentNode).children().css('height')
        )
      ...
      this.observe = observe
      const targetNode = document.querySelector(this.parentNode)
      observe.observe(targetNode, config)
    },
  },
}
