...
export default {
  ...
  beforeMount() {
    if (this.isAsset) {
      this.imgUrl = require(`@/assets/images/dataAssets/portal/noData.png`)
    } else if (this.url) {
      const rex = /(\/static)/g
  ...
  computed: {
    dataTypeComputed() {
      ...
      if (dataType === 'metamodel_object') {
        this.imgUrl = require(`@/assets/images/search/metamodel_object.png`)
