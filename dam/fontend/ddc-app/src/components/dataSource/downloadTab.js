  ...
  methods: {
    ...
    filterData(){
      ...
      this.fullData.forEach(item=>{
        if(item.name && item.name.toLowerCase().includes(keyword) || item.physicalName && item.physicalName.toLowerCase().includes(keyword) || item.definition && item.definition.toLowerCase().includes(keyword)){
          tableData.push(item);
        }
      });
    ...
    download() {
      ...
      let href = this.$url + '/service/models/' + this.modelId + '/exportAsync'
      if (result.length !== 0) {
      ...
      this.$http
        .post(href, {})
        .then(res => {
          this.$bus.$emit('getTaskJobResult', res.data)
        })
        .catch(e => {
          this.$showFailure(e)
        })
    ...
    getTableData2() {
      ...
      this.$http
        .post(this.$url + '/service/models/search/metadata/export', {
