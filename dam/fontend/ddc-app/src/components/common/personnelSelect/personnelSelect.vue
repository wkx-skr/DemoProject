<template>
  <div>
    <datablau-dialog
      :title="$t('component.personSelect.personnel')"
      :visible.sync="$store.state.showPersonnelSelect"
    ...
    >
      <div class="content">
        <div>
          <datablau-input
            v-model="nameInput"
            :iconfont-state="true"
            :placeholder="$t('common.personSelect.searchPeople')"
            clearable
            class="nameInput"
            @input="keyWord"
            @clear="inputClear"
          ...
          <div v-show="!flag">
            ...
            <div v-show="!search">
              <div v-show="!onlyPeople">
                <datablau-checkbox
                  :label="$t('common.personSelect.selectAll')"
                  class="checkbox"
                  :checkboxType="'single'"
                  v-model="checkbox"
                  @click.native.prevent.stop="setCheckbox('checkbox')"
                  v-show="
                    selectListName === $t('common.personSelect.institutional')
                  "
                ></datablau-checkbox>
                <datablau-checkbox
                  :label="$t('common.personSelect.selectAll')"
                  ...
                  v-show="selectListName === $t('common.personSelect.role')"
                ></datablau-checkbox>
                <datablau-checkbox
                  :label="$t('common.personSelect.selectAll')"
                  class="checkbox"
                  ...
                  v-model="checkbox2"
                  v-show="selectListName === $t('common.personSelect.user')"
                ></datablau-checkbox>
              </div>
              <div :class="{ onlyPeople: onlyPeople }">
                <datablau-tree
            ...
            >
              ...
              >
                <datablau-checkbox class="checkbox" v-model="checkboxPersonnel">
                  <el-checkbox
                    :label="item.username"
        ...
        <div class="tagiconBox">
          <p>{{ $t('common.personSelect.chosen') }}</p>

          <el-tag
            :key="(tag.fullName || tag.userName || tag.fullUserName) + tag.id"
          ...
          >
            ...
            <span class="tagBox">
              {{ tag.fullUserName || tag.username || tag.fullName }}
            </span>
          </el-tag>
        </div>
      </div>
      <span slot="footer">
        <datablau-button @click="handleClose">
          {{ $t('common.personSelect.cancel') }}
        </datablau-button>
        <datablau-button type="primary" @click="primary">
          {{ $t('common.personSelect.sure') }}
        </datablau-button>
      </span>
    </datablau-dialog>
  </div>
...
<script>
import HTTP from '@/view/dataAsset/utils/api'
export default {
  /**
  ...
  data() {
    return {
      ...
      flag: true,
      menu: [],
      selectListName: this.$t('common.personSelect.results'),
      defaultProps: {
        children: 'children',
        label: 'fullName',
        isLeaf: 'isLeaf',
      ...
      list: [
        {
          name: this.$t('common.personSelect.institutional'),
          icon: 'department',
          id: 1,
        },
        {
          name: this.$t('common.personSelect.role'),
          icon: 'role',
          id: 2,
        },
        {
          name: this.$t('common.personSelect.user'),
          icon: 'group',
          id: 3,
        },
      ],
  ...
  watch: {
    ...
    '$store.state.showPersonnelSelect': {
      handler(val) {
        this.newTagList = this.$store.state.personnelProp.allSafeTags || []
        this.allSelected = this.$store.state.personnelProp.allSafeTags || []
        this.onlyPeople = this.$store.state.personnelProp.onlyPeople
        this.newTagList.forEach(
          item => (item.fullName = item.fullName || item.fullUserName)
        )
        this.listName = this.newTagList.map(item => {
    ...
    keySelectFlag: {
      handler(val) {
        this.keySelect = this.nodeKeyList.filter(item =>
          this.listName.some(v => item.indexOf(v) !== -1)
        )
  ...
  methods: {
    ...
    keyWord(val) {
      this.flag = false
      this.search = true
      this.selectListName = this.$t('common.personSelect.results')
      this.loading = true
      this.page = 1
      ...
      HTTP.getAllUserPage(requestBody)
        .then(res => {
          ...
          let ary = this.personnelList.filter(item =>
            this.newTagList.some(v => v.username === item.username)
          )
          this.checkboxPersonnel = ary.map(item => item.username)
          this.loading = false
        })
        .catch(e => {
          this.$showFailure(e)
    ...
    primary() {
      ...
      this.$store.commit('changePersonnelData', this.newTagList)
      this.$utils.personnelSelect.close()
    },
    setChecked(ary) {
    ...
    tagClose(val) {
      ...
      this.$refs.treeList &&
        this.$refs.treeList.setCheckedKeys([...this.listName])
    ...
    selectClick(item) {
      ...
      this.loading = true
      this.setChecked([])
      if (item.name === this.$t('common.personSelect.institutional')) {
        this.mechanism()
      } else if (item.name === this.$t('common.personSelect.role')) {
        this.getRole()
      } else if (item.name === this.$t('common.personSelect.user')) {
        this.getGroup()
      }
      this.flag = false
    },
    ...
    processing(res, data) {
      ...
      if (res.data) {
        res.data.forEach(item => {
          ...
          let som = data.children.some(items => {
            return items.fullName === item.fullName
          })
          !som && data.children.push(...res.data)
        })
    ...
    nodeGreyed(ary, data) {
      ...
      ary.forEach(item => {
        ...
        if (this.listName.indexOf(names + item.id) !== -1) {
          // item.disabled = true
        } else {
          // item.disabled = false
        }
    ...
    handleNodeClick(data) {
      ...
      if (
        this.selectListName === this.$t('common.personSelect.institutional')
      ) {
        this.mechanismAcquisition(data)
      } else if (this.selectListName === this.$t('common.personSelect.role')) {
        this.roleAcquisition(data)
      } else if (this.selectListName === this.$t('common.personSelect.user')) {
        this.groupAcquisition(data)
      }
    },
    renderContent(h, { node, data, store }) {
      const personUnit = this.$t('common.personSelect.peopleUnit')
      const style = {
        ...
        border:
          'border-right: 1px solid #ddd;padding-right:10px;margin-right:10px;display: inline-block;max-width: 77px;overflow: hidden;text-overflow: ellipsis;white-space: nowrap;line-height: 11px;',
        email:
          'width: 100px;overflow: hidden;display: inline-block;vertical-align: top;white-space: nowrap;text-overflow: ellipsis;',
        fullName:
        ...
        <div style={style.flex}>
          ...
          {data.fullUserName && (
            <div style={style.position}>
              ...
              {(data.emailAddress && data.emailAddress.length > 15 && (
                <el-tooltip
                  placement="top"
                  effect="light"
                  popper-class="emailtipBox"
                >
                  <div slot="content"> {data.emailAddress || '--'} </div>
                  <span style={style.email}> {data.emailAddress || '--'} </span>
                </el-tooltip>
              )) || (
                <span style={style.email}> {data.emailAddress || '--'} </span>
              )}
    ...
    checkedList(obj, obj1) {
       ...
       * */
      let names = obj.fullUserName || obj.fullName
      if (!currentNode.checked) {
        // b 情况
        if (this.listName.indexOf(names + obj.id) !== -1) {
      ...
      try {
        this.menu.forEach(item => {
          ...
          if (!node.checked) {
            ...
            if (
              this.selectListName ===
              this.$t('common.personSelect.institutional')
            ) {
              this.checkbox = true
            } else if (
              this.selectListName === this.$t('common.personSelect.role')
            ) {
              this.checkbox1 = true
            } else if (
              this.selectListName === this.$t('common.personSelect.user')
            ) {
              this.checkbox2 = true
            }
          }
    ...
    newTagSplice(obj) {
      ...
      this.newTagList.map((item, index) => {
        if (item.fullName === names && item.id == obj.id) {
          this.newTagList.splice(index, 1)
    ...
    addNewTag(obj) {
      ...
      let names = obj.fullUserName || obj.fullName
      if (this.listName.indexOf(names + obj.id) === -1) {
        this.newTagList.push(obj)
        this.listName.push(names + obj.id)
      }
      this.newTagList = [
    ...
    handleClose() {
      // this.$emit('close')
      this.$utils.personnelSelect.close(true)
    },
  },
  mounted() {},
}
