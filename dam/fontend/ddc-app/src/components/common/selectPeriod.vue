<template>
  <div>
    <datablau-form label-width="0px">
      ...
      <el-form-item
        :label="
          hasLable ? '' : $t('quality.page.qualityExamineJob.repetitionPeriod')
        "
      >
...
export default {
  ...
  mounted() {
    ...
    if (this.defaultSchedule) {
      this.radio2 = this.defaultSchedule
    }
  },
  beforeDestroy() {
    setTimeout(() => {
  ...
  methods: {
    ...
    make() {
      ...
      if (this.radio === 'scheduleByCron') {
        ...
        const yearPart = self.year
          ? typeof self.year === 'object'
            ? self.year.getFullYear()
            : self.year
          : ''
        // const msg = function () {
