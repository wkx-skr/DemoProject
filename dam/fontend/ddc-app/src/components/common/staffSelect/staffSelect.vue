<template>
  <div>
    ...
    >
      <div
        class="staff-select-header"
        :class="{ 'hide-org': !$damEnabled || !$store.state.showStaffTree }"
      >
        <datablau-select
          v-model="toBm"
          filterable
...
export default {
  ...
  computed: {
    useDam() {
      return window.sessionStorage.getItem('ddmFirst') !== 'true'
    },
  },
  beforeDestroy() {
    setTimeout(() => {
  ...
  methods: {
    ...
    initData() {
      ...
      if (this.$store.state.showStaffSelectType === 1) {
        HTTP.getGroupsPage(obj, this.$damEnabled)
          .then(res => {
        ...
        this.$http
          .post(
            `${this.$url}/service/quality/usersWithHasAuth/getPage?taskId=${this.$store.state.taskId}`,
            obj
          )
          .then(res => {
            this.tableLoading = false
    ...
    getBranchList() {
      ...
      HTTP.getOrgTree()
        .then(res => {
          if (this.$store.state.showStaffTree) {
            ...
            this.allDepartmentList = this.flatten([res.data])
            this.toBm = res.data.bm
            this.departmentList = [{ ...res.data, children: undefined }]
            this.initData()
          }
        })
        .catch(e => {
    ...
    // itemSelectable (row) {
    //   return !this.disabledList.includes(row.username)
    // }
  },
  watch: {
    orgKey(val) {
