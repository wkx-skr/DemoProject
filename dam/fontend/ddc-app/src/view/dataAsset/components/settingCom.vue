...
export default {
  ...
  methods: {
    ...
    handleClose(val) {
      ...
      //
      this.$DatablauCofirm(
        this.$t('assets.settingCom.confirmDelete'),
        ...
        .then(() => {
          ...
          HTTP.delPermission(params)
            .then(res => {
              this.$message.success(this.$t('assets.saveSuccess.delSuccess'))
              this.loading = false
    ...
    primary(val) {
      ...
      HTTP.addNewPermission(params, this.currentNode.id)
        .then(res => {
          this.$message.success(this.$t('assets.saveSuccess.saveSuccess'))
          this.dialogVisible = false
          this.$emit('getSettings')
        })
        .catch(e => {
