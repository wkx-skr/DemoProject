<template>
  <div class="apply-power-page">
    <datablau-dialog
      width="480px"
      :title="$t('assets.applyPower.applyForPermission')"
      :visible.sync="showApply"
      :before-close="close"
      :height="300"
    >
      <el-form ref="applyForm" :model="applyForm" label-width="80px">
        <el-form-item :label="$t('assets.applyPower.applyForPermission')">
          <!-- <datablau-radio
            class="power-content"
          ...
          <span>{{ $t('assets.applyPower.accessRights') }}</span>
        </el-form-item>
  ...
  methods: {
    ...
    submit(name) {
      this.$refs[name].validate(valid => {
        if (valid) {
          const params = {
            catalogId: this.applyInfo.catalogId,
            catalogName: this.applyInfo.assetsName,
            // mandateType: this.applyForm.type,
            mandateType: 'PERSON',
            reason: this.applyForm.reason,
          }
          api
            .applyAuthForCatalog(params)
            .then(res => {
