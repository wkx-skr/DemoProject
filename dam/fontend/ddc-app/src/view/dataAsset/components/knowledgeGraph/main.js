...
import { toBrowsing } from '@/view/dataAsset/utils/methods.js'
...
export default {
  ...
  mixins: [Resource],
  props: ['summary', 'name', 'currentNode', 'permission', 'structureId', 'editable'],
  data() {
    const hierarchy = []
    for (let i = 1; i <= 100; i++) {
      hierarchy.push({
  ...
  methods: {
    handleResize() {
      ...
      this.smallScreen = window.innerWidth < 1566
    },
    ...
    nodeOptionsValueChange() {
      this.searchFormData.nodeOptions.forEach(element => {
        if (this.metaModelTypeMap[element.value]) {
          this.$set(
            element,
            'label',
            this.metaModelTypeMap[element.value].chineseName
          )
        }
        if (this.typeNameArr[element.value]) {
          this.$set(element, 'label', this.typeNameArr[element.value])
        }
      })
      this.relationshipValue = []
    },
    ...
    async goDetail(detail) {
      if (!this.typeNameArr[detail.typeId]) {
        // 元模型
        detail = _.cloneDeep(detail)
        detail.typeId = AssetsTypeEnum.META_MODEL
        detail.itemId = detail.Id
        detail.assetsId = ''
        detail.catalogId = ''
        toBrowsing(detail, this)
      }
      let query = {}
      let queryData = ''
      if (this.isAssets) {
      ...
      if (detail.typeId === '80010000') {
        ...
        // 指标
        this.$skip2Domain(detail.Id, this.isAssets, 'INDEX')
        ...
        window.open(
          baseUrl +
            `lineageGraph?id=${detail.Id}&filename=${encodeURIComponent(
              detail.Name
        ...
        if (this.name === 'assets') {
          window.open(
            this.BaseUtils.RouterUtils.getFullUrl('metaFolder', {
              objectId: detail.Id,
              type: 'file',
              blank: true,
              isAssets: this.isAssets,
        ...
        window.open(
          this.BaseUtils.RouterUtils.getFullUrl('indexModifierDdm', {
            id: detail.Id,
          })
        )
      } else if (parseInt(detail.typeId) === LDMTypes.TimeModifierType) {
        window.open(
          this.BaseUtils.RouterUtils.getFullUrl('indexTimerDdm', {
            id: detail.Id,
          })
        )
        ...
        // 暂时先跳转数据分类页面，下期会做调到相应的目录（目前缺少相应的数据，数据分类树也是懒加载）
        window.open(this.BaseUtils.RouterUtils.getFullUrl('accessControl', {}))
      } else {
        console.log(detail)
      }
    },
    ...
    getMetaModelTypeMap() {
      HTTP2.getMetaModelTypes()
        .then(res => {
          let data = res.data || []
          let metaModelTypeMap = {}
          data.forEach(item => {
            metaModelTypeMap[item.id] = item
          })
          this.metaModelTypeMap = metaModelTypeMap
          this.searchFormData.nodeOptions.forEach(element => {
            if (this.metaModelTypeMap[element.value]) {
              element.label = this.metaModelTypeMap[element.value].chineseName
            }
            if (this.typeNameArr[element.value]) {
              this.$set(element, 'label', this.typeNameArr[element.value])
            }
    ...
    // 获取数据
    getSearchNodeRelation(givenId, givenType) {
      ...
      this.$http
        ...
        .then(res => {
          ...
          this.nodesArrayInitial = _.clone(this.nodesArray)
    ...
    siblingFilter(list) {
      ...
      if (max) {
        ...
        const map = new Map()
        list.forEach(item => {
          const startId = item.relation.startbusinessId
          if (!map.has(startId)) {
            map.set(startId, 0)
          }
          const count = map.get(startId)
          if (count < max) {
            result.push(item)
          }
          map.set(startId, count + 1)
        })
        return result
      } else {
        return list
    ...
    imageNode(element, type) {
      let typeId = ''
      let result = ''
      let DIR = 'static/kgimg/'
      // result = DIR + element.startNode.typeId + '.svg'
      // if (type === "endNode") {
      //   if (element.endNode.properties.logicalElement === "true") {
      //     if (
      //       element.endNode.typeId === "80000004" ||
      //       element.endNode.typeId === "80000005"
      //     ) {
      //       result = DIR + element.endNode.typeId + "1.svg";
      //     } else {
      //       result = DIR + element.endNode.typeId + ".svg";
      //     }
      //   } else {
      //     result = DIR + element.endNode.typeId + ".svg";
      //   }
      //   typeId = element.endNode.typeId;
      // } else {
      //   if (element.startNode.properties.logicalElement === "true") {
      //     if (
      //       element.startNode.typeId === "80000004" ||
      //       element.startNode.typeId === "80000005"
      //     ) {
      //       result = DIR + element.startNode.typeId + "1.svg";
      //     } else {
      //       result = DIR + element.startNode.typeId + ".svg";
      //     }
      //   } else {
      //     result = DIR + element.startNode.typeId + ".svg";
      //   }
      //   typeId = element.startNode.typeId;
      // }
      if (type === 'endNode') {
        typeId = element.endNode.typeId
      } else {
      ...
      result = DIR + element[type].typeId + '.svg'

      let getImg = async () => {
        let url = ''
        let hasIcon = this.typeNameArr[typeId] // 是否已经存在图标
        if (!hasIcon) {
          try {
            // const reqUrl = `/metadata/mm/${typeId}`
            let resImg = await HTTP2.getMetaModelIconNew(typeId)
            ...
            this.$set(this.metaModelIconMap, typeId, url)
          } catch (e) {
            console.log(e)
          }
        } else {
    ...
    getNodeArr() {
        ...
        this.nodesArray.forEach(element => {
          if (this.relationshipValue.includes(element.relation.relationName)) {
            this.nodesArr.push({
              ...
              shape: 'image',
              image: this.imageNode(element, 'startNode'),
            ...
            this.nodesArr.push({
              ...
              shape: 'image',
              image: this.imageNode(element, 'endNode'),
              imagePadding: { left: 2, top: 10, right: 8, bottom: 20 },
              size: 24,
              scope: element.startNode.scope,
            })
      ...
      const relationshipOptionsPeon =
        this.searchFormData.relationshipOptions.reduce((cur, next) => {
          relationshipOptionsObj[next.value]
            ? ''
            : (relationshipOptionsObj[next.value] = true && cur.push(next))
          return cur
    ...
    getStart() {
      ...
      if (!this.firstEstablish) {
        ...
        this.network.on('oncontext', params => {
          ...
          const nodeId = this.network.getNodeAt({ x: x, y: y })
          this.nodesArr.forEach(element => {
