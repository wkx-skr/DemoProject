        ...
        >
          ...
          >
            <template slot-scope="scope">
              <div class="iconImg" @click="upload(scope.row, scope.$index + 1)">
            ...
            <template slot-scope="scope">
              ...
              <datablau-button type="icon" v-if="!scope.row.inUse">
                <datablau-tooltip
                  ...
                  popper-class="tooltipBox"
                  :content="$t('assets.common.delete')"
                >
                  <i
                    :class="{
                      iconfont: true,
    ...
    >
      ...
      <span slot="footer">
        <datablau-button type="secondary" @click="handleCloseDialog">
          {{ $t('assets.generalSettings.cancel') }}
        </datablau-button>
        <datablau-button
          type="primary"
...
import level5 from '@/assets/images/dataAssets/leve0.png'
import level0 from '@/assets/images/dataAssets/level1.png'
import level1 from '@/assets/images/dataAssets/leve2.png'
import level2 from '@/assets/images/dataAssets/leve3.png'
import level3 from '@/assets/images/dataAssets/leve4.png'
import level4 from '@/assets/images/dataAssets/leve5.png'
export default {
  name: 'assetSetting',
  components: {
    catalogTypeDetail,
            ...
            children: [
              {
                name:
                  this.nodeDataName ||
                  this.$t('assets.common.new', { name: '目录类型' }),
                couldClick: false,
              },
            ],
          },
  ...
  methods: {
    ...
    edit(item, index) {
      ...
      HTTP.getTypeDet(item.id)
        .then(res => {
          let data = res.data
          data.algorithmDto.udpDtoList &&
            data.algorithmDto.udpDtoList.forEach(item => {
    ...
    deleteAsset(obj) {
      if (obj.inUse) return
      this.$DatablauCofirm(
        this.$t('assets.common.deleteContentTips', { name: obj.name }),
        this.$t('assets.common.deleteHeadTips', { name: '目录类型' }),
        {
        ...
        .then(res => {
          HTTP.delAssetType(obj.id)
            .then(res => {
              this.$blauShowSuccess(this.$t('assets.common.deleteSuccess'))
              this.allMethod()
            })
            .catch(e => {
              this.$showFailure(e)
    ...
    save() {
      ...
      this.nodeDataName = this.$t('assets.common.new', { name: '目录类型' })
      this.oldRevise = _.cloneDeep(this.typeDetailData)
    ...
    primary(obj) {
      ...
      if (obj.algorithmDto) {
        const algorithmDto = []
        obj.algorithmDto.map(item => {
          if (!this.propertyCorrespond[item.attribute]) {
            item.extend = true
          } else {
            item.extend = false
          }
          item.attribute && item.weight && algorithmDto.push(item)
        })
        obj.catalogType.algorithmDto = algorithmDto
      } else if (obj.udps) {
    ...
    upload(item, index) {
      this.imageUrl = item.icon
        ...
        : // : index <= 4
          ...
          this.imgListAry[5]
      this.recoverImg = index <= 5 ? index : 0 // 获取默认图片的名字
      this.recoverImgURl =
        index <= 5 ? this.imgListAry[index - 1] : this.imgListAry[5]
    ...
    restoreIcon() {
      this.disabled = false
      this.showError = false
      HTTP.getDefault('level' + this.recoverImg)
        .then(res => {
          this.fileList = [
            {
              name: 1,
    ...
    upchange(file, fileList) {
      const oldFileList = this.fileList[0]
      this.showError = false
      this.fileList = []
      // console.log(file)
