          ...
          <div class="spaceBetween flex tabChang">
            ...
            <div class="newDirectory">
              <datablau-button
                ...
                @click="primary"
              >
                {{ $t('assets.directoryStructure.new') }}
              </datablau-button>
            </div>
          </div>
          <div class="tables">
            ...
            >
              ...
              >
                ...
                <el-table-column
                  prop="name"
                  :label="$t('assets.directoryStructure.strucureName')"
                ...
                >
                  <template scope="{row}">
                    ...
                    >
                      <img
                        class="useIcon"
                        src="@/assets/images/dataAssets/inuse.svg"
                        alt=""
                      />
                    </datablau-tooltip>
                    <datablau-tooltip
                    ...
                    >
                      <img
                        class="useIcon"
                        src="@/assets/images/dataAssets/uninuse.svg"
                        alt=""
                      />
                    </datablau-tooltip>
                    {{ row.name }}
                ...
                <el-table-column
                  prop="processName"
                  :label="$t('assets.directoryStructure.level')"
                  show-overflow-tooltip
                  width="100px"
                >
                  <template scope="{row}">
                ...
                <el-table-column
                  prop="approver"
                  :label="$t('assets.directoryStructure.directoryApprover')"
                  show-overflow-tooltip
                >
                  <template scope="{row}">
                ...
                >
                  <template scope="{row}">
                    <datablau-status
                      :type="row.catalogStraightPublish ? 4 : 7"
                      :desc="
                        row.catalogStraightPublish
                          ? $t('assets.directoryStructure.enable')
                          : $t('assets.directoryStructure.disableds')
                      "
                    ></datablau-status>
                ...
                >
                  <template scope="{row}">
                    <datablau-status
                      :type="row.assetsStraightPublish ? 4 : 7"
                      :desc="
                        row.assetsStraightPublish
                          ? $t('assets.directoryStructure.enable')
                          : $t('assets.directoryStructure.disableds')
                      "
                    ></datablau-status>
...
export default {
        ...
        children: [
          {
            ...
            children: [
              {
                name:
                  this.title || this.$t('assets.directoryStructure.addTitle'),
                couldClick: false,
              },
            ],
          },
  ...
  methods: {
    ...
    delet(obj) {
      ...
      this.$DatablauCofirm(
        text,
        this.$t('assets.directoryStructure.deleteTitle'),
        {
            ...
            HTTP.delStructure(obj.item.id)
              .then(res => {
                this.$message.success(
                  this.$t('assets.directoryStructure.deleteSuccess')
                )
                this.getStruc()
                this.listShow = true
              })
    ...
    saveItem(obj) {
      ...
      if (!item.name && !openStatus) return
      if (!item.approver && !openStatus) return
      if (item.structureManagerDto.length == 0 && !openStatus) return
      if (item.publicShow === undefined && !openStatus) return
      if (!openStatus) {
        let flag = true
        item.detailDtos
        ...
        .then(res => {
          if (item.id) {
            this.$blauShowSuccess(
              this.$t('assets.directoryStructure.editSuccess')
            )
          } else {
            this.$blauShowSuccess(
              this.$t('assets.directoryStructure.newSuccess')
            )
          }
    ...
    returnList() {
      if (
        ...
        this.$DatablauCofirm(
          ...
          this.$t('assets.directoryStructure.tips'),
          {
            confirmButtonText: this.$t('assets.directoryStructure.leave'),
            cancelButtonText: this.$t('assets.directoryStructure.stay'),
    ...
    async edit(row) {
      ...
      this.listShow = false
      this.loading = false
      this.title = this.$t('assets.directoryStructure.editTitle')
    ...
    activeNameClick() {
      ...
      if (activeName === 'second') {
        ...
        this.structureList = this.allStrctureList
          .filter(structure => structure.openStatus)
          .concat(
            this.allStrctureList.filter(structure => !structure.openStatus)
          )
      }
      this.currentTabKey++
    },
      ...
      HTTP.getStruc({ openStatus: '' })
        .then(res => {
          ...
          res.data.forEach(item => {
            let manager = []
            item.structureManagerDto.forEach(k => {
              manager.push(`${k.firstName}（${k.username}）`)
            })
          ...
          HTTP.strucOrder(_this.apiObjDrag)
            .then(res => {
              ...
              _this.$blauShowSuccess(
                _this.$t('assets.directoryStructure.sortingStructure')
              )
              _this.getStruc()
            })
            .catch(e => {
...
.structure {
  ...
  /deep/.el-table.datablau-table.datablau-table-5dot9 th.drag-column,
