    ...
    <div class="top-btn-line">
      <div class="left-btn-group">
        ...
        <focus-switch-button
          v-if="lineageData && !bound"
          :style="{
            'border-color': tableIsLoaded ? '#ddd' : '#409eff',
            color: tableIsLoaded ? '#666' : '#409eff',
          }"
          :disabled="tableIsLoaded"
...
import focusSwitchButton from '../components/focusSwitchButton.vue'
...
export default {
  ...
  mounted() {
    this.getIconMap()
    this.$bus.$on('tableInViewLoad', this.tableInViewLoaded)
  },
  beforeDestroy() {
    clearTimeout(this.timeout)
    this.destroyGraph()
    this.$bus.$off('tableInViewLoad')
  },
  watch: {
    anaLoading(newVal) {
      this.$bus.$emit('lineageLoad', newVal)
    },
    fixed(newVal) {
  ...
  methods: {
    tableInViewLoaded(val) {
      this.tableIsLoaded = val
    },
    // 将 icon 转成 base64
    getIconMap() {
      let iconMap = {}
    ...
    bindLineage() {
      ...
      this.$http
        .post(this.$meta_url + `/entities/view/${this.objectId}/tables/bind`)
        .then(res => {
    ...
    unbindSp() {
      this.$http
        .post(this.$meta_url + '/lineage/unbind/sp', this.lineageData)
        .then(res => {
    ...
    checkIfBound() {
      this.$http
        .get(this.$meta_url + `/entities/view/${this.objectId}/tables/bound`)
        .then(res => {
          this.bound = res.data
        })
        .catch(e => {
    ...
    analysize() {
      ...
      this.$http
        .post(
          this.$meta_url + `/entities/view/${this.objectId}/tables/analysis`
        )
        .then(res => {
          this.anaLoading = false
    ...
    getData() {
      ...
      if (type === 'REPORT') {
        ...
        const url = `${this.$meta_url}/lineage/object/${this.data.objectId}/type?type=${type}`
    ...
    exportExcel() {
      ...
      const url = `${this.$url}/service/lineage/excel?objectId=${this.data.objectId}&type=${type}`
      this.$downloadFile(url)
      // this.$downloadFile(this.$url + '/service/lineage/excel?objectId=' + this.data.objectId)
    },
    ...
    paintGraph(para) {
      ...
      setTimeout(() => {
        this.disableShowColumn =
          data.properties && data.properties.$table_lineage === true
        if (this.drawGraph) {
          this.drawGraph.destroy()
        }
        this.drawGraph = null
