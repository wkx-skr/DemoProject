...
export default {
  ...
  methods: {
    ...
    skip2Noti() {
      const url = '/base-app/#/main/userModal?currentNav=message'
    ...
    openUserModal() {
      const url = '/base-app/#/main/userModal?currentNav=myTodo'
      window.location.href = 'http://' + window.location.host + url
    },
    getLogs() {
      this.$http
        ...
        .then(() => {
          if (this.gatewayEnable) {
            if (window.setting.ssoLoginUrl) {
              ...
              location.href = '../base-app/datablau.html?product=dam'
            }
          } else {
            if (window.setting.ssoLoginUrl) {
              window.$ssoLogin(true)
