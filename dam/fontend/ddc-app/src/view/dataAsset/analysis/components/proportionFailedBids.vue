<template>
  <!-- 数据资产落标占比  proportionFailedBids -->
  <div style="background: #fff" class="picBox" v-loading="loading">
    <div class="picTil" v-if="!datas || datas.length === 0" style="width: 100%">
      <is-show-tooltip
        :content="$t('assets.dashboard.proportionFailedBids')"
        :open-delay="200"
        placement="top"
      >
        <template>
          {{ $t('assets.dashboard.proportionFailedBids') }}
        </template>
      </is-show-tooltip>
    </div>
    <template v-if="datas">
      <!--    数据资产落标占比(proportionFailedBids)-->
      <pieChartCom
        :picTil="$t('assets.dashboard.proportionFailedBids')"
        :datas="datas"
        :label="false"
        :options="options"
        :co="colors"
    ...
    getDistribution() {
      ...
      HTTP.getGuanwangTotalAsset()
        .then(res => {
          let ary = []
          ary.push({
            value: res.data.compliancePct.complianceY,
            name: '已落标',
            pre:
          ...
          ary.push({
            value: res.data.compliancePct.complianceN,
            name: '未落标',
            pre:
              (res.data.compliancePct.complianceN /
                (res.data.compliancePct.complianceY +
