      ...
      id &&
        HTTP.numberOfQuality(id)
          .then(res => {
            ...
            let totalAssetsNum = 0
            res.data.data.forEach(item => {
              totalAssetsNum = totalAssetsNum + item.assetsNums
              qualityNums.push(item.qualityNums)
              qualityRatio.push(item.assetsNums)
              dayTime.push((item.dayTime || '').slice(5))
            })
            ...
            let setOptions = {
              ...
              right: String(Math.max(...qualityNums, 100)).length * 10,
              left: Math.max(String(totalAssetsNum).length * 10, 25),
            }
