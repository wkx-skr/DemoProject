...
export default {
  ...
  methods: {
    drawEcharts() {
      let pieces = []
      let maxLength = 3
      let maxCount = Math.max(...(this.source || []).map(i => parseInt(i[1])))
      this.source.forEach((item, index) => {
        if (maxLength < String(item[1]).length) {
          maxLength = String(item[1]).length
        }
        pieces.push({
          data: [item],
          type: 'scatter',
          symbolSize: Math.ceil(parseInt(item[1]) * (50 / maxCount) + 10),
