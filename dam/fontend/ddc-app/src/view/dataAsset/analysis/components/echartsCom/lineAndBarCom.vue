...
关联:
- 被多个统计分析组件使用:
  - assetDistribution: 资产目录数据资产分布
  - assetDistributionByDept: 各部门数据资产分布
  - qualityProblemRate: 数据资产质量问题率
  - qualityProblemRateByCatalog: 各资产目录质量问题率
主要属性:
- picTil: 图表标题
- options: 图表配置项
- formatter: 是否启用格式化
      ...
      let categoryData = [
        {
          ...
          axisLabel: {
            formatter: function (value) {
              // 在这里对axisLabel的名称进行处理，可以截断或者换行
              if (value.length > 6) {
                return value.substring(0, 6) + '...' // 截断处理
              }
              return value
            },
          },
