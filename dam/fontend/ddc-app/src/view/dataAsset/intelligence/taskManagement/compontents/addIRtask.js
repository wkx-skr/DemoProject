...
export default {
  ...
  computed: {
    schemaShow () {
      return this.datasourceType !== 'SMBSHAREFILE' && this.datasourceType !== 'DATADICTIONARY' && this.datasourceType !== 'DATADICTIONARY_LOGICAL' && this.datasourceType !== 'FINE' && this.datasourceType !== 'YONGHONG' && this.datasourceType !== 'COGNOS' && this.datasourceType !== 'SMARTBI' && this.datasourceType !== 'DAVINCI' && this.datasourceType !== 'FINE_REPORT'
    }
  },
  props: {
    editMode: {
  ...
  mounted() {
    this.$refs.radio.$children.forEach((item) => {
      item.$children.forEach((v) => {
        v.$refs.radio.removeAttribute("aria-hidden")
      })
    })
  ...
  methods: {
    getDatasourceList() {
      API.getAssessFromre()
        .then(res => {
          this.datasourceList = res.data.data.filter(item => !item.parentId)
          // .filter(v => {
          //   return (
    ...
    handleDatasource(id) {
      this.datasourceType = this.datasourceList.find(v => v.modelId === id)?.type || ''
      if (id) {
        this.generalList = []
        this.form.elementObjectId = ''
        // if (this.datasourceType === 'DATADICTIONARY' || this.datasourceType === 'DATADICTIONARY_LOGICAL') {
        //   this.getFileSchemaList(id)
        //   return
        // }
        this.getAllDataSource(id)
      }
    },
    getFileSchemaList(id) {
      this.$http.get(`${this.$asstes_url}/discern/task/get/schema/${id}`)
        .then(res => {
          this.generalList = res.data.map(val => {
            return {
              modelId: val.objectId,
              definition:val.physicalName
            }
          })
          this.loading = false
        })
    ...
    getAllDataSource(id) {
      ...
      this.$http.get(`${this.$asstes_url}/metadata/model/${id}`)
        .then(res => {
    ...
    initForm() {
      ...
      this.datasourceType = this.datasourceList.find(v => v.modelId === this.taskDetail.datasourceId)?.type || ''
    },
    getDetail() {
      this.loading = true
    ...
    save() {
      ...
      let obj = {
        ...
        modelId: form.elementObjectId || form.datasourceId,
        ruleIds: ids,
        enabled: this.enable,
