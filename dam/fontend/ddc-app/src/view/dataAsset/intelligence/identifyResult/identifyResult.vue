<template>
  <div class="identify-result-page">
    <datablau-dialog
      ...
      width="500px"
      :isTree="true"
      :title="$t('intelligence.assetCatalog')"
    ...
    >
      <classify-tree
        ...
        :showCheckbox="true"
        :cloumnType="cloumnType"
        @update="updateSelectedAssetCatalogs"
      ></classify-tree>
      <div slot="footer">
        <datablau-button
          type="primary"
          @click="changeClassification"
          :disabled="!selectedAssetCatalogs.length"
          style="margin-left: 5px"
        >
          {{ $t('common.button.ok') }}
        </datablau-button>
        <datablau-button type="secondary" @click="cancelChangeCatalog">
        ...
        <datablau-form-submit>
          ...
          >
            <el-table-column
              ...
              v-if="$auth.RECOMMEND_RESULT_MANAGE"
              type="selection"
              width="20"
            ...
            <el-table-column width="28">
              <template slot-scope="scope">
                ...
                <div v-else>
                  ...
                  <datablau-icon
                    v-else
                    :data-type="$fileTypeFormatter(scope.row.fileType)"
                    :size="20"
                    style="position: relative; top: 3px; left: -3px"
                  ></datablau-icon>
                </div>
            ...
            <el-table-column
              :label="$t('intelligence.recommenAssetCatalog')"
              prop="catalogList"
              :min-width="getMinWidth(typeNum)"
            >
              <template
            ...
            <el-table-column
              :label="$t('intelligence.recommendedTags')"
              prop="levelObj"
              :min-width="120"
              show-overflow-tooltip
            >
              <!--<template scope="{row}">
                <el-tag type="success" v-for="v in row.levelList" :key="v.tagId" style="margin-right: 5px">{{v.tagName}}</el-tag>
              </template>-->
            </el-table-column>
            <el-table-column
            ...
            >
              <template scope="{row}">
                <el-tag type="success" v-for="v in row.levelList">
                  v.tagName
                </el-tag>
              </template>
          ...
          <template slot="buttons">
            <div class="bottom">
              <template
                v-if="selections.length > 0 && $auth.RECOMMEND_RESULT_MANAGE"
              >
                ...
                <datablau-button
                  ...
                  type="secondary"
                >
                  {{ $t('intelligence.assetCatalog') }}
                </datablau-button>
                <datablau-button @click="modifyTagEdit" type="secondary">
                  {{ $t('intelligence.modifyTag') }}
                </datablau-button>
                <!--<datablau-button
                  :disabled="noNext"
                  @click="sureSubmit"
