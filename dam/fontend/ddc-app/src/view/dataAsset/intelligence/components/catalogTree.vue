    ...
    <template v-if="treeList">
      <datablau-tree-header :noTitle="!showTitle">
        ...
        <template slot="search">
          ...
          <template v-if="!showTitle">
            <datablau-button
              v-if="type !== 'DISCERN_RULE'"
              type="icon"
              low-key
              :tooltip-content="$t('securityModule.new')"
              class="iconfont icon-tianjia"
              :disabled="!($auth.IDENTIFY_TASK_CATALOG_MANAGE || $auth.DATA_ASSET_DISCERN_ALGORITHM_CATALOG_MANAGE)"
              @click="openCatalogDialog('new', {})"
            ></datablau-button>
            <el-dropdown
            ...
            >
              ...
              <el-dropdown-menu slot="dropdown">
                <el-dropdown-item
                  command="add"
                  v-if="$auth.IDENTIFY_RULE_CATALOG_MANAGE"
                >
                  新建目录
                </el-dropdown-item>
                <el-dropdown-item
      ...
      <div class="tree-content" :class="{ 'tree-content-spe': !showTitle }" v-show="treeShow">
        <datablau-tree
      ...
      <div v-if="!treeShow" style="text-align: center">您暂无权限</div>
    </template>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    treeShow() {
      return (this.type === 'DISCERN_RULE' && (this.$auth.IDENTIFY_RULE_CATALOG_MANAGE || this.$auth.IDENTIFY_RULE_CATALOG_VIEW)) || (this.type !== 'DISCERN_RULE' && (this.$auth.IDENTIFY_TASK_CATALOG_MANAGE || this.$auth.IDENTIFY_TASK_CATALOG_VIEW)) || (this.type === 'DISCERN_ALGORITHM' && (this.$auth.DATA_ASSET_DISCERN_ALGORITHM_CATALOG || this.$auth.DATA_ASSET_DISCERN_ALGORITHM_MANAGE))
    }
  },
  data() {
  ...
  methods: {
    ...
    setAllAttr(type) {
      ...
      switch (type) {
        case 'DISCERN_ALGORITHM':
          ...
          params.manage = this.$auth.DATA_ASSET_DISCERN_ALGORITHM_CATALOG_MANAGE
          params.view = this.$auth.DATA_ASSET_DISCERN_ALGORITHM_CATALOG
          break
        case 'MASK_RULE':
          // 数据脱敏规则
        ...
        case 'DISCERN_TASK':
          // 识别任务管理
          params.manage = this.$auth.IDENTIFY_TASK_CATALOG_MANAGE
          params.view = this.$auth.IDENTIFY_TASK_CATALOG_VIEW
        ...
        case 'DISCERN_RULE':
          // 识别规则管理
          params.manage = this.$auth.IDENTIFY_RULE_CATALOG_MANAGE
          params.view = this.$auth.IDENTIFY_RULE_CATALOG_VIEW
    ...
    openCatalogDialog(type, data) {
      // if(!this.$auth.IDENTIFY_TASK_CATALOG_MANAGE) return
      if (type === 'new') {
        this.catalogDetails = {
          name: '',
          describe: '',
    ...
    dataOptionsFunction(data) {
      let options = []
      if (
        (this.from === 'rule' && this.$auth.IDENTIFY_RULE_CATALOG_MANAGE) ||
        (this.from === 'task' && this.$auth.IDENTIFY_TASK_CATALOG_MANAGE) ||
        (this.from === 'algorithm' && this.$auth.DATA_ASSET_DISCERN_ALGORITHM_CATALOG_MANAGE)
      ) {
    ...
    deleteCatalog(data) {
      ...
      let contName = data.name.length > 20? data.name.substring(0,20) + '……' : data.name
      if (data.subNodes && data.subNodes.length !== 0) {
        hasChild = true
        tipContent = this.$t('securityModule.delTip1', { name: contName })
      } else {
        tipContent = this.$t('securityModule.sureDelTip1', { name: contName })
        hasChild = false
      }
      this.$DatablauCofirm(
        tipContent,
    ...
    handleCommand(command) {
        switch (command) {
        case 'add':
          this.openCatalogDialog('new', {})
          break
        case 'import':
          this.$emit('importRule')
          break
        case 'export':
          this.$emit('exportRule')
          break
        default:
          break
      }
...
.catalogDialog {
  ...
  /deep/.datablau-input .el-input__suffix {
    right: 1px;
  }
  /deep/.el-input__icon {
    width: 14px;
