...
export default {
  ...
  data: function () {
    return {
      ...
      fullScreen: false,
      modelValueSingle: '',
      modelValueName: this.$t('common.dashboard.select'),
  ...
  methods: {
    ...
    handleContextMenu(component, evt) {
      ...
      options.push({
        icon: 'el-icon-delete',
        label: this.$t('common.dashboard.removeComponents'),
        callback: () => {
      ...
      options.push({
        icon: 'el-icon-bottom',
        label: this.$t('common.dashboard.wholeMoveDown'),
        callback: () => {
    ...
    removeComponent(component) {
      this.$confirm(
        this.$t('common.dashboard.moveConfirm', { name: component.title })
      )
        .then(() => {
    ...
    addComponent() {
      const getH5List = new Promise(resolve => {
        this.$http
          .post(this.$base_url + '/widget/getWidgetConfig?id=dashboard-h5-list')
          .then(res => {
            resolve(
    ...
    resetAll() {
      this.$confirm(
        this.$t('common.dashboard.resetConfirm'),
        this.$t('common.dashboard.resetTitle')
      )
        .then(() => {
          this.$http
    ...
    handleResize(component, args) {
      ...
      if (isValid) {
        ...
        this.$message.error(this.$t('common.dashboard.resizeErrorMessage'))
        if (this.dashboardProperties.dashboardName === 'dataQuality') {
          this.$bus.$emit('dashboardSearch')
        }
        this.resetLocation()
