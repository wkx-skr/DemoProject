...
mxEdgeHandler.prototype.redrawHandles = function()
{
	...
	if (this.bends != null && this.bends.length > 0)
	{
		...
		// this.bends[0].redraw();
    if (this.state.cell.isPolygon) {
      this.bends[0].fill = '#87CEFA'
      this.bends[0].redraw();
    }
...
mxEdgeHandler.prototype.redrawInnerBends = function(p0, pe)
{
	for (var i = 1; i < this.bends.length - 1; i++)
	{
        ...
        if (this.state.cell.isPolygon) {
          this.bends[i].fill = '#87CEFA'
          this.bends[i].redraw();
        } else {
          if(i === (this.state.cell.mxFirstIndex || 1)) {
            this.bends[i].fill = '#B2B2FF'
            this.bends[i].redraw();
          } else if(i === (this.state.cell.mxLastIndex || this.bends.length - 2)) {
            this.bends[i].fill = '#DC143C'
            this.bends[i].redraw();
          }else {
