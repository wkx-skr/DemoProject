  ...
  <div>
    ...
    <div class="banner-content">
      <div class="content-left">
        ...
        <datablau-button v-if="auth['DDD_DATAREPO_STANDARDEDIT']" class="addLayer iconfont icon-tianjia"  @click="openCreateLayer" type="important">
    ...
    <div class="layer-list-wrapper">
      ...
      >
        ...
        <el-table-column
          label="操作"
          v-if="auth['DDD_DATAREPO_STANDARDEDIT']"
...
export default {
  ...
  methods: {
    ...
    getLayerData () {
      this.$http.get(`${this.$dddUrl}/service/layers?currentPage=${this.page.current}&pageSize=${this.page.size}`).then(res => {
        let { data } = res
    ...
    createLayer () {
      this.$refs.form.validate(valid => {
        if (valid) {
          this.$http.post(`${this.$dddUrl}/service/layer`, {
    ...
    modifyLayer () {
      this.$refs.form.validate(valid => {
        if (valid) {
          this.$http.put(`${this.$dddUrl}/service/layer`, {
            layerId: this.layerData.layerId,
    ...
    deleteLayer (row) {
      this.$DatablauCofirm('确定要删除项目？').then(() => {
        this.$http.delete(`${this.$dddUrl}/service/layer/${row.id}`).then(res => {
