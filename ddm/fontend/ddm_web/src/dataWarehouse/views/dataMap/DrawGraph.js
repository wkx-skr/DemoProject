...
if (typeof mxGraph === 'undefined') {
  $(document.body).append('<script src="../../../../static/libs/mxgraph/mxClient.min.js"></script>')
}
const STD = {
  TABLE_WIDTH: 180,
  PADDING_TOP: 10,
...
class DrawGraph {
    ...
    try {
      ...
      if (self.param.showColumn) {
        self.crossModelLines.forEach(line => {
          const src = line.source && line.source - 0 !== 0 ? colCells[line.sourceStepId + line.source] : stepCells[line.sourceStepId]
          const dst = line.target && line.target - 0 !== 0 ? colCells[line.targetStepId + line.target] : stepCells[line.targetStepId]
        ...
        self.crossModelLines.forEach(line => {
          ...
          if (!paintedLines[line.sourceStepId + '' + line.targetStepId]) {
            paintedLines[line.sourceStepId + '' + line.targetStepId] = graph.insertEdge(parent, null, null, src, dst, 'normalEdge')
          }
        })
      }
    } finally {
  ...
  drawSingleModel (model, graph) {
    ...
    try {
      if (model.ddmModel) {
        modelCell = graph.insertVertex(parent, 'm:' + model.name, '系统:&nbsp;' + model.categoryName + '<br/>模型:&nbsp;' + model.name, 0, 0, 2000, 2000, 'ddmModelCell')
      } else {
      ...
      model.innerLines.forEach(line => {
        ...
        if (!paintedLines[src + dst]) {
          paintedLines[src + dst] = graph.insertEdge(modelCell, null, null, stepCells[src], stepCells[dst])
        }
      })
    ...
    try {
      ...
      if (self.param.showColumn) {
        model.innerLines.forEach(line => {
          const srcCell = line.source ? colCells[line.sourceStepId + line.source] : stepCells[line.sourceStepId]
          const dstCell = line.target ? colCells[line.targetStepId + line.target] : stepCells[line.targetStepId]

          graph.insertEdge(modelCell, null, null, srcCell, dstCell, 'normalEdge')
        })
      } else {
        ...
        model.innerLines.forEach(line => {
          ...
          if (!innerModelLines[src + dst]) {
            innerModelLines[src + dst] = graph.insertEdge(modelCell, null, null, stepCells[src], stepCells[dst], 'normalEdge')
          }
        })
      }
    } finally {
  ...
  prepareModelData () {
    ...
    for (var v in steps) {
      ...
      if (!self.stepGroup[modelName]) {
        self.stepGroup[modelName] = {
          ...
          categoryName: step.properties.modelCategoryName + '(' + step.properties.modelCategoryAbbr + ')',
          stepIds: {},
          steps: [],
          innerLines: []
        }
    ...
    self.crossModelLines.forEach(line => {
      ...
      self.interModelLines[srcModel.name + dstModel.name] = { src: srcModel.name, dst: dstModel.name }
    })
  }
    ...
    try {
      ...
      tLines.forEach(line => {
        if (!drewLines[line.source + line.target]) {
          drewLines[line.source + line.target] = graph.insertEdge(parent, null, null, ver[line.source], ver[line.target])
        }
      })
    } finally {
      if (callback) {
        ...
        try {
          ...
          try {
            self.data.lines.forEach(line => {
              ...
              graph.insertEdge(parent, null, null, colCells[src] ? colCells[src] : stepCells[src], colCells[dst] ? colCells[dst] : stepCells[dst], 'edgeStyle=datablauStyle')
            })
  ...
  paintBindedOverlays (graph) {
    ...
    for (let i = 0; i < self.lineageData.length; i++) {
      ...
      if (overlays == null) {
        const overlay = new mxCellOverlay(new mxImage('./static/images/bind_24.png', 24, 24),
          '<div style="padding:5px"><span style="font-weight:bold">已绑定:</span><br/><span style="width:45px;display:inline-block">系统</span>:&nbsp;' + step.modelCategoryName + '<br/><span style="width:45px;display:inline-block">' + (step.ddmStep ? '模型' : '数据源') + '</span>:&nbsp;' + step.modelName + '</div>'
          , mxConstants.ALIGN_RIGHT, mxConstants.ALIGN_TOP, offset)
  ...
  static propertyFormatter (value) {
    const map = new Map()
    map.set('$input_table', 'input_table')
    map.set('$output_table', 'output_table')
    map.set('databaseName', 'database_name')
    map.set('databaseType', 'database_type')
    map.set('port', 'port')
    ...
    if (msg.properties) {
      for (const key in msg.properties) {
        if (DrawGraph.propertyFormatter(key)) {
          ...
          titleString += `\r${DrawGraph.propertyFormatter(key)}:${value}`
        }
      }
    }
    if (!this.param.showColumn && !this.param.showModel && !msg.properties.databaseName) {
      // tableHtml = `<div style="width:${STD.TABLE_WIDTH}px" class="svg-col svg-title" title="${titleString}">${msg.name}</div>`
      tableHtml = `<div style="width:${STD.TABLE_WIDTH}px" class=" svg-title" title="${titleString}">${msg.name}</div>`
    } else {
      tableHtml = '<div style="width:' + STD.TABLE_WIDTH + 'px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;padding:0 0.5em;text-align: left;" class="'

      if (msg.properties.$output_table || msg.properties.$input_table || this.param.developerMode) {
      ...
      if (msg.properties.databaseName) {
        tableHtml += ' title="模型：' + msg.properties.databaseName + '\r' + msg.name + '">'
      } else {
        tableHtml += ' title="' + titleString + '">'
      }

      if (this.param.showColumn) {
        if (!msg.properties.$input_table && !msg.properties.$output_table && self.param.collapseSteps) {
          tableHtml += '<i class="fa fa-plus step-icon" aria-hidden="true" id="step-' + msg.id + '" style="float: right;padding: 3px 4px"> </i>'
        } else {
          tableHtml += '<i class="fa fa-minus step-icon" aria-hidden="true" id="step-' + msg.id + '" style="float: right;padding: 3px 4px"></i>'
        }
      }

      tableHtml += '<i class="iconfont icon-biao" style="font-size: 14px;margin-right: 5px"></i>' + msg.name + '</div>'
    }

    if (this.param.showColumn) {
      if (msg.columnsLength === 0) {
        result.stepCell = graph.insertVertex(parent, buildCellId(msg.id), tableHtml, 0, 0, STD.TABLE_WIDTH, 25, stepStyle)
      } else {
        result.stepCell = graph.insertVertex(parent, buildCellId(msg.id), tableHtml, 0, 0, STD.TABLE_WIDTH, STD.COL_HEIGHT * msg.columnsLength + 5, stepStyle)
      }
      ...
      msg.columns.forEach((col, index) => {
        result.columnCells[msg.id + col.id] = graph.insertVertex(
          result.stepCell,
          null,
          // '<div style="width:' + STD.TABLE_WIDTH + 'px" class="svg-col" title="' + col.name + '">' + col.name + '</div>',
          '<div style="width:' + STD.TABLE_WIDTH + 'px" class="" title="' + col.name + '">' + col.name + '</div>',
          0,
          index * STD.COL_HEIGHT + 5 ,
          STD.COL_WIDTH,
          STD.COL_HEIGHT,
          'colCell')
      ...
      try {
        ...
        if (addedCells) {
          for (i = 0; i < addedCells.length; i++) {
            cell = addedCells[i]
            if ((cell.isColumn && showColumn) || (cell.isStep && !showColumn)) {
  ...
  initStyles (graph) {
    ...
    style[mxConstants.STYLE_FILLCOLOR] = '#4e85f7'
    style[mxConstants.STYLE_FONTCOLOR] = '#fff'
    style[mxConstants.STYLE_ROUNDED] = 1
    style[mxConstants.STYLE_ARCSIZE] = 3
    style[mxConstants.STYLE_MOVABLE] = 1
    style[mxConstants.STYLE_FOLDABLE] = 1
    style[mxConstants.STYLE_STROKECOLOR] = '#4e85f7'
    ...
    style = mxUtils.clone(style)
    style[mxConstants.STYLE_FILLCOLOR] = '#dbdfee'
    style[mxConstants.STYLE_FONTCOLOR] = '#555'
    graph.getStylesheet().putCellStyle('selectedStepCell', style)

    style = mxUtils.clone(style)
    style[mxConstants.STYLE_FILLCOLOR] = '#ddd'
    ...
    style = mxUtils.clone(style)
    style[mxConstants.STYLE_FILLCOLOR] = '#f6ecd8'
    style[mxConstants.STYLE_STROKECOLOR] = '#ddd'
    style[mxConstants.STYLE_STROKEWIDTH] = 1
    style[mxConstants.STYLE_FONTCOLOR] = '#409eff'
    ...
    mxEdgeStyle.DatablauStyle = function (state, source, target, points, result) {
      const scale = source.width / source.unscaledWidth
      if (source != null && target != null) {
        var pt = new mxPoint(source.x + source.width, source.getCenterY())
        result.push(pt)
        pt = new mxPoint(source.x + source.width + (scale * 20), source.getCenterY())
        result.push(pt)
