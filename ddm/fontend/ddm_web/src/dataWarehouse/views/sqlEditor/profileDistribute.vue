<template>
  <div
    style="height: 40px; max-width: 128px; margin: 0 auto; cursor: pointer;background:transparent"
    @click="pushDetail"
  ></div>
</template>
...
export default {
  ...
  mounted () {
    const type = this.fullData.columnType
    if (
      ...
      // judge by data
      const sample = this.data.topSamples && this.data.topSamples[0]
  ...
  methods: {
    buildStringDistribution () {
      ...
      const topSamples = this.data.topSamples && this.data.topSamples
    ...
    buildNumberDistribution () {
      this.$el.style.background = '#F6F6F6'
      const topSamples = this.data.topSamples
      if (topSamples && topSamples.length > 0) {
        let width = 3
        let max = topSamples[0].count
        topSamples.forEach(item => {
