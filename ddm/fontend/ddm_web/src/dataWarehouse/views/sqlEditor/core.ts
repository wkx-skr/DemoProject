...
export class MyersDiff {
  ...
  public buildDiff<T> (path: PathNode, orig: Array<T>, rev: Array<T>): Array<LineContent> {
    let result: Array<LineContent> = []
    let resultIncludeLine: Array<LineContent> = []
    ...
    while (path && path.prev && path.prev.j >= 0) {
      if (path.isSnake()) {
        ...
        if (prei < i) {
          result.push({
            type: ContentType.Minus,
            content: String(orig[i - 1])
          })
        } else {
          result.push({
            type: ContentType.Plus,
            content: String(rev[j - 1])
          })
        }
      }
      path = path.prev
    ...
    const AddMiddleLine = () => {
      resultIncludeLine.push({
        type: ContentType.MiddleLine,
        content: '======='
      })
    }
    const AddBottomLine = () => {
      resultIncludeLine.push({
        type: ContentType.EndLine,
        content: '>>>>>>> Local(本地)'
      })
    }
    for (let i = 0; i < result.length; i++) {
      ...
      if (currentLine.type === ContentType.Normal) {
        currentLine.added = true
        resultIncludeLine.push(currentLine)
      } else {
        /**
         * 1.生成TopLine
         */
        resultIncludeLine.push({
          type: ContentType.TopLine,
          content: '<<<<<<< HEAD(线上)'
        })
        let minusFinish = false
        for (let j = i; j < result.length; j++) {
          /**
           * 2. 插入删掉的部分。
           */
          if (result[j].type === ContentType.Minus) {
            result[j].added = true
            resultIncludeLine.push(result[j])
          }
          /**
           * 3. 插入中间分隔符
           */
          if (result[j].type === ContentType.Plus || result[j].type === ContentType.Normal) {
            if (!minusFinish) {
              minusFinish = true
              AddMiddleLine()
            }
           ...
           * 4. 插入新增的部分
           */
          if (result[j].type === ContentType.Plus) {
            result[j].added = true
            resultIncludeLine.push(result[j])
          }
          /**
           * 5. 插入BottomLine
           */
          if (result[j].type === ContentType.Normal || (minusFinish && result[j].type === ContentType.Minus)) {
            AddBottomLine()
            break
          }
        }
        const lastLine = resultIncludeLine[resultIncludeLine.length - 1]
        if (lastLine.type === ContentType.Minus) {
          AddMiddleLine()
          AddBottomLine()
        }
        if (lastLine.type === ContentType.Plus) {
          AddBottomLine()
        }
      }
    }
    return resultIncludeLine
  }
  public buildDiffAutoMerge<T> (path: PathNode, orig: Array<T>, rev: Array<T>): string {
    let result: Array<LineContent> = []
    let resultIncludeLine: Array<LineContent> = []
    if (!path) throw new Error('path is null')
    if (!orig) throw new Error('original sequence is null')
    ...
    for (let i = 0; i < result.length; i++) {
      ...
      if (currentLine.type === ContentType.Normal || currentLine.type === ContentType.Plus) {
        currentLine.added = true
        resultIncludeLine.push(currentLine)
      }
      if (currentLine.type === ContentType.Minus) {
        let isOnlyMinus = true
        for (let j = i; j < result.length; j++) {
          if (result[j].type === ContentType.Plus) {
            isOnlyMinus = false
            break
          }
          if (result[j].type === ContentType.Normal) {
            break
          }
        }
        if (isOnlyMinus) {
          // 本行直接忽略,无需加入resultIncludeLine
        } else {
          resultIncludeLine.push({
            type: ContentType.TopLine,
            content: '<<<<<<< HEAD(线上)'
    ...
    return resultIncludeLine.map(i => i.content).join('\n')
  }
}
