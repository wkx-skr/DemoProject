...
import version from '@/dataWarehouse/views/dataIndex/damComponents/newDataStandard/version.vue'
...
import standardSelector from '@/dataWarehouse/views/dataIndex/damComponents/standardSelector.vue'
import FormValidator from '@/next/class/FormValidator'
import dimSelector from './dimSelector'
import ModifierSelector from '@/dataWarehouse/views/dataIndex/indexManagement/indexDefinition/component/ModifierSelector.vue'
import { ModifierCategory } from '@/dataWarehouse/views/dataIndex/modifier/entrance/Constant'
import {
  IndexPage,
  CategoryId
} from '@/dataWarehouse/views/dataIndex/indexManagement/indexDefinition/entrance/Constant'
...
export default {
  ...
  methods: {
    ...
    getPermission () {
      this.$http
        .post(
          `${this.$domains}metricManagement/metricAuth/getPermission?metricId=${this.currentItem.domainId}`
        )
        .then(res => {
    ...
    processUpdate (requestBody, closeAfterSuccess) {
      this.$http
        .post(`/metric/flow/domain/applyUpdate`,
          requestBody
        )
        .then(res => {
          this.submitLoading = false
    ...
    // 获取需求列表
    getRequirementData () {
      let url = `${HTTP.$domains}requirementmanagement/getall`
      this.$http
        .post(url)
        .then(res => {
