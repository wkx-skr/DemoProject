...
export default {
  ...
  methods: {
    getMappingList () {
      this.$http
        .post(`${this.$domains}metricManagement/mapping/list?metricId=${this.metricId}`)
        .then(res => {
    ...
    getTableDetail () {
      if (this.tableId) {
        this.$http
          .get(this.$damUrl + `/service/entities/${this.tableId}/summary`)
          .then(res => {
    ...
    searchColumns () {
      if (this.tableId) {
        ...
        const requestUrl = `${this.$damUrl}/service/entities/${this.tableId}/columns`
        this.$http
          .get(requestUrl)
          .then(res => {
    ...
    addOrUpdateMapping (row, newValue) {
      ...
      if (row.metricMappingId) {
        // update
        this.$http
          .post(`${this.$domains}metricManagement/mapping/update`, {
        ...
        this.$http
          .post(`${this.$domains}metricManagement/mapping/add`, {
            attrInfo: row.attrInfo,
            attrType: row.attrType,
            itemId: row.itemId,
            dimensionLevelId: row.dimensionLevelId,
    ...
    bindNullObject (requestBody) {
      return new Promise(resolve => {
        this.$http
          .post(`${this.$domains}metricManagement/mapping/add`, requestBody)
          .then(() => {
            resolve()
          })
          .catch(e => {
    ...
    realUnbindMapping (row) {
      this.$http
        .post(`${this.$domains}metricManagement/mapping/delete`, [row.metricMappingId])
        .then(() => {
          if (row) {
            row.objectId = null
