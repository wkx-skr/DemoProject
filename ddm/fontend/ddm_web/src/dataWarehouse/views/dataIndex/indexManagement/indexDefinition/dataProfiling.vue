...
<script>
import emptyPage1 from '@/dataWarehouse/views/dataIndex/indexManagement/indexDefinition/emptyPage1'
...
export default {
  ...
  methods: {
    getData () {
      this.$http
        .post(
          `${HTTP.$domains}metricManagement/dataPreview?metricId=${this.metricId}&currentPage=${this.currentPage}&pageSize=${this.pageSize}`
        )
        .then(res => {
          const tableData = []
          ...
          res.data.content.forEach((item, index) => {
            ...
            this.columns.forEach((c, subIndex) => {
              oneItem[c] = item[subIndex]
            })
            tableData.push(oneItem)
          })
