...
export default {
  ...
  methods: {
    ...
    getTreeData (oneStep) {
      ...
      url = `${this.$domains}categories/tree?type=82800023`
      this.$http
        .post(url)
        .then(res => {
    ...
    handleNodeClick (data, node) {
      ...
      this.$bus.$emit('updateDimensionItem', data)
    },
    // 节点展开
    nodeExpand (node) {
    ...
    deleteFolder () {
      if (this.folderData.parentId === 0) {
        ...
        this.$DatablauCofirm(
          ...
          .then(() => {
            let url = `${this.$domains}categories/delete?categoryId=` + this.folderData.id
    ...
    addfolder () {
      this.$refs.form.validate(valid => {
        if (!valid) {
          ...
          let url = `${this.$domains}categories/create`
          let body = {
          ...
          if (this.folderTitle === this.$t('indicator.demand.editCategory')) {
            url = `${this.$domains}categories/update?categoryId=` + this.folderData.id
            body = {
            ...
            if (
              ...
              url = `${this.$domains}categories/create`
              body = {
                description: this.formData.description,
                name: this.formData.name,
                parentId: this.folderData.parentId || null,
