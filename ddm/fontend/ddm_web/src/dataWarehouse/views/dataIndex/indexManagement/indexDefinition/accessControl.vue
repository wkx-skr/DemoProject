...
<script>
import accessFormRule from '@/dataWarehouse/views/dataIndex/indexManagement/indexDefinition/accessFormRule'
...
export default {
  ...
  methods: {
          ...
          this.$http
            .post(
              formData.id || formData.id === 0
                ? `${this.$domains}metricManagement/metricAuth/updatePermission`
                : `${this.$domains}metricManagement/metricAuth/createPermission`,
              [formData]
            )
            .then(res => {
              this.visible = false
    ...
    getPermissionList () {
      this.$http
        .post(
          `${this.$domains}metricManagement/metricAuth/getPermissionsList?metricId=${this.metricId}`
        )
        .then(res => {
    ...
    removePermission (row) {
      this.$DatablauCofirm(this.$t('indicator.access.sure'), '')
        .then(() => {
          const id = row.id
          this.$http
            .post(`${this.$domains}metricManagement/deleteMetricAuth`, [id])
            .then(() => {
              this.getPermissionList()
            })
            .catch(e => {
