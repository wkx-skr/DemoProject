...
export default {
  ...
  methods: {
    getData () {
      this.$http
        .post(
          `${this.$domains}metricManagement/metricWarning/list?metricId=${this.metricId}`
        )
        .then(res => {
    ...
    removeAlert (row) {
      this.$DatablauCofirm(`确定要删除预警信息“${row.name}”吗？`, '')
        .then(() => {
          this.$http
            .post(
              `${this.$domains}metricManagement/metricWarning/delete?metricWarningId=${row.id}`
            )
            .then(() => {
    ...
    getCurrentSchedule () {
      this.$http
        .post(
          `${this.$domains}metricManagement/metricWarning/getSchedule?metricId=${this.metricId}`
        )
        .then(res => {
    ...
    handleScheduleChange (schedule) {
      if (schedule) {
        this.$http
          .post(
            `${this.$domains}metricManagement/metricWarning/updateSchedule?metricId=${this.metricId}&schedule=${schedule}`
          )
          .then(() => {
