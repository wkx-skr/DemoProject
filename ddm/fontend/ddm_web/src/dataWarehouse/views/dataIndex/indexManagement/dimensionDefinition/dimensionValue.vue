<template>
  <div>
    <el-upload
      ...
      :action="$uploadUrlFormatter($domains + 'dimension/value/import?dimensionId=' + dataId)"
      ...
      <div class="row-main">
        ...
        <div class="table-area">
          <datablau-table :data="valueFormData.valueTableData">
            ...
            >
              <template slot-scope="scope">
                <datablau-button type="text" @click="addValueDelete(scope.row)">
...
export default {
  ...
  methods: {
    ...
    getValueUdp () {
      this.$http
        .post(
          `${this.$domains}dimension/udp/get?category=${
    ...
    // 查询全部维度树
    getValueTree () {
      let url = `${this.$domains}dimension/tree/get?id=${this.dataId}`
      this.$http
        .post(url)
        .then(res => {
    ...
    // 查询全部维度值
    getValue () {
      let url = `${this.$domains}dimension/list/get?id=${this.dataId}`
      this.$http
        .post(url)
        .then(res => {
          if (res.data) {
    ...
    valueDelete (scope) {
      this.$DatablauCofirm(
        ...
        if (arr.length > 0) {
          ...
          let url = `${this.$domains}dimension/value/delete`
    ...
    exportTemplate () {
      const url = `${this.$domains}dimension/value/downloadTemplate?dimensionId=${this.dataId}`
      this.$downloadFilePost(url)
    },
    exportValue () {
      const url = `${this.$domains}dimension/value/export?dimensionId=${this.dataId}`
      this.$downloadFilePost(url)
    },
    importValue () {
    ...
    submitValue () {
      this.$refs.valueForm.validate(valid => {
        if (!valid) {
          ...
          let url = `${this.$domains}dimension/value/maintain?dimensionId=${this.dataId}`
      ...
      this.$refs.addValueForm.validate(valid => {
        if (!valid) {
          ...
          if (this.addValueForm.index !== undefined) {
            ...
            let url = `${this.$domains}dimension/value/update`
