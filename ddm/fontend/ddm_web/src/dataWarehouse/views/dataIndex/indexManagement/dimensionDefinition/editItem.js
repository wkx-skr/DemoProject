...
export default {
  ...
  methods: {
    ...
    // 获取详情
    getFormData () {
      let url = `${HTTP.$domains}dimension/get?id=${this.id}`
      this.$http
        .post(url)
        .then(res => {
    ...
    // 查询全部维度值
    getValue () {
      let url = `${HTTP.$domains}dimension/list/get?id=${this.id}`
      this.$http
        .post(url)
        .then(res => {
          if (res.data) {
    ...
    onSubmit () {
      this.$refs.form.validate(valid => {
        if (!valid) {
          ...
          let url = `${HTTP.$domains}dimension/create`
          ...
          if (this.iscreate) {
            // 新建
            url = `${HTTP.$domains}dimension/create`
          } else {
            // 修改
            url = `${HTTP.$domains}dimension/update`
          }
          this.$http
            .post(url, body)
            .then(res => {
              ...
              let url = `${HTTP.$domains}dimension/value/create`
    ...
    // 获取目录树
    getTreeData () {
      let url = `${HTTP.$domains}categories/tree?type=82800023`
      this.$http
        .post(url)
        .then(res => {
    ...
    getTreeList (callback) {
      let url = `${HTTP.$domains}categories/get?type=82800023`
      this.$http
        .post(url)
        .then(res => {
