...
export default {
  ...
  methods: {
    ...
    handleAllShow () {
      ...
      this.$bus.$emit('updateThemeItem', { id: null })
    },
    clearTree () {
      this.$refs.tree.setCurrentKey(null)
    },
    ...
    getTreeData () {
      let url = this.$damUrl + '/service/models/modeltree'
      url = `${HTTP.$domains}categories/tree?type=${this.type}`
      this.$http
        .post(url)
        .then(res => {
    ...
    handleNodeClick (data, node) {
      ...
      this.$bus.$emit('updateThemeItem', data)
    },
    // 节点展开
    nodeExpand (node) {
    ...
    nodeCollapse (node) {
      // console.log(node,'node')
      this.defaultExpandedKeys = this.defaultExpandedKeys.filter(item => item !== node.id)
      // console.log(this.defaultExpandedKeys,'this.defaultExpandedKeys')
    },
    //
    deleteFolder () {
      ...
      if (this.folderData.parentId === 0) {
        ...
        this.$DatablauCofirm(
          ...
          .then(() => {
            let url = `${HTTP.$domains}categories/delete?categoryId=` + this.folderData.id
    ...
    addfolder () {
      this.$refs.form.validate(valid => {
        if (!valid) {
          ...
          let url = `${HTTP.$domains}categories/create`
          let body = {
          ...
          if (this.folderTitle === this.$t('indicator.demand.editCategory')) {
            url = `${HTTP.$domains}categories/update?categoryId=` + this.folderData.id
            body = {
            ...
            if (
              ...
              url = `${HTTP.$domains}categories/create`
              body = {
                description: this.formData.description,
                name: this.formData.name,
                parentId: this.folderData.parentId || null,
