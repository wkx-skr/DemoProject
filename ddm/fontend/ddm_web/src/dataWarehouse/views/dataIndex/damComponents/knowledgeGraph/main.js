...
export default {
  ...
  mixins: [Resource],
  props: ['summary', 'name', 'currentNode', 'permission', 'thema'],
  data () {
    const hierarchy = []
    for (let i = 1; i <= 100; i++) {
      hierarchy.push({
  ...
  mounted () {
    ...
    document.onclick = function (e) {
      ...
      if (o.tagName === 'CANVAS') {
        self.$nextTick(() => {
          console.log(self.$refs.hierarchySelect, 'self.$refs.hierarchySelect')
      ...
      this.edges.forEach(edge => {
        if (edge.from === element.id) {
          ...
          this.edges.forEach(edge1 => {
            if(edge1.from === edge.to) {
              this.removeSubNodes(this.nodes.get(edge1.to), true)
            }
          })
        }
      })
    ...
    getSearchNodeRelation (givenId, givenType, app = 'graph') {
      ...
      if (app === 'graph') {
        getData = this.$http
          .post(
            this.$graph_url +
            '/service/graph/searchNodeRelation?type=' +
            typeId +
            '&id=' +
            id +
            '&layers=' +
          ...
          if (!nodeId && edgeId) {
            ...
            this.nodesArr.forEach(element => {
              if (nodeId === element.id) {
                ...
                if (edgeArr.filter(edge =>edge.from === element.id).every(edge => {
