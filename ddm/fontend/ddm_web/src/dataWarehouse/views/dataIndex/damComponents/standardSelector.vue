...
export default {
  ...
  methods: {
    ...
    innerLoadStandardTree () {
      ...
      return new Promise(resolve => {
        ...
        if (this.folderValue === '1') {
          url = `${this.$domain}domains/tree/getTree`
        } else {
          url = `${this.$domains}domains/tree/getTree`
        }
        this.$http
          .post(url, obj)
    ...
    innerLoadStandard () {
      ...
      let url = ''
      if (this.folderValue === '1') {
        url = `${this.$domain}domains/domain/getPublicPage`
      } else {
        url = `${this.$domains}domains/domain/getPublicPage`
      }
      this.$http
        .post(url, obj)
