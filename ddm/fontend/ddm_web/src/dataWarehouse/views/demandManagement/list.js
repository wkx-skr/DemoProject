...
export default {
  ...
  methods: {
    ...
    getTreeData () {
      ...
      url = `${this.$domains}categories/tree?type=82800021`
      this.$http
        .post(url)
        .then(res => {
    ...
    handleNodeClick (data, node) {
      this.$emit('getNodeClickId', data.id)
    ...
    deleteFolder () {
      if (this.folderData.parentId === 0) {
        ...
        this.$DatablauCofirm(
          ...
          .then(() => {
            let url =
              `${this.$domains}categories/delete?categoryId=` + this.folderData.id
            this.$http
              .post(url)
              .then(res => {
