...
import indexEditor from '@/views/list/graph/editComponents/indexEditor.vue'
...
export default {
  name: 'tableDetails',
  props: ['nodeClickDataLayer', 'layerId', 'layerIdName', 'rawData', 'currentModel', 'dataByType', 'isLogicalModel', 'getTableHTMLFunction', 'graph', 'Changes', 'startIdToEndIds', 'createDeepRelation', 'calculateStartIdToEndIds', 'cellDialogData', 'currentStyleRelatedShapeTemplate', 'formatThemeTemplateData', 'currentId', 'isCassandraOrMongoDB', 'isLogical', 'tableDialogKey', 'isConceptual', 'getTableNameNamingMap', 'getColumnNameNamingMap', 'getIndexNameNamingMap', 'isShowSchema', 'isDesignModel', 'LayerEdit', 'TypeUniqueIdToColumnDefaultType', 'deliverNum', 'editorType', 'typeDataWareHouse', 'authPro', 'reNameColumnType', 'loading'],
  ...
  data () {
    ...
    return {
      ...
      currentRow: null,
      writable: true,
      // this.$store.state.lic.editor && (this.$store.state.user.isAdmin || this.currentModel.permission?.editor),
      admin: true,
      // this.currentModel.permission.admin,
      rules: {
        tableName: {
  ...
  mounted () {
    ...
    this.getEntitytemplateList()
    if (this.rawData.tableMsg.Layer) {
      this.$set(this.requestBody, 'layerId', this.rawData.tableMsg.Layer)
      this.getLayertreeDataFind()
    }
    if (this.rawData.SubjectId) {
      this.$set(this.requestBody, 'subjectId', this.rawData.SubjectId)
      // this.getThemetreeDataFind()
      // this.getLayerThemes(this.rawData.tableMsg.Theme)
    }
    if (this.rawData) {
      this.$set(this.requestBody, 'nameAbbreviation', this.rawData.nameAbbreviation)
    }
    if (this.rawData.editMode && this.nodeClickDataLayer.forceCheckFlag) {
      this.getSubjectTreeChildren()
    }
    // if (!this.rawData.editMode && !this.rawData.appendMode) {
    //   this.getVersionList()
    // }
    if (this.rawData.editMode || this.rawData.appendMode) {
      this.getLayerThemes(this.layerId)
    } else {
      this.getLayerThemesAll()
    }
    this.$nextTick(() => {
      if (this.rawData.editMode && this.nodeClickDataLayer.forceCheckFlag) {
        this.entitytemplateChange(this.nodeClickDataLayer.entityTemplateId)
      }
    })
    this.getCategories()
  },
  beforeDestroy () {
    if (this.editorType === 'table' && this.rawData.editMode) {
      this.unLockModel()
    }
  ...
  methods: {
    ...
    getThemeNamealias (id) {
      return this.themeList.filter(item => item.id === Number(id))[0]?.alias
    },
    getThemeName (id) {
      return this.themeList.filter(item => item.id === Number(id))[0]?.name
    },
    getLayerThemes (id) {
      this.$http
    ...
    getLayerThemesAll () {
      this.$http
        ...
        .then(res => {
          this.themeList = this.flattenTree(res.data.children)
        })
        .catch(e => {
          this.$showFailure(e)
        })
    ...
    flattenTree (arr) {
      return this.traverse(arr) // 调用traverse函数，并传入初始的树形结构数组
    },
    traverse (nodes, result = []) {
      for (let node of nodes) {
        result.push(node) // 将当前节点添加到结果数组中
        if (node.children && node.children.length > 0) {
          this.traverse(node.children, result) // 递归遍历子节点，并传入当前的结果数组
        }
      }
      return result
    },
    getVersionList () {
      console.log(this.rawData.tableMsg.Id, 'this.rawData.tableMsg.Id')
    ...
    getThemetreeDataFind () {
      this.$http
        .get(`${this.$url}/subject/tree`)
        .then(res => {
          this.$set(this.requestBody, 'theme', this.findNodeById([res.data], Number(this.rawData.tableMsg.Theme)).name)
        })
        .catch(e => {
          this.$showFailure(e)
        })
    },
    getLayertreeDataFind () {
      this.$http
        .get(`${this.$url}/layer/tree`)
        .then(res => {
          this.$set(this.requestBody, 'layer', this.findNodeById([res.data], Number(this.rawData.tableMsg.Layer)).name)
        })
        .catch(e => {
          this.$showFailure(e)
        })
    ...
    cutString (str, obj) {
      ...
      let end = str.lastIndexOf(suffix)
      if (start !== -1 && end !== -1) {
    ...
    getSubjectTreeChildren () {
      this.$http
        .get(`${this.$url}/subject/tree`)
        .then(res => {
          this.subjectTreeChildren = res.data.children
        })
        .catch(e => {
        })
    },
    ...
    getPropertyList (data) {
      this.$http
        ...
        .then(res => {
          ...
          // this.strongChecksPrefix = this.replaceWithKeyKey(data.prefix, res.data)
          // this.strongChecksSuffix = this.replaceWithKeyKey(data.suffix, res.data)
          // let replaceName = this.replaceWithKeyValue(this.requestBody.name, res.data)
          // this.$set(this.requestBody, 'name', this.cutString(this.requestBody.name, data))
        })
        .catch(e => {
          this.$showFailure(e)
        })
    ...
    getTranslationName (data, list) {
      list.forEach(element => {
        if (element.value === '%80020075%') {
          element.valueName = this.layerIdName
        } else if (element.value === '%80020076%') {
          element.valueName = this.getThemeNamealias(this.requestBody.subjectId)
        } else if (element.value === '%800200761%') {
          element.valueName = this.findAliasPath(this.subjectTreeChildren, this.requestBody.subjectId)
          // let path = this.themeList.filter(item => item.id === Number(this.requestBody.subjectId))[0]?.path
          // element.valueName = path?.split('/').slice(1).join('_')
        ...
        if (element.value.includes('{')) {
          // for (const [key, value] of this.udpMessageDisplay.entries()) {
          //   for (const [key2, value2] of value.entries()) {
          //     if (element.value.includes(key2)) {
          //       for (const [key3, value3] of this.dataByType.udp.entries()) {
          //         if (key3 === key2) {
          //           if (value3.UdpValueType === 'STRING' && value3.ExtendedEnumStruct) {
          //             let result = value2?.split(', ').map(num => {
          //               let foundItem = JSON.parse(value3.ExtendedEnumStruct).find(item => item.i === num)
          //               if (foundItem) {
          //                 return foundItem.n
          //               }
          //               return ''
    ...
    findAliasPath (arr, targetId, parentAlias = '') {
      for (const item of arr) {
        // 拼接当前 alias
        const currentAlias = parentAlias ? `${parentAlias}_${item.alias}` : item.alias

        // 如果找到目标 id，返回拼接后的 alias
        if (item.id === targetId) {
          return currentAlias
        }
        ...
        if (item.children && item.children.length > 0) {
          const result = this.findAliasPath(item.children, targetId, currentAlias)
          if (result) {
            return result // 如果找到目标节点，返回结果
          }
        }
      }
    ...
    moreClickHeightTable () {
      this.$nextTick(() => {
        this.$refs.tableDetailList.doLayout() // 重新计算表格布局
        setTimeout(() => {
          const height1 = $('.table-box1').height()
          const height2 = $('.table-box2').height()
          this.tableHeight = this.editMode ? height1 - 2 : height2 - 2
        }, 500)
      })
    },
    moreClick () {
      this.moreShow = !this.moreShow
      this.moreClickHeightTable()
    },
    moreEditClick () {
      this.moreShowEdit = !this.moreShowEdit
      this.moreClickHeightTable()
    },
    themeSave () {
      if (this.openThemeVisibleType === 'theme') {
        this.$set(this.requestBody, 'theme', this.themeCheckList.name)
        this.$set(this.requestBody, 'themeId', this.themeCheckList.id)
      } else {
        this.$set(this.requestBody, 'layer', this.themeCheckList.name)
        this.$set(this.requestBody, 'layerId', this.themeCheckList.id)
      }
    ...
    handleCheckChange (data, data2) {
      this.themeCheckList = data
    },
    openThemeVisible (type) {
      this.themeCheckList = []
      this.openThemeVisibleType = type
      this.getThemetreeData(type)
      this.addThemeVisible = true
    },
    getThemetreeData (type) {
      if (type === 'theme') {
        this.$http
          .get(`${this.$url}/subject/tree`)
          .then(res => {
    ...
    getEntitytemplateList () {
      this.$http.get(this.$url + '/service/entitytemplate/publish').then(res => {
        this.entitytemplateList = res.data
      }).catch(err => {
        this.$showFailure(err)
      })
    },
    ...
    flatFormatTransfer (obj, parentId) {
      ...
      res.properties[LDMTypes.EntityTemplate] = this.requestBody.entitytemplate
      if (this.rawData.editMode && this.nodeClickDataLayer.forceCheckFlag) {
      ...
      res.subjectId = this.requestBody.subjectId
    ...
    saveModel (request) {
      this.getTranslationName(this.entitytempSlicing, this.entitytempList)
      if (this.loading) {
      ...
      saveFlatFormat.updated = uniqBy(saveFlatFormat.updated, 'elementId')
      this.$http.put(`${this.$url}/service/layer/models/${this.currentModel.id}/save?ver=${request.lastVersion}`, saveFlatFormat).then(res => {
        if (res.data) {
          this.$emit('nameAbbreviationChange', this.requestBody.nameAbbreviation)
          this.$router.replace({
            query: {
          ...
          if (this.rawData.appendMode) {
            this.$emit('removeBlankTab', this.rawData.tableMsg.Id, this.currentModel.id)
          }
    ...
    heartBeat () {
      this.$http.put(`${this.$url}/service/layer/${this.currentModel.id}/${this.rawData.tableMsg.Id}/heart`).then(res => {
        if (!res.data) {
    ...
    unLockModel () {
      ...
      // }
      this.$http.post(`${this.$url}/layer/${this.currentModel.id}/${this.rawData.tableMsg.Id}/unlock`).then(res => {
        // if (res.data) {
    ...
    goToEdit () {
      ...
      this.$http.put(`${this.$url}/layer/${this.currentModel.id}/${this.rawData.tableMsg.Id}/lock`).then(res => {
        if (res.data) {
          ...
          this.$message.error('表尝试加锁失败，请重试')
        }
      }).catch(err => {
        this.$showFailure(err)
      })
    ...
    cancel () {
      ...
      if (this.rawData.appendMode) {
        ...
        this.$emit('removeTab', this.rawData.tableMsg.Id)
      } else {
      ...
      this.editMode = false
      this.rawData.editMode = false
      this.$set(this.rawData, 'editMode', false)
    },
    updateTableUdp () {
    ...
    addColumn (addDwItem = false) { // addDwItem 是否添加字段映射的item
      ...
      let body = {
        ...
        elementId: `#seed${++this.deliverNum.seed}#`,
        focusArea: [false, false, false],
        deleted: false,
        new: true
      }
    ...
    handleTableCNNameTab (cnName, checkRealTimeTranslate) {
      ...
      if (cnName && (!checkRealTimeTranslate || (checkRealTimeTranslate && this.dataByType.namingOption.IsUsingRealTimeTranslate))) {
        this.$http.post(this.$url + `/service/nametranslate/`, {
          ...
          if (res.data[0]) {
            if (this.rawData.editMode && this.nodeClickDataLayer.forceCheckFlag) {
              this.requestBody.nameAbbreviation = this.getTableNameNamingMap(res.data[0])
            } else {
  ...
  watch: {
    ...
    'rawData.editMode': {
      ...
      handler: function () {
        this.editMode = this.rawData.appendMode || this.rawData.editMode
        if (this.rawData.editMode && this.nodeClickDataLayer.forceCheckFlag) {
          console.log(this.nodeClickDataLayer, 'this.nodeClickDataLayer')
          this.entitytemplateChange(this.nodeClickDataLayer.entityTemplateId)
        }
      }
    },
  ...
  computed: {
    // computedTopStyle () {
    //   console.log(this.rawData.visible, 'this.rawData.visible')
    //   // console.log(this.rawData.editMode, this.editMode, 'this.rawData')
