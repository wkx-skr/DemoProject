      ...
      >
        <div class="content">
          ...
          <div v-if="manageType !== 'model'" class="left-checkbox" style="display: inline-block; float: left;height: 32px;line-height: 32px;margin-right: 5px;padding-left: 32px; color: #3a3e44;">
            ...
            <datablau-checkbox style="display: inline-block;margin-right: 20px;" v-model="applyToModel" checkbox-type="single">应用到表</datablau-checkbox>
            <datablau-checkbox style="display: inline-block;" v-model="applyToChildren" checkbox-type="single">应用到下级分层</datablau-checkbox>
          </div>
        </div>
        <div slot="footer">
        ...
        <div class="content" :class="{'content-black-theme': sqlEdit}" v-loading="dataLoading" v-if="usersDialog">
          ...
          <span class="footer">
            <div v-if="manageType !== 'model'" class="left-checkbox" style="display: inline-block; float: left;height: 32px;line-height: 32px;margin-right: 5px;">
              <datablau-checkbox style="display: inline-block;margin-right: 5px;" v-model="applyToModel" checkbox-type="single">应用到表</datablau-checkbox>
              <datablau-checkbox style="display: inline-block;" v-model="applyToChildren" checkbox-type="single">应用到下级分层</datablau-checkbox>
            </div>
  ...
  methods: {
    ...
    changeUserTypeConfirm (val, row) {
      ...
      if (this.manageType === 'model') {
        ...
        editUserPromise = HTTP.updateCategoryPermissionLayer2({
    ...
    handleDeletePermission (row) {
      ...
      if (this.manageType === 'model') {
        ...
        deletePromise = HTTP.updateCategoryPermissionLayer2({
          categoryId: this.categoryId,
          applyToChildren: this.applyToChildren,
          applyToModel: this.applyToModel,
          requestBody
    ...
    addUsers () {
      ...
      if (this.manageType === 'model') {
        ...
        addUserPromise = HTTP.updateCategoryPermissionLayer2({
          categoryId: this.categoryId,
          applyToChildren: this.applyToChildren,
          applyToModel: this.applyToModel,
          requestBody
    ...
    getModelPermissionUsers () {
      ...
      if (this.manageType === 'model') {
        ...
        getUserPromise = HTTP.getCategoryPermissionLayer(this.categoryId)
      }
      getUserPromise
        .then(res => {
          let data = this.manageType === 'model' ? res.data : res
          this.adminUsers = data.adminUsers.map(i => ({ ...i, type: 'adminUsers', typeBak: 'adminUsers' }))
          if (this.$store.state.user.isAdmin || this.$store.state.$auth['ROLE_DW_SUPERUSER_DDM'] || this.adminUsers.some(u => u.userId === this.$store.state.user.userId)) {
            this.editable = true
          } else {
            this.editable = false
