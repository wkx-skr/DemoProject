<template>
  ...
  >
    ...
    overflow: hidden;">{{rawData.tableMsg.Name}}</h3> -->
            <span  style="margin-left:10px;padding-bottom: 2px;">
              <datablau-button
                v-if="writable && !editMode && rawData.tableMsg.RawType !== 'View' && !authPro "
                @click="goToEdit"
                size="small"
                :disabled="!(rawData.permissionLi&&(rawData.permissionLi.admin || rawData.permissionLi.editor))"
                type="icon">
                <i class="iconfont icon-bianji" style="margin-right:5px;font-size:14px"></i>
              </datablau-button>
              <span v-else>
          ...
          <datablau-form
              ...
              <div style="display:flex">
                <el-form-item label="主题：" >
                  <p style="width:200px" v-if="editScan">{{ requestBody.subjectId?getThemeName(requestBody.subjectId):''  }}</p>
                  <p style="width:200px" v-else>{{ rawData.SubjectId?getThemeName(rawData.SubjectId):''  }}</p>
                </el-form-item>
              </div>
...
<!--    <div class="sub-title">基本信息</div>-->
        ...
        <div class="property">
          ...
          <span v-else class="value">
            <datablau-input
              ...
              :style="typeDataWareHouse?'width: 240px': ''"></datablau-input>
          </span>
        </div>
              ...
              <el-form @submit.native.prevent :model="requestBody" ref="tableNameRef" size="mini">
                <el-form-item style="height:28px;" :rules="rules.tableName" prop="tableName">
                  <datablau-input
                    ...
                    :themeBlack="typeDataWareHouse"
                    :style="typeDataWareHouse?'width: 240px': ''"
                  ></datablau-input>
                </el-form-item>
              </el-form>
            </span>
      ...
      <div v-else>
        <datablau-form
              ...
              <div style="display:flex">
                ...
                <el-form-item  :rules="rules.tableName" prop="tableName" v-if="!(nodeClickDataLayer.forceCheckFlag && this.rawData.editMode)">
                    <span slot="label" >
                      <span class="tableNameLabel required">表名</span>
                    </span>
                  <div style="display:flex;">
              ...
              <el-form-item label="主题"  v-if="moreShowEdit">
                    <datablau-select
                        v-model="requestBody.subjectId"
                        clearable
                        style="width:300px"
                    >
                        <el-option
              ...
              <el-form-item label="定义" v-if="moreShowEdit">
                <datablau-input
    ...
    top: 22px;">
          <datablau-button
            type="text"
            @click="moreEditClick"
          > {{moreShowEdit?'收起属性':'更多属性'}}
          <i v-if="moreShowEdit" class="iconfont icon-rightarrow"></i>
          <i v-else class="iconfont icon-downarrow"></i>
         </datablau-button>
        </div>
      </div>
...
<!--            <el-form-item label="TableType">
    ...
    <datablau-dialog
          ...
          :title="openThemeVisibleType === 'theme' ? '添加主题' : '添加分层'"
          size="s"
          append-to-body
          height="500px"
        >
          ...
          <span slot="footer">
            <datablau-button  @click="closeThemeVisible">
              取消
            </datablau-button>
            <datablau-button type="primary" @click="themeSave" :disabled="Object.keys(themeCheckList).length === 0||themeCheckList.id===0">
              确定
            </datablau-button>
          </span>
        </datablau-dialog>
...
&:hover{
  ...
  border:1px solid gba(64,158,255,.1);
}
    img {
      width: 16px;
    }
