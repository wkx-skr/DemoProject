  ...
  methods: {
      ...
      this.$refs.resetPassword.validate((valid, obj) => {
        if (!valid) {
          if (obj.hasOwnProperty('password')) {
            self.$message.error(obj.password[0]?.message || '密码错误')
            // alert('密码错误')
            ...
            .catch(e => {
              /* if (e.response.data.errorMessage) {
                e.response.data.errorMessage =
                  '密码修改失败:' + e.response.data.errorMessage
              } */
    ...
    initReset() {
      this.$refreshStrongPassword()
        .then(res => {
          this.strongPassword = this.$strongPassword
        })
        .catch(e => {
          this.$showFailure(e)
        })
    ...
    getDetails() {
      ...
      //   .then(res => {
      //     this.strongPassword = res === true
      //     this.$strongPassword = this.strongPassword
      //   }).catch(e => {
      //     this.$showFailure(e)
      //   })
      this.$http.get(`/user/usermanagement/users/details/self`)
        .then(res => {
    ...
    //   // TODO 需要先读取 base 的配置
    //   if (this.$damEnabled) {
    //     let damConfig = await HTTP.getDamPasswordConfig()
    //     strongPassword = damConfig.data.strongPassword
    //   } else {
    //     const ddmConfig = await this.$http.get(`${this.$url}/service/configs/configurable.user.force.strong.password`)
    //     strongPassword = ddmConfig.data?.propertyValue
    //   }
    //   return strongPassword === 'true' || strongPassword === true
    // },
    modifyDetails() {
      this.$http
