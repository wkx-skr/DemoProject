...
class DrawGraph {
    ...
    try {
      if (model.ddmModel) {
        modelCell = graph.insertVertex(
          parent,
          'm:' + model.name,
          '系统:&nbsp;' + model.categoryName + '<br/>模型:&nbsp;' + model.name,
          0,
          0,
          2000,
          2000,
      ...
      tLines.forEach(line => {
        if (!drewLines[line.source + line.target]) {
          drewLines[line.source + line.target] = graph.insertEdge(
            parent,
            null,
            self.param.showMiddleProcess ? line.relationType || '' : '',
            ver[line.source],
            ver[line.target]
          )
        }
  ...
  // logicalName: "CA1"
  // modelName: "CA1"
  // modelPathName: "模型库/等级模型/C'级模型/CA1"
  // modelRoleName: "C'级模型"
  // objectType: "80010001"
  // parentId: "24"
  static propertyFormatter (value) {
    ...
    map.set('modelRoleName', 'modelRoleName')
    map.set('modelName', 'modelName')
    map.set('logicalName', 'logicalName')
    map.set('application', 'application')
    if (map.has(value)) {
