...
export default {
  ...
  mounted () {
    ...
    //   navigator.sendBeacon(`${self.$url}/service/editor/${self.currentModel.id}/unlock`)
    //   this.$http.get(`${self.$url}/service/editor/${self.currentModel.id}/unlock`).then(res => {
    //
    //   }).catch((err) => {
  ...
  methods: {
        ...
        successCallback: (data) => {
          ...
          setTimeout(() => {
            if (this.diagrams.length) {
              ...
              // }
              let diagram = this.diagrams.find(d => d.Id === this.diagramNow)
              if (diagram) {
                this.handleDiagramClick(diagram)
              } else {
                this.handleDiagramClick({ Id: this.diagramNow })
                // HTTP.getModelDiagrams({ modelId: this.currentModel.id }).then(res => {
                //   this.$bus.$emit('update-model', res.data)
                //   setTimeout(() => {
                //     let diagram = this.mapIdToDiagramData[this.diagramNow]
                //     if (diagram) {
                //
                //     }
                //   })
    ...
    updateDiagramData () {
      this.$nextTick(() => {
        document.querySelectorAll('.grey-tree .el-tree-node.is-current').forEach(ele => {
