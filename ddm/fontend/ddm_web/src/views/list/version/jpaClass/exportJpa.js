  ...
  methods: {
    ...
    downloadScript () {
      if (this.currentJpaStr) {
        ...
        this.$message.info('没有JPA class可以导出')
      }
    },
    copyScript () {
      if (this.currentJpaStr) {
        string.setClipBoard(this.currentJpaStr)
        this.$message.success('JPA class已经拷贝到剪贴板，请在目的位置使用 Ctrl + V 进行粘贴')
      } else {
        this.$message.info('没有JPA class可以拷贝')
      }
    },
    callSetting () {
      ...
      this.$http
        ...
        .then(res => {
          if (res.data.success) {
            ...
            this.$showFailure(`模板保存失败:${errorMsg}`)
