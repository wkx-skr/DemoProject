  ...
  >
    ...
    <div v-if="!rawData.appendMode && !editMode" class="new-title-box" style="position: relative;">
      <div class="header clearfixed" :class="{'headerBlack':typeDataWareHouse}">
        <img style="margin-top:4px;" width="20"
          ...
          <div style="display:flex;align-items: center;">
            ...
            <span style="margin-left:10px;padding-bottom: 2px;">
              ...
              <datablau-button
                ...
                :themeBlack="typeDataWareHouse"><i class="iconfont icon-lineage"
                                                   style="margin-right:5px;font-size:14px"></i></datablau-button>
            </span>
          </div>
          <p style="color:#777">中文名称：{{ rawData.tableMsg.LogicalName }} <span v-if="!rawData.tableMsg.LogicalName"
                                                                                  style="color:#b0b0b0">暂无名称</span>
          </p>
        </div>
...
white-space: normal; ">{{ rawData.tableMsg.Definition }}</p> -->
    ...
    <div v-if="editMode" class="header-wrapper" v-show="!typeDataWareHouse">
      <h2 v-if="rawData.tableMsg.TypeId === 80100073"><img style="margin-right: 5px;vertical-align: text-bottom;"
                                                           :src="rawData.tableMsg && rawData.tableMsg.TypeId === 80500008 ? viewImg : rawData.tableMsg && rawData.tableMsg.TypeId === 80100073 ? businessImg : tableUsedImg"
                                                           width="18"/>{{ (rawData.tableMsg && rawData.tableMsg.TypeId === 80100073 ? '业务对象' : isLogical ? '实体' : '表') + '：' + requestBody.name }}
      </h2>
    ...
    <div v-if="editMode" class="top-area" :class="{'black-theme': typeDataWareHouse}">
      <div v-if="rawData.tableMsg.TypeId === 80100073">
        ...
        <div class="translate-wrapper" :style="typeDataWareHouse?'margin-left: 72px': ''"
             @click="handleTableCNNameTab(requestBody.cnName)" @mouseenter="translateActive=true"
             @mouseleave="translateActive=false">
          <span>翻译</span>
          <img :src="translateActive ? translateActiveImg : translateImg" alt=""/>
        </div>
        <div class="property">
          <span class="label"
                :class="{required:editMode,required2:!isLogical}">{{ (rawData.tableMsg && rawData.tableMsg.TypeId === 80100073 ? '业务对象' : isLogical ? '实体' : '表') + '名' }}</span>
      ...
      <div v-else>
        ...
        >
          <div style="display:flex;width:1200px">
            ...
            <el-form-item :rules="rules.tableName" prop="tableName"
                          v-if="!(currentModel.forceCheckFlag && this.rawData.editMode)">
                    <span slot="label">
    ...
    <div
      :class="{'content-box': true,'is-edit': editMode, 'editor-type-table': editorType === 'table', 'editor-type-model': editorType === 'model'}"
      :style="computedTopStyle" style="transition: top 0.5s ease-in-out;background:#fff">
      <datablau-tabs
        v-model="activeTab"
      ...
      >
        <el-tab-pane
          ...
          >
            <span
              slot-scope="{ option }">{{ option.properties.Name + (option.properties.LogicalName ? '（' + option.properties.LogicalName + '）' : '') }}</span>
          </datablau-transfer>
        ...
        <el-tab-pane :label="attributeName + '信息'" name="column">
          ...
          <div class="table-box"
               :class="{'table-box1':editMode,'table-box2':!editMode,'black-theme': typeDataWareHouse}">
            <datablau-table
                  ...
                  >
                    ...
                    >
                      <el-autocomplete
                        ...
                        :disabled="fk.indexOf(allCols[scope.$index].elementId) > -1 || $store.state.changeLeftPaneValue === 'dataStandard'"
              ...
              >
                ...
                <template slot-scope="scope">
                  <div class="operator-wrapper">
                    <datablau-button :disabled="fk.indexOf(allCols[scope.$index].elementId) > -1"
                                     :tooltipContent="fk.indexOf(allCols[scope.$index].elementId) > -1 ? `外键成员${attributeName}不允许被删除` : ''"
                                     type="text" @click="editRow(allCols[scope.$index])"
                                     style="height: 30px;font-size: 14px;"
                                     class="iconfont icon-bianji"></datablau-button>
                    <datablau-button :disabled="fk.indexOf(allCols[scope.$index].elementId) > -1"
                                     :tooltipContent="fk.indexOf(allCols[scope.$index].elementId) > -1 ? `外键成员${attributeName}不允许被删除` : ''"
                                     class="iconfont icon-delete" type="text"
                                     @click="deleteRow(allCols[scope.$index], scope.$index)"
                                     style="font-size:14px;height: 30px;"></datablau-button>
                    <datablau-button v-if="!isCassandraOrMongoDB" type="text" size="mini" @click.stop="up(scope.$index)"
                                     :disabled="scope.$index===0" class="iconfont icon-up"
                                     style="font-size:14px"></datablau-button>
                    <datablau-button v-if="!isCassandraOrMongoDB" type="text" size="mini"
                                     @click.stop="down(scope.$index)" :disabled="scope.$index===allCols.length-1"
                                     class="iconfont icon-down" style="font-size:14px"></datablau-button>
                  </div>
                </template>
              </el-table-column>
            </datablau-table>
            ...
            >
              ...
              >
                <template slot-scope="scope">
                  <span
                    v-if="!editMode">{{ $globalData.domainCodes && $globalData.domainCodes.map.get(allCols[scope.$index].dataStandardCode) }}</span>
                  <span
                    :title="$globalData.domainCodes && $globalData.domainCodes.map.get(allCols[scope.$index].dataStandardCode)"
                    v-else>
                    <el-input
                      size="mini"
              ...
              >
                <template slot-scope="scope">
                  ...
                  <datablau-checked
                    v-if="!editMode && pk.indexOf(allCols[scope.$index].elementId) > -1"></datablau-checked>
                </template>
              </el-table-column>
              <el-table-column
                label="外键"
              ...
              >
                <template slot-scope="scope">
                  ...
                  <i class="el-icon-check" style="font-weight:bold;font-size:16px;color:#6acf72;"
                     v-if="!editMode && allCols[scope.$index].notNull"></i>
                  <template v-if="editMode">
                    <span style="padding-left: 16px;"></span>
                    <el-checkbox
              ...
              >
                <template slot-scope="scope">
                  <udp-setting :themeBlack="typeDataWareHouse" :edit-mode="editMode" class="in-column-wrapper"
                               ...
                               :type="80000005" :udp-map="new Map([[udp.Id, allCols[scope.$index].allUdps[udp.Id]]])"
                               :data-by-type="dataByType" :request-body="allCols[scope.$index]">
                  </udp-setting>
              ...
              >
                ...
                <template slot-scope="scope">
                  <div class="operator-wrapper">
                    <datablau-button v-if="editorType === 'model'"
                                     :disabled="fk.indexOf(allCols[scope.$index].elementId) > -1"
                                     :tooltipContent="fk.indexOf(allCols[scope.$index].elementId) > -1 ? `外键成员${attributeName}不允许被删除` : ''"
                                     type="text" @click="editRow(allCols[scope.$index])"
                                     style="height: 30px;font-size: 14px;"
        ...
        <el-tab-pane v-if="rawData.tableMsg.TypeId === 80100073"
        ...
        <el-tab-pane label="自定义属性" v-else>
          <div :style="{top: 0}"
               style="position: absolute;top: 0px;bottom: 20px;left: 0px;right: 0px;overflow-y: auto;overflow-x: hidden">
            <div :key="key" v-for="key of udpMessageDisplay.keys()">
              <!-- TODO: 实体的自定义属性 -->
              <h2 class="udp-class-title-hint" style="font-size: 14px;margin-top: 20px;"
                  :style="typeDataWareHouse?'color:#bbbbbb':''">{{ key }}</h2>
              <udp-setting @myValidate="myValidate" ref="udpSetting" :currentModel="currentModel"
                           :type="rawData.tableMsg.TypeId" :themeBlack="typeDataWareHouse"
                           :udp-map="udpMessageDisplay.get(key)" :data-by-type="dataByType" :edit-mode="editMode"
                           :request-body="requestBody">
              </udp-setting>
            </div>
          </div>
        </el-tab-pane>
        <el-tab-pane class="code-generate" v-if="!isLogicalModel && !editMode"
                     :label="$store.state.$v.dataEntity.codeGenerate" name="Code" :lazy="true">
          <datablau-button
            type="important"
            size="default"
            @click="callSet"
          ...
          <datablau-tabs style="position: absolute;top: 10px;left: 0;right: 0;bottom: 0;" type="card"
                         v-model="activeTool" tab-position="top">
            <el-tab-pane
              label="JPA Class"
              name="jpa"
        ...
        <el-tab-pane label="分区" name="partition"
                     v-if="currentModel.TypeUniqueId === '0f691fa0-3fd7-42f1-b868-051b0ffad10c' || currentModel.TypeUniqueId === '2c85c9ca-d052-467c-8c55-d367d69982cf' || currentModel.TypeUniqueId === '647f241e-bf2f-7f19-6754-ec94e1c07bec'">
          <!--Mysql、PostgreSQL、iceberge-->
          <partion-editor
        ...
        <el-tab-pane label="Cluster" name="cluster"
                     v-if="currentModel.TypeUniqueId === '647f241e-bf2f-7f19-6754-ec94e1c07bec'"> <!--iceberge-->
          <cluster-editor
            style="padding-top: 10px;"
            ref="clusterRef"
            :allCols="allCols"
        ...
        <el-tab-pane label="Storage" name="storage"
          ...
          <datablau-form
            ...
            <el-form-item label="Location">
              <datablau-input :autosize="{ minRows: 2, maxRows: 5 }" resizeState type="textarea" style="width: 450px;"
                              :disabled="!editMode" v-model="requestBody.Location" clearalbe></datablau-input>
            </el-form-item>
          </datablau-form>
        </el-tab-pane>
        <el-tab-pane label="高级属性" name="highproperties"
                     v-if="currentModel.TypeUniqueId === '647f241e-bf2f-7f19-6754-ec94e1c07bec'"> <!--iceberge-->
          <datablau-form
            @submit.native.prevent
            label-width="110px"
            ...
            <el-form-item label="TblProperties">
              <datablau-input :autosize="{ minRows: 2, maxRows: 5 }" resizeState type="textarea" style="width: 450px;"
                              :disabled="!editMode" v-model="requestBody.TblProperties" clearalbe></datablau-input>
            </el-form-item>
          </datablau-form>
        </el-tab-pane>
        ...
        <el-tab-pane v-if="editorType === 'model'" label="样式" name="theme">
          <ul class="edit-style-dialog-content-wrapper" style="margin-top: 10px;">
            <li>样式：&nbsp;&nbsp;&nbsp;&nbsp;
              <datablau-select :disabled="!editMode" style="display: inline-block;width: 182px;" size="mini"
                               placeholder="请选择" v-model="currentStyleRelatedShapeTemplate.StyleThemeRef">
                <template v-for="themeId in Object.keys(dataByType.theme)">
                  <el-option v-if="!dataByType.theme[themeId].properties.deleted"
                             :key="dataByType.theme[themeId].properties.Name"
                             :value="dataByType.theme[themeId].properties.Id"
                             :label="dataByType.theme[themeId].properties.Name"></el-option>
                </template>
              </datablau-select>
            </li>
            <li v-if="rawData.tableMsg.TypeId !== 80100073" class="style-box">
              <!-- 标题背景： -->
              <el-color-picker :disabled="!editMode" style="margin-right: 15px;vertical-align: middle" show-alpha
                               v-model="currentStyleRelatedShapeTemplate.StyleBackColor2" size="mini"></el-color-picker>
              <span class="label-disc middle-y">标题背景</span>
              <opacity-component :disabled="!editMode" class="middle-y"
                                 :rgba.sync="currentStyleRelatedShapeTemplate.StyleBackColor2"></opacity-component>
            </li>
            <li class="style-box">
              <el-color-picker :disabled="!editMode" style="margin-right: 15px;vertical-align: middle" show-alpha
                               v-model="currentStyleRelatedShapeTemplate.StyleBackColor" size="mini"></el-color-picker>
              <span class="label-disc middle-y">背景颜色</span>
              <opacity-component :disabled="!editMode" class="middle-y"
                                 :rgba.sync="currentStyleRelatedShapeTemplate.StyleBackColor"></opacity-component>
            </li>
          </ul>
    ...
    <div v-if="editMode && editorType === 'table'" class="bottom-tool"
         :class="{'shadow': activeTab === 'column','bottom-tool-black': typeDataWareHouse}">
      <datablau-button
      ...
      <datablau-button
        ...
        :disabled="buttonLoading"
        :themeBlack="typeDataWareHouse"
      >{{ $store.state.$v.dataEntity.cancel }}
      </datablau-button>
    </div>
  </div>
</template>
