import DrawGraph from '@/views/modelLevel/lineage/DrawGraph.js'
...
export default {
  props: {
    ...
    type: {
      type: [String, Number], // businessArea
      default: '' // LogicalBusinessDomain  逻辑领域模型 ConceptualBusinessDomain  概念领域模型
    }
  },
  components: {},
  data () {
  ...
  methods: {
    getData () {
      ...
      let url = `${self.$url}/service/archy/object/object/Lineage?uuid=${this.modelId}`
      if (this.type) {
        url = `${self.$url}/service/archy/domain/domain/Lineage?uuid=${this.modelId}&modelType=${this.type}`
      }
      self.$http
        .get(url)
        .then(res => {
    ...
    dev (e) {
      if (e.keyCode === 46 && e.ctrlKey) {
        this.paridam.developerMode = !this.param.developerMode
        this.paintGraph()
      }
    },
    handleJson () {
                ...
                if (!prev || prev === item.source) {
                  ...
                  if (inputOrOutputSteps.has(item.targetStepId)) {
                    finalLines.push({
                      ...
                      relationType: (item.properties || {}).type || ''
                    })
                  } else {
    ...
    paintGraph () {
      ...
      lines.forEach(line => {
        ...
        if (tLinesString.indexOf(str) === -1) {
          ...
          tLines.push({ source: source, target: target, properties: line.properties, relationType: line.relationType })
        }
      })
