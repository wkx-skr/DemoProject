<template>
  <div class="dialog-outer" :class="{ 'en-page': $i18n.locale !== 'zh' }">
      ...
      <div v-for="(item, index) in formContent.standard" :key="index + 'upd'">
        <el-form-item :key="index + 'catalog'" :label="''">
          ...
          >
            <el-option
              :key="1"
              :value="$t('domain.domain.baseInfo')"
              :label="$t('domain.domain.baseInfo')"
            ></el-option>
            <!--//, BUSINESS, , ,-->
            <!--<el-option-->
            <!--  v-if="!standardCode && categoryTypeId !== 4"-->
            <!--  :key="2"-->
        ...
        <el-form-item :key="index + 'button'" :label="''" class="right-buttons" style="float: right;">
          <datablau-button
            type="icon"
            class="iconfont icon-delete"
            @click="remove(index)"
            :disabled="!(formContent.standard && formContent.standard.length > 1)"
          ></datablau-button>
          <datablau-button
            type="icon"
    ...
    <div class="dialog-bottom">
      ...
      <!--  @click="exporttUdp"-->
      <!--  type="primary"-->
      <!--  v-if="categoryTypeId === 1 || categoryTypeId > 6 || standardCode"-->
      <!--&gt;-->
