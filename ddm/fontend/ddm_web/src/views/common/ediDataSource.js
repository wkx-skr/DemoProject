...
export default {
  ...
  data () {
    ...
    // if (
    //   this.$versionFeature.metadata_Report &&
    //   this.$auth.METADATA_REPORT_VIEW
    // ) {
    //   dsTypeArr.push({
    //     type: 'report',
    //     name: this.$t('meta.dataSource.edit.report')
    //   })
    // }
    // if (this.$versionFeature.metadata_File && this.$auth.METADATA_FILE_VIEW) {
    //   dsTypeArr.push({
    //     type: 'file',
    //     name: this.$t('meta.dataSource.edit.file')
    //   })
    // }
    // if (
    //   this.$versionFeature.metadata_NonDBDataSourceType &&
    //   this.$settingIni.agentRe &&
    //   this.$settingIni.agentRe.enable
    // ) {
    //   dsTypeArr.push({
    //     type: 'Agent',
    //     name: 'Agent'
    ...
    // }
    return {
      modelFilters: dbType.DB_TYPE_TREE.subTree.slice(2, 3), // 取出关系线数据库
      reverseBaseUrl: '/ddm-re-instance',
      driverTypeDisabled: false,
      dbsVersion: '',
      preDbnames: [],
      originSelTables: '',
      originCheckboxes: {},
      ...
      sqlDBTypesArr: [
        ...
        { label: 'TDSQLMySQL', value: 'TDSQLMYSQL' },
  ...
  computed: {
    ...
    disableCommitButton () {
      if (this.dataSourceType !== 'report') {
        ...
        const displayName = _.trim(this.dsform.displayName || '')
        ...
        // if (this.uploadingData || !category || !displayName) {
        //   bool = true
        // }
    ...
    userPasswordRequ () {
      ...
      if (
        ds.dbtype === 'HIVE' ||
        ds.dbtype === 'ICEBERGONHIVESPARK' ||
        ds.dbtype === 'HUDIONHIVESPARK' ||
        ds.dbtype === 'IMPALA' ||
        ds.dbtype === 'HBASE' ||
        ds.dbtype === 'ES' ||
    ...
    showDbName () {
      let bool = true
      const dbArr = ['ES', 'HBASE', 'DB2I', 'HANA', 'DAMENG']
  ...
  created () {
    ...
    this.$set(this.dsform, 'GBaseMppVcName', null)
  },
  mounted () {
    // add custom props
    const defaultProp = [
    ...
    if (this.dsEditing) {
      ...
      if (this.dsform && this.dsform.original && this.dsform.original.type) {
        const typeSelectWraper = this.dsform.original.type
    ...
    if (!this.dsEditing) {
      ...
      if (this.dsform.dbtype === 'HBASE') {
        ...
        this.InstanceName =
          this.dsform.original.connectionInfo.parameterMap.InstanceName
      } else if (this.dsform.dbtype === 'DB2LUW') {
        ...
        if (
          ...
          let fileInfo = this.dsform.original.connectionInfo.parameterMap
      ...
      } else if (this.dsform.dbtype === 'GBASE') {
        this.GBaseMppVcName =
          this.dsform?.original?.connectionInfo?.parameterMap?.GBaseMppVcName ||
          ''
        if (this.GBaseMppVcName) {
          this.isMPP = true
        }
      }
      if (
        ...
        if (parameterMap.SelectedTables) {
          ...
          this.$set(this.dsform, 'SelectedViews', parameterMap.SelectedTables)
        }
      ...
      if (this.dsform.original.connectionInfo.connUrl) {
        this.customConnection = true
        this.$set(
          this.dsform,
          'connUrl',
          this.dsform.original.connectionInfo.connUrl
        )
        this.dbTypeSelected(this.AuthenticationType)
      }
      this.driverValue = this.dsform.original.driverId
  ...
  methods: {
    ...
    formatDataSource (
      ...
      //      json.parameterMap.DBVersion = ds.DBVersion;
      ...
      if (ds.SelectedTables) {
        ...
        json.parameterMap.SelectedViews = ds.SelectedTables
      }
      json.credentialInfo = {}
      if (isFileData) {
        ...
        json.parameterMap.ReFK = ds.ReFK
        json.parameterMap.SelectedBlackList = ds.SelectedBlackList
        json.parameterMap.BlackListAppliedTypeIds = ds.BlackListAppliedTypeIds
      ...
      if (ds.dbtype === 'SMBSHAREFILE') {
        json.parameterMap.sharePath = ds.sharePath
        json.parameterMap.description = ds.description
      }
      ...
      const showErrMsg = item => {
        ...
        // });
        // json = null;
        this.$datablauMessage.error(item)
        returnFalure = true
        return null
      }
      ...
      if (!isTest && useOfflineDs) {
        if (!OfflineDumpTargetDBName) {
          return showErrMsg('实例名称')
        }
        if (
          (ds.dbtype === 'ORACLE' || ds.dbtype === 'OCEANBASE-ORACLE') &&
          !OfflineDumpTargetSchemaName
        ) {
          return showErrMsg('schema')
        }
      }
      // 必选项 数据区域
      if (requireDataZone && !json.parameterMap.TagIds) {
        return showErrMsg('数据区域')
      }
      // 必选项 名称,系统
      // if (!json.sourceName || !json.categoryId) {
      //   showErrMsg('名称,系统')
      // }
      if (!isFileData && ds.connectType !== 'WebLogic') {
        if (!typeSelectWraper) {
          return showErrMsg('请选择数据库类型')
        }
        // 服务器与端口
        if (customConnection) {
          if (!ds.connUrl) {
            return showErrMsg('请输入连接串')
          }
        } else {
          if (!useCusDb && (!p.HostServer || (requireDbport && !p.PortNumber))) {
            return showErrMsg('请输入服务器与端口')
          }
        }
        if (AuthenticationType === 1) {
          // kerberos
          if (!p.ServicePrincipal) {
            return showErrMsg('kerberos服务认证规则')
          }
        } else if (
          AuthenticationType === 0 &&
          (!c.user || !c.password) &&
          ...
          // 用户名密码
          return showErrMsg('请输入用户名密码')
        }
        // 数据库名称
        if (dbNameRequ) {
          if (!p.DatabaseName) {
            return showErrMsg(`请输入${this.inputDBname}名称`) // 数据库名称
          } else if (isOracle && !ds.dbname) {
            return showErrMsg(`请输入${this.inputDBname}名称`)
          }
        }
        if (ds.dbtype === 'GBASE' && isMPP) {
          if (!ds.GBaseMppVcName) {
            return showErrMsg('vcname')
          }
        }
      }
      if (useOfflineDs) {
    ...
    toTrim () {
      this.$set(
        this.dsform,
        'displayName',
        this.dsform.displayName.replace(/\s+/g, '')
      )
    },
    // 查询推荐数据库版本
    getVersion () {
    ...
    dbTypeSelected (type) {
      ...
      /** default value */
      this.AuthenticationType =
        this.useOfflineDs && this.typeSelectWraper !== 'OFFLINEDUMP_RAW'
          ? 0
          : type || 0
      ...
      if (ds.dbtype === 'MYSQL' || ds.dbtype === 'OCEANBASEMYSQL' || ds.dbtype === 'TDSQLMYSQL' || ds.dbtype === 'GOLDENDB' || ds.dbtype === 'TIDB' || ds.dbtype === 'CBASE' || ds.dbtype === 'PolarDBMySQL') {
        ds.dbport = '3306'
      } else if (ds.dbtype === 'DAMENG') {
        ds.dbport = '5236'
      } else if (ds.dbtype === 'DORIS') {
        ds.dbport = '9030'
      } else if (ds.dbtype === 'CLICKHOUSE') {
        ds.dbport = '8123'
      } else if (ds.dbtype === 'ARGODB') {
        ds.dbport = '10000'
      } else if (ds.dbtype === 'VERTICA') {
        ds.dbport = '5433'
      } else if (ds.dbtype === 'MARIADB') {
        ds.dbport = '3307'
      } else if (ds.dbtype === 'POSTGRESQL' || ds.dbtype === 'POLAR-DB' || ds.dbtype === 'HASHDATA' || ds.dbtype === 'TDSQLPOSTGRESQL' || ds.dbtype === 'KINGBASEES') {
        ds.dbport = '5432'
      } else if (ds.dbtype === 'HOLOGRES') {
        ds.dbport = '80'
      } else if (ds.dbtype === 'GAUSSDBA') {
        ds.dbport = '25308'
      ...
      } else if (ds.dbtype === 'HIVE' || ds.dbtype === 'ICEBERGONHIVESPARK' || ds.dbtype === 'HUDIONHIVESPARK') {
        ds.dbport = '10000'
    ...
    // test 测试数据源
    async testDataSource ({ hideMsg } = {}, nextStep = false) {
      if (this.onlyDatabase) {
        this.dsform.dbname = this.preDbnames
      }
      this.disableTest = true
      ...
      if (this.dsform.dbtype === 'SMBSHAREFILE') {
        ...
        // )
        if (this.driverState) {
          let driver = this.driverOptions.find(driver => driver.id === this.driverValue)
          if (!driver) {
            this.$datablauMessage.error('请选择驱动')
            return
          }
          ds.driverValue = this.driverValue
          ds.driverClassName = driver.driverClassName
        } else {
          ds.driverValue = null
          ds.driverClassName = ''
        }
        ...
        // )
        try {
          if (!ds.dbname && ds.dbtype !== 'ORACLE' &&
            ds.dbtype !== 'DB2LUW' &&
            ds.dbtype !== 'INFORMIX' &&
            ds.dbtype !== 'MAXCOMPUTE') {
            let { data } = await this.$http.post(this.reverseBaseUrl + '/service/re/databases', {
              'address': ds.hostname,
              'port': ds.dbport,
              'database': '',
              'authDatabase': null,
              ...
              'type': ds.dbtype,
              'useKerberos': this.AuthenticationType === 1,
              'cred': {
                'user': ds.username,
              ...
              'kerberosInfo': this.AuthenticationType === 1 ? {
                keyTabPath: ds.KeyTabPath,
                servicePrincipal: ds.ServicePrincipal,
                userPrincipal: ds.UserPrincipal
              } : null,
            ...
            data.sort()
            if (nextStep) { // 下一步不需要修改当前数据库名称
              if (JSON.stringify(this.dataBaseNames) !== JSON.stringify(data)) {
                this.dataBaseNames = data
                this.dsform.dbname = this.dataBaseNames[0]
              }
              if (this.schemaSelected?.length) {
                return true
              } else {
                this.$showFailure('请选择schema')
                return false
              }
            } else {
              this.dataBaseNames = data
          ...
          let res = await this.$http.post(this.reverseBaseUrl + '/service/re/schemas', {
            'address': ds.hostname,
            'port': ds.dbport,
          ...
          // this.schemaSelected = [this.schemas[0]]
          // 选择当前schema
          let res1 = await this.$http.post(this.reverseBaseUrl + '/service/re/curschema', {
            'address': ds.hostname,
            'port': ds.dbport,
            'database': ds.dbtype === 'ORACLE' ? 'SID:' + ds.dbname : ds.dbname,
            'authDatabase': null,
          ...
          if (!nextStep) { // 下一步不需要成功提示
            if (res1.data) {
              this.schemaSelected = [res1.data]
            }
            this.$datablauMessage.success(this.$t('meta.dataSource.edit.datasourceTestSucceed'))
          } else {
            if (this.schemaSelected?.length) {
              return true
            } else {
          ...
          this.testSuccessed = true // 是否可用下一步
        } catch (err) {
          this.$showFailure(err)
          this.testSuccessed = false
          this.dbConnected = false // schema是否可选
          this.schemas = []
          this.schemaSelected = []
        } finally {
          this.disableTest = false // 测试loading
        }
        ...
        // )
        try {
          let res = await this.$http.post(this.reverseBaseUrl + '/service/re/databases', {
            'address': ds.hostname,
            'port': ds.dbport,
            'database': '',
            'authDatabase': null,
          ...
          if (!nextStep) {
            ...
            if (JSON.stringify(this.dataBaseNames) !== JSON.stringify(res.data)) {
              this.dataBaseNames = res.data
              this.preDbnames = this.dataBaseNames[0]
            }
          }
    ...
    getDbName (json, successcallback, failureCallback) {
      ...
      if (this.dsform.dbtype === 'GBASE' && this.isMPP) {
        json.parameterMap.GBaseMppVcName = this.GBaseMppVcName
        json.parameterMap.DatabaseName = null
      }
      this.$http
    ...
    handleDbnameChange (value) {
      ...
      if (this.onlyDatabase) {
        ...
        if (
          ...
          this.dsform.dbtype !== 'DB2LUW' &&
          this.dsform.dbtype !== 'MAXCOMPUTE' &&
          value
        ) {
    ...
    emitData () {
      ...
      // console.log(this.dsform.displayName);
    ...
    addOldDataSource () {
      ...
      if (
        ...
        /* this.$DatablauCofirm(
          this.$t('meta.dataSource.edit.updateAboutDbsConfirm'),
          this.$t('meta.dataSource.edit.tips'),
          'info',
          {
            showCancelButton: true,
    ...
    handleIEUpload () {
      this.dsform.files = $('input[type="file"]')[0].value
      setTimeout(() => {
        this.addDataSource()
      }, 3000)
    },
    ...
    getSchemas (json, successcallback, failureCallback) {
      ...
      this.$http
        ...
        .then(res => {
          ...
          // console.log(this.testSuccessed)
          if (!this.dsEditing) {
            if (this.typeSelectWraper !== 'OFFLINEDUMP') {
              this.targetType = 'schema'
    ...
    async getDriverType (type) {
      if (!type) return
      this.driverTypeDisabled = true
      try {
        let res = await this.$http
          .get(this.$url + `/service/driver/isJdbc?type=${type}`)
        this.customConnectionState = res.data
      } catch (err) {
        this.customConnectionState = true
        this.$showFailure(err)
      }
      this.$http
  ...
  watch: {
    disableCommitButton () {
      this.$emit('changeDisabledStepState', this.disableCommitButton)
    },
    ...
    GBaseMppVcName (newVal) {
      this.dsform.GBaseMppVcName = newVal
    }
