...
import createVirds from './placeholder/createVirDs.vue'
import createView from './placeholder/createView.vue'
  ...
  methods: {
    dataInit () {
      ...
      this.formDtos.forEach(item => {
        if (item.code === 'applyUser') {
          this.userInfo.userName = item.value
        }
        if (item.code === 'applyContent') {
          Object.assign(this.userInfo, JSON.parse(item.value))
        }
        if (item.code === 'applyViews') {
          this.showViewList = JSON.parse(item.value)
        }
      })
      const arr = []
      this.userInfo.targetTableIdArr.forEach((item, index) => {
        ...
        succesedCallback: data => {
          ...
          if (this.type === 1 || this.type === 3) {
            result.views = result.views.filter(
              view =>
                !!this.showViewList && this.showViewList.includes(view.uuid)
            )
          }
        },
        failureCallback: e => {
    ...
    handleChooseView (data, view) {
      ...
      this.$bus.$emit('viewData', this.applaTableData)
    },
    skipToFinishedPage () {
      this.$router.push({
        name: 'finishedPage',
