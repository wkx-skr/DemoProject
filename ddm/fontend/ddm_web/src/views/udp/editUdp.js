...
export default {
          ...
          if (udpArr && Array.isArray(udpArr)) {
            udpArr.forEach(item => {
              ...
              if (this.udpData.udpId) {
                ...
                let children = this.udpMap[this.udpData.udpId]?.children || []
  ...
  methods: {
    dDisabledChange (row) {
      let arr = []
      this.udpData?.enumValues.forEach(element => {
        if (!this.udpData.multiSelect) {
          if (element.d && element.p === '') {
            arr.push(element.i)
          }
        } else {
          if (element.d && element.p === '') {
      ...
      this.udpData.defaultValue = arr.join(',')
    },
    multiSelectChange () {
      this.udpData?.enumValues.forEach(element => {
        element.d = false
        element.dDisabled = false
      })
    ...
    confirmPost () {
      ...
      if (requestBody.enumValues && Array.isArray(requestBody.enumValues) && requestBody.enumValues.length > 0) {
        ...
        requestBody.enumValues.forEach(item => {
          ...
          if (item.d) {
            noPaerntArr.push(item.i)
          }
          if (item.p) {
            if (!parentKeyMap[item.p]) {
              parentKeyMap[item.p] = []
            }
            parentKeyMap[item.p].push(item)
          }
        })
        noPaerntKey = requestBody.enumValues.every(item => item.p === '')
        if (enumValueEmpty) {
        ...
        if (!this.udpData.multiSelect) {
          if (noPaerntKey) {
            if (noPaerntArr.length > 1 && !this.udpData.multiSelect) {
              this.$datablauMessage.error('枚举值列表父键下有多个默认值')
              return
            }
          }
          for (let parentKey in parentKeyMap) {
            let itemsWithSameParentKey = parentKeyMap[parentKey]
            let trueCount = itemsWithSameParentKey.filter(item => item.d).length
            if (trueCount > 1) {
              this.$datablauMessage.error(`枚举值列表父键下有多个默认值`)
              return
            }
          }
          let noParentItems = requestBody.enumValues.filter(item => !item.p)
          let trueCountInNoParentItems = noParentItems.filter(item => item.d).length
          if (trueCountInNoParentItems > 1) {
            this.$datablauMessage.error('枚举值列表父键下有多个默认值')
            return
          }
        }
      ...
      if (this.isAdd) {
        let order = this.udpObjArr[this.udpData.targetTypes].length
        this.udpObjArr[this.udpData.targetTypes].forEach(udp => {
          if (udp.order > order) {
            order = udp.order
          }
        })
        requestBody.order = order + 1
      }
      let para = {
