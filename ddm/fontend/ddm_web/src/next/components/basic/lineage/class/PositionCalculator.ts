...
class PositionCalculator {
  ...
  public calculate(): Map<string, Position> {
    if ([GraphType.SCHEMA_AND_TABLE_AND_COLUMN, GraphType.SCHEMA_AND_TABLE, GraphType.CATEGORY_AND_TABLE_AND_COLUMN, GraphType.CATEGORY_AND_TABLE, GraphType.MODEL_AND_TABLE, GraphType.MODEL_AND_TABLE_AND_COLUMN].includes(this.graphType)) {
  ...
  */
  ...
  private getSchemaOrCategory(stepId): string {
    ...
    if (this.graphType == GraphType.SCHEMA_AND_TABLE || this.graphType == GraphType.SCHEMA_AND_TABLE_AND_COLUMN) {
      ...
      } else if (step.properties.modelCategoryName) {
        return step.properties.modelCategoryName
      } else {
        return '未分组'
      }

    } else if ([GraphType.MODEL_AND_TABLE, GraphType.MODEL_AND_TABLE_AND_COLUMN].includes(this.graphType)) {
      if (step.properties.modelName) {
        return step.properties.modelName
      } else {
        return '未分组'
      }
    } else {
    ...
    const calculateGroupInner = (stepIds, schema) => {
      ...
      filteredRows.forEach((row, rowIndex) => {
        ...
        row.forEach((step, idx) => {
          ...
          if (this.graphType == GraphType.SCHEMA_AND_TABLE_AND_COLUMN || this.graphType == GraphType.CATEGORY_AND_TABLE_AND_COLUMN || this.graphType === GraphType.MODEL_AND_TABLE_AND_COLUMN) {
