    ...
    action: {
      ...
      handler: function (newVal, oldVal) {
        if (typeof newVal === 'string' && newVal.length > 0) {
          this.actionFormatter = this.$uploadUrlFormatter(newVal)
        } else {
          this.actionFormatter = newVal
        }
      }
    }
  },
  ...
  methods: {
    ...
    previewList() {
      ...
      this.showList.map(item => {
        const url = `${this.$url}/service/files/${item}/download`
        urlArr.push(url)
      })
      return urlArr
    },
...
.datablau-upload {
  ...
  &.black-theme{
    /deep/ .el-upload-dragger{
      background-color: #187FFF !important;
