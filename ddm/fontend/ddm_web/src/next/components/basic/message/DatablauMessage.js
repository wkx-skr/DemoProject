...
function blauShowFailure() {
  ...
  if (status === 401) {
    if (window.setting.ssoLoginUrl) {
      ...
      window.location.href = '/datablau.html'
    }
...
function showMessage(options) {
  if (window.localStorage.getItem('editorTheme') === 'black' && window.location.href.indexOf('sql_editor') !== -1){
    Message({
      ...options,
      customClass: 'datablau-messagetip datablau-messagetipBlack',
    });
  } else {
    Message({
      ...options,
...
// // 对 没有权限的报错进行节流
// const noPermissionMessage = '由于权限不足，访问被拒绝，请联系管理员授权'
// const noPermissionMessageShow = _.throttle(showMessage, 10000)
