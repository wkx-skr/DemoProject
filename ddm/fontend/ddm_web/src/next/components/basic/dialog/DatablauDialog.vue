...
export default {
  ...
  props: {
    testName: String,
    // 当有 table 时，滚动事件绑定回调函数，处理是否显示阴影
    table: {
      type: Boolean,
      default: false,
    },
  ...
  watch: {
    ...
    height(val) {
      this.$nextTick(() => {
        if (val) {
          $(this.$refs[this.ranDialog].$refs.dialog).css({
            height: parseFloat(val) + 'px',
            'max-height':
              parseFloat(val) >= 750 ? 750 + 'px' : parseFloat(val) + 'px',
          })
        }
        this.initMethod()
      })
  ...
  beforeDestroy() {
    ...
    this.$nextTick(() => {
      ...
      let scrollInnerDom = this.scrollDom ? $(this.scrollDom).children()[0] : this.$refs[contentRan]
      if (scrollInnerDom) {
        scrollInnerDom.removeEventListener('scroll', this.scrollChange, true)
        scrollInnerDom.removeEventListener('resize', this.scrollChange, true)
      }
    })
  ...
  mounted() {
    this.scrollChange = _.throttle(this.scrollChangeThrottled, 200)

    this.ran = Math.random()
    this.ranDialog = 'dialog' + this.ran
    window.onresize = this.initDialog
    this.initDialog()
  },
  methods: {
    // 窗口 size 改变时触发初始化，校验是否显示阴影
    initDialog() {
      this.$nextTick(() => {
        setTimeout(() => {
          ...
          if (this.table) {
            ...
            // 绑定事件
            const contentRan = 'contentInner' + this.ran
            this.scrollContainer = this.scrollDom || this.$refs.content
            let scrollInnerDom = this.scrollDom ? $(this.scrollDom).children()[0] : this.$refs[contentRan]
            this.scrollInnerDom = scrollInnerDom
            $(this.scrollContainer).off('scroll', this.scrollChange)
            $(this.scrollContainer).on('scroll', this.scrollChange)
            if (scrollInnerDom) {
              scrollInnerDom.removeEventListener('scroll', this.scrollChange)
              scrollInnerDom.removeEventListener('resize', this.scrollChange)
              scrollInnerDom.addEventListener('scroll', this.scrollChange)
              scrollInnerDom.addEventListener('resize', this.scrollChange)

              const config = {attributes: true, childList: true, subtree: true}
              const observe = new MutationObserver(this.scrollChange)
              observe.observe(scrollInnerDom, config)
            }
            this.scrollChange()
          }
        })
    ...
    scrollChangeThrottled() {
      // 滚动事件发生后，检查是否显示阴影
      const contentRan = 'contentInner' + this.ran
      let scrollContainer = this.scrollDom || this.$refs.content
      let scrollInnerDom = this.scrollDom ? $(this.scrollDom).children()[0] : this.$refs[contentRan]
      if (!scrollContainer || !scrollInnerDom) {
        return
      }
      const handler = () => {
        const boxH = $(scrollContainer).height()
        const contentH = $(scrollInnerDom).height()
    ...
    initMethod() {
      ...
      if (
        this.$refs[this.ranDialog] && $(this.$refs[this.ranDialog].$refs.dialog).height() -
          this.screenHeight <
        0
      ) {
        this.$refs[this.ranDialog] && $(this.$refs[this.ranDialog].$refs.dialog).css(
          'marginTop',
          (this.screenHeight -
        ...
        this.$refs[this.ranDialog] && $(this.$refs[this.ranDialog].$refs.dialog).css('marginTop', '0px')
      }
    },
    close(...args) {
