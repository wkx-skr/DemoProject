...
import {VerticalAlign} from "@/next/service/lineage/types/Configuration";
import Version from "@/next/service/lineage/class/Version";
import {VersionContent} from "@/next/service/lineage/types/Version";
...
class LineageGraph {
  ...
  public constructor({data, container, param}) {
    ...
    Object.keys(this.steps).forEach(key => {
      ...
      if (this.param.showColumn) {
        step.height = Configuration.TABLE_HEIGHT + Configuration.COLUMN_HEIGHT * step.columns?.length + Configuration.COLUMN_MARGIN
      } else {
        step.height = Configuration.TABLE_HEIGHT
      }
      step.width = Configuration.TABLE_WIDTH
    })
  ...
  */
  ...
  private handleParams(): void {
    ...
    this.usePositionFromVersion = !this.param.showMiddleProcess && this.param.showColumn && !this.param.groupBySchema && !this.param.groupByCategory;
  }
  ...
  private calculatePosition(): void {
    ...
    this.positionCalculator = new PositionCalculator(this.tLines, this.steps, this.showModelMessage, this.graphType, (this.param.directionTB ? 'TB' : 'LR'));
    ...
    this.exploredXY = this.positionCalculator.getExploredXYDagre();
  }
  ...
  private drawGroups(): void {
    if ([GraphType.SCHEMA_AND_TABLE, GraphType.SCHEMA_AND_TABLE_AND_COLUMN, GraphType.CATEGORY_AND_TABLE, GraphType.CATEGORY_AND_TABLE_AND_COLUMN].includes(this.graphType)) {
  ...
  */
  private drawStep(step: Step) {
    ...
    if (step.properties.$input_table === 'true') {
      stepType = StepType.INPUT
    }
    if (step.properties.$output_table === 'true') {
    ...
    if ([
      ...
      step.columns && step.columns.forEach((col, index) => {
        if (position == null) {
          throw new Error('table position not found');
        }

        let columnPosition = this.positions.get(step.id);
        ...
        let columnShape = new ColumnShape(
          ...
          position.x,
          position.y + Configuration.TABLE_HEIGHT + index * Configuration.COLUMN_HEIGHT,
          col.name,
          shape
        )
  ...
  private createView(): void {
    ...
    if ([GraphType.TABLE_ONLY, GraphType.SCHEMA_AND_TABLE, GraphType.CATEGORY_AND_TABLE].includes(this.graphType)) {
      view.setEdges(this.tLines);
    } else if ([GraphType.TABLE_AND_COLUMN, GraphType.SCHEMA_AND_TABLE_AND_COLUMN, GraphType.CATEGORY_AND_TABLE_AND_COLUMN].includes(this.graphType)) {
