...
class PositionCalculator {
  constructor(tableLines: Array<TableLine>, steps: Map<string, Step>, showModelMessage: boolean, graphType: GraphType = GraphType.TABLE_ONLY, direction: string = "LT") {
    if (showModelMessage) {
  ...
  private exploredXDagre: number = Configuration.PAGE_MARGIN_LEFT;
  private exploredY: number = Configuration.PAGE_MARGIN_TOP;
  private exploredYDagre: number = Configuration.PAGE_MARGIN_TOP;
  ...
  private positionsDagre: Map<string, Position> = new Map();
  private positionsDagreEdge: Map<string, Position> = new Map();
  ...
  public getAllPositions(): Map<string, Position> {
    // return this.positions;
    return this.positionsDagre;
  }

  public getAllEdgePositions(): Map<string, Position> {
    return this.positionsDagreEdge;
  }
  ...
  public getExploredXYDagre(): Location {
    return {
      x: this.exploredXDagre,
      y: this.exploredYDagre + Configuration.PAGE_MARGIN_TOP, // 最终的返回值需要增加下边距
    }
  }
 ...
 */
  ...
  private calculatePositions(): void {
    ...
    let TABLE_HEIGHT = Configuration.TABLE_HEIGHT
    let TABLE_WIDTH = Configuration.TABLE_WIDTH
    for (let key in this.steps) {
      let value = this.steps[key]
      g.setNode(key, {label: value.name, width: value.width, height: value.height});
    }

    this.tableLines.forEach((value, key) => {
      g.setEdge(value.source, value.target);
    });
    ...
    g.nodes().forEach((v) => {
      ...
      if (x + node.width / 2 > this.exploredXDagre) {
        this.exploredXDagre = x + node.width / 2
      }
      if (y + node.height / 2 > this.exploredYDagre) {
        this.exploredYDagre = y + node.height / 2
      }

    });
    this.positionsDagreEdge = new Map()
    g.edges().forEach((e) => {
      this.positionsDagreEdge.set(`${e.v}_${e.w}`, g.edge(e))
      // console.log(g.edge(e), 'eg.edge(e)')
    });
