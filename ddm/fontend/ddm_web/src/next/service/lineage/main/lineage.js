...
require('@/assets/style/_page.scss')
...
export default {
  ...
  created() {
    ...
    this.param.showMiddleProcess = this.showMiddleProcess

    // window.vueThis = this.prototype
  },
  mounted() {
    // this.isDark = (localStorage.getItem('editorTheme') || window.setting.editorTheme) === 'black'
  ...
  computed: {
    ...
    // },
    showControlLine () {
      return !!this.controlOptions.showColumn || !!this.controlOptions.groupBySchema || !!this.controlOptions.groupBySchema || !this.disableShowColumn
    }
  },
  watch: {
    theme (newVal) {
  ...
  methods: {
    ...
    getData() {
      ...
      this.getLineageDataPromise()
        .then(res => {
          const columnsAttrs = [
            {label: '对象类型', value: 'objectType'},
            {label: '模型类型', value: 'modelRoleName'},
            {label: '模型路径', value: 'modelPathName'},
          ...
          Object.keys((rawData.steps || {})).forEach(key => {
            let item = rawData.steps[key] || {}
            item.columns = []
            if (item.properties.objectType !== 'Archy主题') {
              columnsAttrs.forEach((attr, index) => {
                let obj = {
                  id: `${item.id}_${index}`,
                  name: `${attr.label}：${(item.properties || {})[attr.value] || ''}`,
                  order: index,
                  stepId: `${item.id}_${index}`,
                  parentId: item.id,
                }
                item.columns.push(obj)
              })
            } else {
              [{label: '对象类型', value: 'objectType'}].forEach((attr, index) => {
                let obj = {
                  id: `${item.id}_${index}`,
                  name: `${attr.label}：${(item.properties || {})[attr.value] || ''}`,
                  order: index,
          ...
          rawData.lines.forEach(item => {
            let source = item.source
            let sourceStepId = item.sourceStepId
            item.source = item.target
            item.sourceStepId = item.targetStepId
            item.target = source
            item.targetStepId = sourceStepId
          })
    ...
    paintGraph() {
      ...
      lines.forEach(line => {
        ...
        if (tLinesString.indexOf(str) === -1) {
          ...
          tLines.push({source: source, target: target, label: line.label})
        }
      })
      this.startDraw(data)
    },
