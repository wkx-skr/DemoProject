        ...
        setTimeoutInterval() {
            setInterval(_ => {
                if (localStorage.getItem('hold-on')) {
                    ...
                    if (this.cachedData?.getAbout?.maxInterval && new Date().getTime() - lastTimestamp > this.cachedData.getAbout.maxInterval * 1000) {
                    ...
                    return new Promise((resolve, reject) => {
                        if (this.cachedData[name]) {
                            ...
                            if (!this.temporaryPromise[name]) {
                                this.temporaryPromise[name] = this.http[this.urls[name].method ? this.urls[name].method: 'post'](this.urls[name].url, this.urls[name].requestBody ? this.urls[name].requestBody : null)
                            }
                            this.temporaryPromise[name].then(res => {
            ...
            this.stablePromise[name] = () => {
                return new Promise((resolve, reject) => {
                    if (this.cachedData[name]) {
                        ...
                        this.stablePromise.getAllRolesOfCurrentProduct().then(allRolesOfProduct => {
                            if (!this.temporaryPromise[name]) {
                                this.temporaryPromise[name] = this.http.post('/gateway/main/getUserInfo')
                            }
                            this.temporaryPromise[name].then(res => {
            ...
            this.stablePromise[name] = () => {
                return new Promise((resolve, reject) => {
                    if (this.cachedData[name]) {
                        ...
                        this.temporaryPromise[name].then(res => {
                            this.cachedData[name] = res.data[localStorage.getItem('currentProduct').toUpperCase()]
                            resolve(this.cachedData[name])
                        }).catch(e => {
                            reject(e)
                        })
            ...
            this.stablePromise[name] = () => {
                return new Promise((resolve, reject) => {
                    if (this.cachedData[name]) {
                        ...
                        this.stablePromise.getAllServersOfCurrentProduct().then(ServerConfig => {
                            if (!this.temporaryPromise[name]) {
                                this.temporaryPromise[name] = this.http.post('/user/role/service', ServerConfig)
                            }
                            this.temporaryPromise[name].then(res => {
                                this.cachedData[name] = res.data
                                resolve(this.cachedData[name])
