...
import { createProcessDefinition, recordFileMapp } from '@/service/modules/process-definition'
  ...
  setup() {
    ...
    const onSave = ({
      ...
      createProcessDefinition(
        ...
        const fileMapping =  taskDefinitions.map((v) => {
          return {
            "fileDetailId": v.taskParams?.fileDetailId,
            'fileType': v.taskParams?.fileType,
            "processId": ignored.code,
            "processChildName": [saveForm.name + '/' + v.name]
          }
        }).filter(item => item.fileDetailId && item.processId)
        fileMapping.length && recordFileMapp(fileMapping).then(res => {})
