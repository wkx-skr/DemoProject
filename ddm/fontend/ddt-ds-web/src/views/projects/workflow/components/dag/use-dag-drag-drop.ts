  ...
  function onDrop(e: DragEvent) {
    ...
    if (dragged.value && graph.value && projectCode) {
      ...
      genTaskCodeList(genNums, projectCode).then((res) => {
        const [code] = res
        themeStore.setCreatSqlTask(null)
        appendTask(code, type, { x: x - eX, y: y - eY })
      })
    }
  }
  function sqlPainting(obj: objOption) {
    if (readonly.value) {
      return
    }
    let { ev } = obj
    ...
    const genNums = 1
    if(graph.value && projectCode) {
      // ev.fileWidth 目录左侧宽度，68，工作流页面距离上面的位置
      const { x, y } = graph.value.clientToLocal(ev.clientX-ev.fileWidth, ev.clientY-68)
      genTaskCodeList(genNums, projectCode).then((res) => {
        const [code] = res
        // @ts-ignore  120,12 是左侧树节点的中心位置
        appendTask(code, typeObj[obj.type] || 'SQL', { x: x - 120, y: y - 12 })
      })
    }
  }
