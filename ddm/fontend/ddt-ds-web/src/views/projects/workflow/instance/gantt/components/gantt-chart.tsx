...
const props = {
  height: {
    ...
    default: window.innerHeight - 110
  },
  width: {
    type: [String, Number] as PropType<string | number>,
    default: '100%'
...
const GanttChart = defineComponent({
  ...
  setup(props) {
    ...
    props.seriesData.forEach(function (task, index) {
      ...
      let stateText = task.status as ITaskState
      let stateIndex: string
      if(stateText.indexOf('desc') !== -1) {
        let reg = /\{.*?\}/g
        // @ts-ignore
        let str =  stateText.match(reg)[0].replace(/=/g, ':')
        // @ts-ignore
        let json =  eval("(" + str + ")")
        stateIndex = json.desc.toUpperCase()
      } else {
        stateIndex = task.status as ITaskState
      }
      // @ts-ignore
      data[stateIndex].push({
        name: task.name,
        value: [index, start, end, end - start],
        itemStyle: {
          // @ts-ignore
          color: state[stateIndex].color
        }
      })
    })
