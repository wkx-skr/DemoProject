...
export function formatParams(data: INodeData): {
  ...
  if (data.taskType === 'SQL') {
    ...
    taskParams.isNewest = data.isNewest
    taskParams.fileDetailId = data.fileDetailId
    taskParams.fileType = data.fileType
    taskParams.branch = data.branch
  ...
  if (data.taskType === 'KETTLE') {
    taskParams.kettleFilePath = data.kettleFilePath
    taskParams.kettleFileType = data.kettleFileType
  }
