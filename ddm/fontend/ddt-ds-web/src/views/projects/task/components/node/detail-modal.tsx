...
const NodeDetailModal = defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const onConfirm = async () => {
      ...
      let value = detailRef.value.value.getValues()
      if(!value.sql && value.taskType === 'SQL'){
        message.error('请选择数据模型或开发程序且文件有内容')
        return
      }
    ...
    const onNext = async () => {
      await detailRef.value.value.validate()
      // @ts-ignore
      taskStore.setStepNum(taskStore.getStepNum + 1)
    }
    const onPrev = async () => {
      await detailRef.value.value.validate()
      // @ts-ignore
    ...
    return () => (
      <Modal
        ...
        sqlModel={sqlModel}
        class={Styles.modalLog}
      >
       {/* {props.data.taskType === 'SQL' &&
          <div>
            <div class={Styles.tilLine}>
              {taskStore.getStepNum === 0 ? '基础信息' : taskStore.getStepNum === 1 ? '任务设置' : '数据源设置' }
            </div>
            <div class={Styles[`line${taskStore.getStepNum}`]}/>
          </div>
        }*/}
        <Detail
          ref={detailRef}
