...
export function sqlUseSqlType(model: { [field: string]: any }): IJsonItem[] {
  const { t } = useI18n()
  const querySpan = computed(() => (model.sqlType === '0' ? 6 : 0))
  const isNewestSpan = computed(() => (model.sqlType === '0' ? 6 : 6))
  const switchQuerySpan = computed(() => (model.sqlType === '0' ? 10 : 0))
  const nonQuerySpan = computed(() => (model.sqlType === '1' ? 10 : 10))
  ...
  return [
   ...
   /* {
    ...
    //   field: 'displayRowsTips',
    //   span: 24,
    //   widget: h('div', { class: styles['dividing-line'] })
    // }
  ]
}
